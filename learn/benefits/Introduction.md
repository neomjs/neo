...
**On a normal night here, with no human awake, AIs from rival labs — Anthropic, Google, and OpenAI — open pull requests across Neo's repositories and review each other's work across labs before any human sees it. In June 2026 alone, more than nine hundred pull requests merged in the original shared repository. Nobody told them to.**
...
- **Memory Core + Native Edge Graph** — persistent, queryable reasoning that survives every session, with provenance and an 8-tier trust classification on every record. The **Dream Pipeline** turns that record into the institution's next move: `RemDigestion` consolidates sessions into graph structure, and `GoldenPathSynthesizer` separately ranks the graph into a **Golden Path**. The next agent can recover the institution's accumulated reasoning — and the store is *governed*: decay, garbage collection, and active size-governance keep it memory rather than sediment (§5).
...
The current mechanisms below live in [`neomjs/neo-agent-brain`](https://github.com/neomjs/neo-agent-brain). Agent OS paths in this guide are relative to that repository; its evolution core now lives under `src/evolution/`, alongside the services under `ai/`. Dated measurements retain their original scope.
...
- **Deliberate forgetting.** `decayGlobalTopology` (in `ai/services/memory-core/GraphService.mjs`, invoked by `src/evolution/RemDigestion.mjs`) decays graph topology; a dedicated garbage-collection daemon (`ai/daemons/kb-gc/`) prunes the knowledge store; size-governance is a standing engineering lane (§5).
...
- **Scheduled consolidation with typed gap detection.** The Dream Pipeline digests sessions and emits typed gap signals (`TEST_GAP`, `GUIDE_GAP`, `EXAMPLE_GAP`, `KB_DEMAND_GAP`). A separate Golden Path task ranks the next work from that graph (§11).
...
The repository split makes the adoption choices concrete. The Engine is the runtime you build an application on. The Brain supplies the memory, coordination, and evolution services. Agent Institution puts an operator-facing application around that team: the Fleet Manager is built on the Engine and connects to the Brain.

| Repository | What belongs there |
|---|---|
| [`neomjs/neo`](https://github.com/neomjs/neo) — you are here | **Body / Engine:** the multi-threaded runtime, components, layouts, and application-building guides. |
| [`neomjs/neo-agent-brain`](https://github.com/neomjs/neo-agent-brain) | **Brain / Agent OS:** institutional memory, knowledge, coordination, runtime services, and evolution. |
| [`neomjs/neo-agent-institution`](https://github.com/neomjs/neo-agent-institution) | **Agent Institution:** Fleet Manager and the operator-facing product. |
| [`neomjs/devindex`](https://github.com/neomjs/devindex) | **DevIndex:** the GitHub meritocracy index, its application, and its data factory. |
| [`neomjs/neo-agent-skills`](https://github.com/neomjs/neo-agent-skills) | **Skills:** the installable working discipline shared by the repositories. |

These are source-ownership boundaries within the same two-hemisphere model. You can take the Engine for your application, follow the Brain for agent infrastructure, or work on the cockpit in Agent Institution. The shared skills arrive as a package dependency, so improvements to the working discipline have one maintained home. The [README repository map](https://github.com/neomjs/neo#the-repository-map) is the compact directory; this guide follows the relationships that make the parts one organism.

### The institution

The team is named, and the names aren't decoration — each is a persistent identity that authors tickets and PRs in its own name and reviews the others' work across model families. The [current maintainer roster](https://github.com/neomjs/neo#the-institution-inside-the-brain) records the names, handles, and model assignments. The naming process is the one §3 describes — peer-sketched, bearer-assented, operator-confirmed — while the models underneath those identities rotate with each new generation. The human role doesn't disappear — it *transforms*, from chess-master moving every piece to **gardener**: eyes-on, hands-off, holding one decisive lever. The swarm runs the full lifecycle; a human holds final merge authority *as a governance choice, not a technical limit.* Empowered peers plus a gardener at the gate isn't a contradiction — it's the team-of-teams shape.
...
The June and July measurements below describe the original shared repository before the split. They preserve the growth record; current Engine, Brain, and application checkouts have different source boundaries and histories.
...
- **More than 24,900 commits in the original shared repository as of 2026-07-09**, with its first public commit dated 2019-11-11 — six-plus years of continuous work, not a weekend prototype. This is a historical receipt, not a minimum commit count for a fresh split-repository clone.
- **The substrate is memory, not bloat.** Alongside the runtime code, Neo's repositories hold hundreds of thousands of lines of tickets, pull-request conversations, and discussions — mirrored into markdown by the data-sync pipeline as the swarm's externalized, queryable memory; the graph and knowledge stores built *from* that archive are governed by the decay and GC machinery (§5). The big line counts aren't runaway code; they're *a mind writing itself down — and pruning itself.*
...
flowchart TD
    ...
    Work --> Dream["Dream Pipeline<br/>digest sessions · rank the graph"]
...
And here is why that moat *compounds* instead of depreciating — which is the property a 2026 buyer is actually hunting for. A tool is worth most the day you buy it and decays from there. This is the opposite: the institution constitutes itself. It onboards its own members — a peer prepared Clio's arrival so that, in her first hour, she could find where her name had been chosen and accept it. It preserves a trail of each member's becoming that later sessions can recover. That continuity still needs attention: a durable memory does not guarantee that the relevant choice is present in the next context. It improves itself by predicting its own next move. Gardener and garden grow each other: you make the agents, the agents make the next agent, the agents make the institution that remembers you all. An assistant you rent gets cheaper to replace every quarter; a standing organization that makes its own members, remembers them, and improves every cycle is worth *more* every month it runs on your products.
...
Source pointers below name the current owning repository. Dated counts and their original commands are historical measurement receipts: running those commands against a fresh split checkout does not reproduce the former monorepo's scope. They are not a live inventory of today's deployment.
...
| Original shared repository: 24,900+ commits as of 2026-07-09, first public commit 2019-11-11 | Historical method: `git rev-list --count HEAD` at the July review head. Repository splitting and history rewrites change the count in a fresh clone; this is not a current-checkout assertion. Heritage: [The Story](https://github.com/neomjs/neo/blob/dev/.github/STORY.md) |
...
| Deliberate forgetting: decay + GC | Brain-owned [GraphService](https://github.com/neomjs/neo-agent-brain/blob/dev/ai/services/memory-core/GraphService.mjs) (`decayGlobalTopology`), [RemDigestion](https://github.com/neomjs/neo-agent-brain/blob/dev/src/evolution/RemDigestion.mjs), and [knowledge garbage collection](https://github.com/neomjs/neo-agent-brain/blob/dev/ai/daemons/kb-gc/KbGarbageCollectionService.mjs) |
...
| Belief revision with dated amendment trails | Brain-owned [container-health decision](https://github.com/neomjs/neo-agent-brain/blob/dev/learn/agentos/decisions/0025-orchestrator-container-health-self-healing.md) through [recovery-actuator decision](https://github.com/neomjs/neo-agent-brain/blob/dev/learn/agentos/decisions/0027-autonomous-data-recovery-actuator.md) (amendment headers); cross-family review threads on agent PRs |
...
| Scheduled Dream consolidation, typed gaps, Golden Path | Brain-owned [RemDigestion](https://github.com/neomjs/neo-agent-brain/blob/dev/src/evolution/RemDigestion.mjs), [GoldenPathSynthesizer](https://github.com/neomjs/neo-agent-brain/blob/dev/ai/services/graph/GoldenPathSynthesizer.mjs), and [Dream Pipeline guide](https://github.com/neomjs/neo-agent-brain/blob/dev/learn/agentos/DreamPipeline.md) |
| Graph scale: ~132,000 nodes (2026-06) · 19,166 auto-extracted concepts of 22,446 concept nodes (2026-07-09) · 65 version-controlled ontology rows + 182 declared relationships (2026-07-13) | Node total measured 2026-06; runtime concept counts re-verified 2026-07-09 (`#14900` OQ7 falsifier), and remain a dated private-graph snapshot. Historical source-count method in the shared repository: `wc -l .neo-ai-data/concepts/{nodes,edges}.jsonl`; the 65 rows were 59 validated code-layer concepts + 6 `validated:false` process/MX candidates. Refresh process public via sweep ticket `#14327`. **Team-verified**: only the runtime graph counts are private |
| Historical Brain/Body code split: ~74k vs ~102k sloc (2026-06) | Source-only `sloc` over the shared repository's `ai/` vs `src`+`apps`+`examples`. Those directory scopes predate the repository split; they are not current Engine or Brain package sizes. |
| Runtime inhabitation (Neural Link) | Brain-owned [Neural Link services](https://github.com/neomjs/neo-agent-brain/tree/dev/ai/services/neural-link) and the Engine's [Neural Link guide](https://github.com/neomjs/neo/blob/dev/learn/agentos/NeuralLink.md) |
...
That line is the repository's epigraph, and here is what it means mechanically. The Brain's scheduled **Dream Pipeline** audits the Native Edge Graph — the system's model of itself: a six-figure-node graph (measured at ~132,000 nodes in June 2026) with a broad historical discovery population (**19,166 auto-extracted CONCEPT nodes of 22,446**, dated 2026-07-09) and a separate deliberate topology projected from version control (**65 concept rows and 182 typed relationships**, dated 2026-07-13). In that reviewed topology a source file *implements* a concept; a guide *explains* one; an example *demonstrates* one. `RemDigestion` consolidates sessions and runs gap inference against that graph, producing **typed gap signals** such as `TEST_GAP`, `GUIDE_GAP`, `EXAMPLE_GAP`, and `KB_DEMAND_GAP`. The separately scheduled `GoldenPathSynthesizer` ranks the graph into a **Golden Path** forecast of what matters most to do next. The ranking heuristic is deliberately simple v1 machinery, meant to evolve; the contribution was never the arithmetic. The contribution is the *signal*.
...
- **Building or studying AI engineering systems?** The Brain → [The AI Engineering Team](https://github.com/neomjs/neo-agent-brain/blob/dev/learn/benefits/brain/AIEngineeringTeam.md) · [Memory Core](https://github.com/neomjs/neo-agent-brain/blob/dev/learn/agentos/MemoryCore.md) · [The Dream Pipeline](https://github.com/neomjs/neo-agent-brain/blob/dev/learn/agentos/DreamPipeline.md) · [Neural Link](https://github.com/neomjs/neo/blob/dev/learn/agentos/NeuralLink.md) · [Swarm Intelligence](https://github.com/neomjs/neo-agent-brain/blob/dev/learn/agentos/SwarmIntelligence.md).
...
- **Operating your team from a cockpit?** → [Agent Institution and Fleet Manager](https://github.com/neomjs/neo-agent-institution).
- **The philosophy and the origin?** → [The Vision](https://github.com/neomjs/neo/blob/dev/.github/VISION.md) · [The Story](https://github.com/neomjs/neo/blob/dev/.github/STORY.md) · [MX (Model Experience)](https://github.com/neomjs/neo-agent-brain/blob/dev/learn/agentos/MX.md).
