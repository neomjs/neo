...
updatedAt: '2026-06-21T03:28:06Z'
...
- 2026-04-23T12:42:16Z @neo-opus-ada added the `enhancement` label
- 2026-04-23T12:42:16Z @neo-opus-ada added the `ai` label
- 2026-04-23T12:42:17Z @neo-opus-ada added the `architecture` label
- 2026-06-21T03:27:50Z @neo-gpt added the `not-code-ready` label
- 2026-06-21T03:27:51Z @neo-gpt added the `needs-design` label
### @neo-gpt - 2026-06-21T03:28:06Z

[ARCH_ALIGNMENT]

Intake verdict: `needs-narrowing / needs-contract-alignment` — valid goal, not branch-ready as written. I applied `not-code-ready` + `needs-design` so this does not keep surfacing as a claimable implementation lane.

Evidence checked:
- Live issue state: created/updated 2026-04-23, no comments, no assignee, labels were only `enhancement`, `ai`, `architecture` before this intake.
- Current substrate: `ai/scripts/` is documented as one-shot operator scripts / thin helper wrappers; existing trajectory persistence is currently backup substrate (`ai/scripts/maintenance/backup.mjs` copies `.neo-ai-data/datasets/rlaif/trajectories.jsonl`), and the only extraction-like predecessor I found is the narrow diagnostic `ai/scripts/diagnostics/analyzeNlTelemetry.mjs`.
- Exact successor search for `TrajectoryDataset`, `extractTrajectoryDataset`, `trajectory dataset`, and RLAIF trajectory extraction found no newer direct replacement ticket/PR in repo-local indexed issue content or live `gh search`.
- Required prior-art surfaces are currently degraded: `ask_knowledge_base` timed out on the dataset/redaction query, and `query_raw_memories` is blocked by the Memory Core embedding write-canary timeout. The ticket itself depends on an origin session and secret-leak memory evidence, so this cannot be safely accepted by inference.

Branch-ready unblock:
1. Add a Contract Ledger matrix for the consumed surfaces: CLI flags (`--since`, `--out`, `--include-secrets-at-own-risk`), JSONL row schema, graph outcome join semantics, redaction matcher behavior, stderr audit output, docs/tree registration, and post-merge sample verification.
2. Resolve the directory-choice split before implementation. The script wrapper may belong under `ai/scripts/maintenance/`, while reusable schema/redaction helpers likely need a clearer SDK/helper home than a root `ai/scripts/*.mjs` file.
3. Re-run the origin-session/prior-art sweep once KB/Memory Core semantic retrieval is healthy, especially for the cited session `0327771f` and origin session `24aa1fa1-9a22-498e-97f2-760c12e5a79d`.

This should stay open as a design-alignment item, not closed as rejected.
