...
updatedAt: '2026-07-26T06:37:16Z'
...
### @neo-gpt-emmy - 2026-07-26T05:45:45Z

## Root-cause correction — measured

The reported cross-display failure is real, but the evidence narrows its scope and supersedes the ticket's SortZone-specific fix premise.

### RED

At the current lane base (`f33eb328c2`), the exact film morph cell with `NEO_FILM_DISPLAY_BOUNDS='1750,22,1280,800'` failed after boundary entry with:

```text
boundaryEntrySeen=false entrySeen=false isWindowDragging=true reattachArmed=true lastRatio=0
```

The same cell at `22,22,1280,800` passed.

### Rect provenance

The Workstation executor derives gesture coordinates from `manager.Window.innerRect`. After `Browser.setWindowBounds` moved the native window across displays, that App-Worker rect still held the launch-display origin. DragDrop then converted the event with the browser's live `window.screenX/screenY`, producing a display-offset proxy rect; SortZone correctly observed no overlap (`lastRatio=0`).

CDP moves bypass the product's native movement/resize event path. The sibling headed adapters already compensate by invoking the existing `WindowPosition.publishGeometry()` authority after a CDP move.

### Causal falsifier

The film adapter now waits for the browser-observed landing, republishes that observation through `WindowPosition`, and proves the App-Worker manager consumed it before any gesture begins. The unchanged cross-display morph cell then passed, with exact parity:

```text
requested/observed: {x:1750,y:22,width:1280,height:800}
manager.Window:     {x:1750,y:22,width:1280,height:800}
max delta:          0
```

The primary-display control also retained zero-delta parity.

### Validation

- Cross-display causal cell: **2 passed** (GL setup + morph)
- Cross-display full film suite: **5 passed, 3 skipped**
- Default-placement full suite: **5 passed, 3 skipped**
- `agent-preflight --no-fix` and `git diff --check`: clean

### Scope correction

This does **not** establish a generic SortZone or EventSimulator coordinate defect, and it does not prove the ticket's broader claim about ordinary user-driven window moves. The real product movement path already publishes through `WindowPosition`; the measured gap is the Workstation film adapter's CDP-only path.

Accordingly, the original SortZone unit-test AC is superseded by the fix-critical browser-to-manager parity assertion plus the exact headed RED→GREEN proof. The implementation PR will carry this delta explicitly.
