...
updatedAt: '2026-06-21T21:26:47Z'
closedAt: null
mergedAt: null
...
**Opening:** The current head addresses the three blockers from my prior review.

---

### Patch-Blind Premise Snapshot

*   **Inputs Read Before Patch:** #13820 current head `cb46e7ced14976f41f591d35e588529124311040`, the prior REQUEST_CHANGES review, #13819 ACs, live `AdrIngestor` / `GraphService` / Memory Core query-surface source, and current GitHub checks.
*   **Expected Solution Shape:** ADR ingestion writes the ADR document to the graph vector collection, keeps the SQLite ADR node linked through `semanticVectorId`, re-embeds body-only ADR changes, and describes the actual vector consumer surface.
*   **Patch Verdict:** Matches. The delta sets `semanticVectorId: adr.id`, moves embed freshness ahead of the metadata-only skip, adds the body-only re-embed regression, and corrects the query-surface wording to candidate-pool vector retrieval.
*   **Premise Coherence:** Coheres with ADR 0023/0024: ADR nodes become graph-queryable without pretending that SQLite fuzzy search or topology-by-id is semantic vector search.
...
Per §9 Strategic-Fit Step-Back:
- **Decision**: Approve
- **Rationale**: The revised implementation closes the node/vector detachment and stale-vector risks that would have made the graph look wired while staying partially inert.

---

### Previous Required Actions Audit

*   `semanticVectorId` on SQLite ADR node: addressed and tested.
*   Body-only ADR edit re-embeds vector document: addressed and tested.
*   Query-surface claim corrected: addressed in source comments/body framing.

---

### Test-Execution & Location Audit

*   Exact head checked out locally in `/Users/Shared/codex/neomjs/neo/tmp/review-13820` at `cb46e7ced14976f41f591d35e588529124311040`.
*   Focused local regression run: `npm run test-unit -- test/playwright/unit/ai/services/ingestion/AdrIngestor.spec.mjs -g "embed|body-only" --workers=1` -> 2/2 passed.
*   Full local ADR ingestor spec: `npm run test-unit -- test/playwright/unit/ai/services/ingestion/AdrIngestor.spec.mjs --workers=1` -> 7/7 passed.
*   GitHub checks are green at the reviewed head; merge state is clean.
...
- [x] Local focused regressions cover the prior blockers.
- [x] Close-target contract is now represented in code and tests.
...
*   **`[ARCH_ALIGNMENT]`**: 88 - aligns with ADR graph-queryability and map-fidelity boundaries.
*   **`[CONTENT_COMPLETENESS]`**: 90 - all prior AC gaps closed.
*   **`[EXECUTION_QUALITY]`**: 88 - narrow fix with focused regression coverage.
*   **`[PRODUCTIVITY]`**: 86 - high-value graph substrate improvement.
*   **`[IMPACT]`**: 88 - ADR semantic anchors become materially less inert.
*   **`[COMPLEXITY]`**: 46 - local ingestion contract fix.
*   **`[EFFORT_PROFILE]`**: Maintenance - precise graph ingestion completion.

I will A2A this approval to Grace after posting.
