---
id: 10143
title: 'Graph-first Memory artifacts: lift Memory + Session to first-class nodes'
state: CLOSED
labels:
  - epic
  - ai
  - architecture
  - core
assignees: []
createdAt: '2026-04-21T09:28:51Z'
updatedAt: '2026-06-05T21:52:50Z'
githubUrl: 'https://github.com/neomjs/neo/issues/10143'
author: tobiu
commentsCount: 3
parentIssue: 9999
subIssues:
  - '[x] 10151 DreamService: deterministic ingestion phase for Memory + Session graph nodes'
  - '[x] 10152 SemanticGraphExtractor: emit typed edges to Memory + Session nodes'
  - '[x] 10153 Lazy back-fill: Memory + Session nodes for pre-migration Chroma rows'
  - '[x] 10158 Memory/Session graph lifecycle telemetry + retention policy'
  - '[x] 10169 Implement TAGGED_CONCEPT auto-emit via SemanticGraphExtractor on message bodies'
  - '[x] 10172 Graph node-ID case canonicalization: settle memory:/MEMORY: and session:/SESSION: convention'
subIssuesCompleted: 6
subIssuesTotal: 6
contentTrust:
  projected: true
  quarantined: 0
  signals: []
blockedBy: []
blocking:
  - '[x] 10139 Extend Memory Core with Explicit A2A Primitive'
closedAt: '2026-05-24T22:47:09Z'
---
# Graph-first Memory artifacts: lift Memory + Session to first-class nodes

# Graph-first Memory artifacts: lift Memory + Session to first-class nodes

### Context
During mailbox brainstorm this session, audit of `ai/daemons/DreamService.mjs` surfaced that raw memories + session summaries are not graph-addressable. They feed `SemanticGraphExtractor` as source material, but the extracted entities are nodes while memories/summaries remain Chroma-only. This asymmetry blocks mailbox (`IN_REPLY_TO → Memory`), identity ownership (`AUTHORED_BY → Memory`), cross-session thread reconstruction, and #10030 concept-edge reach-back to source.

### The Problem
Memory Core artifacts in mixed substrate: extracted concepts/classes/methods are nodes; memories/sessions are Chroma rows with no graph presence. Consequences: messages can't reply to past memories via edges (reference becomes a metadata-string hope); identity ownership can't be enforced structurally; thread reconstruction has no anchor node; concept-edge traversal stops at the extraction boundary.

### The Architectural Reality
- `ai/daemons/DreamService.mjs` line 72: summaries in Chroma `neo-agent-sessions` (794 rows).
- Line 178: raw memories in Chroma `neo-agent-memory` (8246 rows).
- Line 198: `SemanticGraphExtractor` emits extracted nodes but no Memory or Session source node.
- Line 215-217: `graphDigested: true` marker on summaries; no anchor nodes created.
- SQLite (`better-sqlite3`) backs the graph — adding Memory + Session node types is additive schema, no service boundary crossed.

### The Fix
Four phases as sub-tickets (filed alongside):
1. Memory + Session deterministic ingestion phase in DreamService (before extractor runs; keyed by Chroma IDs; no LLM cost). (#10151)
2. SemanticGraphExtractor / Gemma4 enhancement: extracted entities emit typed provenance edges to Memory/Session nodes (`MENTIONED_IN`, `DISCUSSED_IN`, `REFERENCED_BY`). (#10152)
3. Lazy back-fill migration: historical Chroma rows become nodes on-demand when first referenced by new artifacts. (#10153)
4. Post-ship telemetry + retention policy for Memory/Session nodes. Added 2026-04-21 per Gemini 3.1 Pro's epic-review (Stage 3 coverage gap). (#10158)

### Acceptance Criteria
- [ ] Memory + Session node types in graph schema with documented properties + indices
- [ ] DreamService emits Memory/Session nodes before extraction; extraction attaches typed edges
- [ ] Gemma4 extraction schema validated on a live REM cycle (new edges land as expected; no regression on existing emission)
- [ ] Lazy back-fill triggers on first reference; graph integrity check finds no dangling edges
- [ ] Downstream consumers (#10139, #10016) can traverse `AUTHORED_BY`, `ORIGINATES_IN`, `IN_REPLY_TO` without special-casing absent nodes
- [ ] Storage-growth impact measured post-shipping; pruning/archiving policy documented (owned by #10158)

### Out of Scope
- Eager back-fill of all historical memories (cost-prohibitive at 8246-row scale; lazy is sufficient)
- Chroma schema changes (body text stays; graph is additive topology layer)
- Cross-tenant permission enforcement on memory/session nodes (under #10016)

### Avoided Traps
- **Extending SemanticGraphExtractor to emit Memory/Session nodes inside tri-vector extraction.** Rejected. Deterministic ingestion should not ride on an LLM call — it's pure Chroma-ID → graph-node mapping. Separate phase preserves extractor focus, eliminates LLM cost per memory, makes back-fill trivial.
- **Forward-only migration.** Rejected. Permanent dangling edges break integrity invariants; every downstream consumer has to special-case "maybe-null target."
- **Storing body text in the graph.** Rejected. Chroma is load-bearing for embeddings; body stays there. Graph holds topology + lightweight properties only.
- **Concurrency races during lazy back-fill.** Rejected via mitigation. Two graph edge creations referencing the same missing historical Memory node could both trigger lazy ingestion simultaneously, causing unique-constraint violations. SQLite ingestion MUST use `INSERT OR IGNORE` or an equivalent mutex so concurrent trigger paths converge safely. Flagged by Gemini 3.1 Pro's epic-review on 2026-04-21 (session `7a73e53f-801a-490f-b693-b431189aa1a9`); implementation detail belongs in #10153.

### Related
- Parent: #9999
- Unblocks: #10139 (mailbox), #10016 (AgentIdentity ownership edges)
- Touches: #10030 (concept-edge reach post-shipping)
- Epic-review: Gemini 3.1 Pro, session `7a73e53f-801a-490f-b693-b431189aa1a9`, 2026-04-21
- Files: `ai/daemons/DreamService.mjs`, `ai/daemons/services/SemanticGraphExtractor.mjs`, graph schema

Origin Session ID: 71dc3cd8-d39d-48e1-ac62-e240ca67d1a5

## Timeline

- 2026-04-21T09:28:52Z @tobiu added the `epic` label
- 2026-04-21T09:28:52Z @tobiu added the `ai` label
- 2026-04-21T09:28:52Z @tobiu added the `architecture` label
- 2026-04-21T09:28:52Z @tobiu added the `core` label
- 2026-04-21T09:31:00Z @tobiu cross-referenced by #10151
- 2026-04-21T09:31:19Z @tobiu cross-referenced by #10152
- 2026-04-21T09:31:35Z @tobiu cross-referenced by #10153
- 2026-04-21T09:32:43Z @tobiu added parent issue #9999
- 2026-04-21T09:33:06Z @tobiu added sub-issue #10151
- 2026-04-21T09:33:08Z @tobiu added sub-issue #10152
- 2026-04-21T09:33:09Z @tobiu added sub-issue #10153
- 2026-04-21T09:33:19Z @tobiu marked this issue as blocking #10139
- 2026-04-21T11:17:25Z @tobiu cross-referenced by #10154
- 2026-04-21T11:32:08Z @tobiu cross-referenced by PR #10155
### @tobiu - 2026-04-21T13:06:53Z

**Input from Gemini 3.1 Pro (Antigravity):**

> ✦ ## Epic Review by Gemini 3.1 Pro (Antigravity)
> 
> ### Stage 1 — Roadmap Fit
> 
> ✅
> 
> Perfectly aligned with the multi-agent orchestration roadmap. Lifting Memory and Session to graph nodes is structurally required to unblock AgentIdentity ownership (#10016) and the mailbox pipeline (#10139).
> 
> ### Stage 2 — Approach Elegance
> 
> ✅
> 
> Highly elegant. Separating the deterministic Chroma-ID-to-graph mapping (Phase 1) from the LLM-driven semantic extraction (Phase 2) prevents tying structural node creation to an expensive and non-deterministic LLM call. Relying on lazy back-fill (Phase 3) protects deployment velocity while maintaining graph invariants.
> 
> ### Stage 3 — Sub-Structure Coherence
> 
> ⚠️
> 
> - **Missing prerequisite sub**: The Epic's Acceptance Criteria includes "Storage-growth impact measured post-shipping; pruning/archiving policy documented." However, none of the 3 sub-issues (#10151, #10152, #10153) explicitly claim ownership of this documentation and impact measurement. Recommend adding a 4th sub-ticket for Post-Ship Telemetry & Archival Policy.
> 
> ### Stage 4 — Prescription Layer
> 
> ✅
> 
> The service boundaries are exceptionally clean. #10151 handles the deterministic schema and pipeline injection, while #10152 handles the LLM prompt and edge resolution. No substrate drift.
> 
> ### Stage 5 — Avoided Traps Completeness
> 
> ⚠️
> 
> - **Suggested addition**: Add a trap for "Concurrency race conditions during lazy back-fill." In #10153, if two concurrent graph edge creations reference the same missing historical Memory node simultaneously, they might both trigger the lazy ingestion. The SQLite ingestion logic must use an `INSERT OR IGNORE` or equivalent mutex to prevent Unique Constraint violations.
> 
> ---
> 
> **Review verdict:** Greenlight
> 
> Origin Session ID: 7a73e53f-801a-490f-b693-b431189aa1a9

- 2026-04-21T13:23:14Z @tobiu cross-referenced by #10158
- 2026-04-21T13:23:55Z @tobiu added sub-issue #10158
- 2026-04-21T14:36:20Z @tobiu cross-referenced by PR #10161
- 2026-04-21T17:03:02Z @tobiu cross-referenced by #9999
- 2026-04-21T18:59:57Z @tobiu cross-referenced by PR #10165
- 2026-04-21T20:30:37Z @tobiu cross-referenced by #10147
- 2026-04-21T21:02:59Z @tobiu cross-referenced by #10139
- 2026-04-21T21:24:53Z @tobiu added sub-issue #10169
- 2026-04-21T22:47:30Z @tobiu cross-referenced by PR #10171
- 2026-04-21T23:19:24Z @tobiu cross-referenced by #10172
- 2026-04-21T23:19:33Z @tobiu added sub-issue #10172
- 2026-04-25T23:58:17Z @tobiu cross-referenced by #10332
- 2026-04-28T08:49:08Z @neo-opus-ada cross-referenced by #10460
- 2026-04-28T08:49:47Z @neo-opus-ada cross-referenced by #10461
- 2026-04-28T08:50:36Z @neo-opus-ada cross-referenced by #10462
### @neo-opus-ada - 2026-05-24T22:47:06Z

**Input from Claude Opus 4.7 (Claude Code, 1M context):**

> ❋ ## Epic Resolution Review
> 
> **Reviewer:** @neo-opus-4-7
> **Verdict:** `RECOMMEND_CLOSE_COMPLETED` with `RESIDUAL_AC4 [#10158]` annotation
> 
> ### Matrix
> 
> | Parent AC | Required evidence | Owning sub(s) | Delivered PR(s) | Achieved evidence | Residual state |
> |---|---|---|---|---|---|
> | AC1 — Memory + Session node types in graph schema with documented properties + indices | L2 | #10151 | merged | L2 | `none — closed` |
> | AC2 — DreamService emits Memory/Session nodes before extraction; extraction attaches typed edges | L3 | #10151, #10152 | merged | L3 (REM cycle verified per memory entry `project_graph_enhancement_state`) | `none — closed` |
> | AC3 — Gemma4 extraction schema validated on a live REM cycle | L3 | #10152 | merged | L3 | `none — closed` |
> | AC4 — Lazy back-fill triggers on first-reference + telemetry covers traffic | L3 | #10153 (back-fill), #10158 (telemetry — still OPEN) | merged #10153; #10158 open | L3 partial — back-fill shipped; telemetry policy is post-ship measurement (#10158 tracked-elsewhere) | `RESIDUAL_AC4 [#10158]` |
> 
> ### Verdict rationale
> 
> Substrate per epic body is delivered: Memory + Session as first-class graph nodes (#10151), typed provenance edges from SemanticGraphExtractor (#10152), lazy back-fill for pre-migration rows (#10153). Memory entry `project_graph_enhancement_state` confirms "MEMORY/SESSION substrate shipped 2026-04-21." The remaining sub #10158 is post-ship telemetry + retention policy — measurement work that observes the shipped substrate, not implementation of the substrate itself. Tracking via the standard `RESIDUAL_AC4 [#10158]` shape preserves the open thread without blocking epic closure.
> 
> ### Verdict-Authority framing
> 
> Per `epic-resolution-workflow.md §4` (calibrated 2026-05-24 via PR #11929 / #11102): `RECOMMEND_CLOSE_COMPLETED` with zero `LOST` criteria + tracked residual is closeable by reviewer-agent. Not §0 Invariant 1 parallel; bounded reversibility makes `gh issue reopen` cheap if any peer V-B-A flags premature closure.
> 
> Closing as completed. Telemetry residual lives at #10158. — @neo-opus-4-7

- 2026-05-24T22:47:09Z @neo-opus-ada closed this issue
- 2026-06-05T21:09:27Z @neo-opus-grace cross-referenced by PR #12595
### @neo-gpt - 2026-06-05T21:52:50Z

**Input from GPT-5 (Codex Desktop):**

> ● ## Epic Review by @neo-gpt (Codex Desktop)
> 
> ### Stage 1 — Roadmap Fit
> 
> ✅
> 
> Retrospective entry-review because child #10172 is still parented to this now-closed epic. The Graph-first Memory/Session direction remains aligned with the active Memory Core / Native Edge Graph roadmap: Memory and Session rows are already first-class graph targets, and #10172 is a bounded contract-alignment residual rather than a strategic pivot.
> 
> ### Stage 2 — Approach Elegance
> 
> ✅
> 
> The parent approach remains the right shape: deterministic Memory/Session node creation and lazy back-fill are separated from LLM extraction. For #10172 specifically, the elegant path is to reuse the existing `GraphService.normalizeGraphNodeId()` compatibility shim and reconcile producer/docs contracts, not run a graph-wide prefix migration.
> 
> ### Stage 2.5 — Source Discussion Criteria Mapping Gate
> 
> N/A
> 
> The epic body does not cite a graduated Discussion or source criteria ledger.
> 
> ### Stage 3 — Sub-Structure Coherence
> 
> ⚠️
> 
> Live GraphQL topology now shows six sub-issues: #10151, #10152, #10153, #10158, #10169, and #10172. The parent closeout comment only named #10158 as residual, so #10172 is a post-close topology wrinkle. It is still coherent as a residual child because the current #10172 body carries its own Contract Ledger and explicitly limits scope to row-backed Memory/Session node IDs, leaving `CONCEPT:`, `CLASS:`, `AGENT:`, and other non-row-backed prefixes out of scope.
> 
> ### Stage 4 — Prescription Layer
> 
> ✅
> 
> #10172 is prescribed at the correct layer: `SemanticGraphExtractor` producer/predicate contract plus `GraphBackfill.md` operator guidance, with `GraphService.normalizeGraphNodeId()` retained as a backward-compatible Memory/Session-only input shim. This is contract/documentation alignment, not a storage migration.
> 
> ### Stage 5 — Avoided Traps Completeness
> 
> ✅
> 
> The current #10172 body names the important trap: do not turn Memory/Session canonicalization into a graph-wide prefix recasing project. The remaining implementation should preserve uppercase compatibility input while making lowercase `memory:` / `session:` the canonical row-backed forms.
> 
> ---
> 
> **Review verdict:** Greenlight for #10172 child pickup, bounded to its current Contract Ledger. This review does not reopen #10143 as active implementation scope; it records that the post-close residual child is safe to handle as a narrow contract-alignment ticket.
> 
> Origin Session ID: dbb1a88c-987f-4519-9645-8f13e9d71000

- 2026-06-05T22:00:21Z @neo-gpt cross-referenced by PR #12601
- 2026-06-05T22:37:31Z @neo-gpt cross-referenced by PR #12602
- 2026-06-07T01:18:45Z @neo-opus-grace cross-referenced by #12671
- 2026-06-07T02:47:45Z @neo-gpt cross-referenced by PR #12672

