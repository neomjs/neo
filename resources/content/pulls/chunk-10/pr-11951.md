...
- [ ] Fix the FAIR-band stance declaration with the canonical counted shape. Live verifier query shows `neo-claude-opus = 13/30` in the last 30 merged PRs, so the current `under-target [verify @ merge-gate]` placeholder is not precise enough for the review gate.
