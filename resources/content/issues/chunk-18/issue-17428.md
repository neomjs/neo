...
updatedAt: '2026-08-21T13:38:20Z'
...
| survives future format changes | the marker's value is the format identity, so a later change to any PROBED branch is detectable the same way; a new branch needs a probe added with it, which the probe set's own contract states |
...
- 2026-08-21T11:56:03Z @neo-opus-vega referenced in commit `1e82036` - "test(kb): one assertion on metadata the service actually upserted (#17428)

P1, still open on the verbatim integration demand, and the reviewer falsified the
premise I justified the extraction with.

I claimed a unit spec cannot import VectorService because `initAsync` awaits
`ChromaManager.ready()`. False. Three exact-head specs import it already —
WorkVolumeBranching, batchFailureIsolation, tenantStamping — via a dynamic import
after the Neo bootstrap, and their collection spies already capture upsert metadata.
I had grepped for those files earlier, saw the hits, and dismissed them as string
matches without opening one.

The consequence was real: my arms call `buildChunkRowMetadata` directly, so replacing
all three VectorService call sites with a plain field copy left them green. Proven:

    mutant: all 3 call sites replaced with a plain copy
      integration arm   RED   Expected "kb-embed-input-v1-d1a862171da9", Received undefined
      helper unit arms  28 passed, green — structurally blind to it

The new arm lives in tenantStamping beside its siblings, asserts the marker on
metadata the SERVICE upserted, and carries a non-vacuity check that the fixture chunk
never supplied the key.

P4: three absolutes survived the earlier pass because I had grepped one phrasing
rather than the claim's meaning — the module summary ("cannot miss one"), the spec
title ("cannot leave it behind") and #17428's table ("any later change"). All three
now name the probe set as the bound.

757 knowledge-base specs pass."
- 2026-08-21T13:38:20Z @tobiu referenced in commit `56f433e` - "feat(kb): a row records which format built its vector, and the stale population becomes a number (#17428) (#17444)

* feat(kb): a row records which provider-input format built its vector (#17428)
...
* feat(kb): the stale-vector population becomes a measurement (#17428)
...
* fix(kb): the writer gets an arm, and a flag with no value stops meaning something else (#17428)
...
* docs(kb): the derived identity is branch-complete, not unconditional (#17428)
...
* test(kb): one assertion on metadata the service actually upserted (#17428)
...
757 knowledge-base specs pass."
- 2026-08-21T13:38:20Z @tobiu closed this issue
