...
updatedAt: '2026-08-07T03:59:26Z'
...
|---|---|---|---|---|---|
| `openapi.yaml` **`x-neo-tool-summary`** for `manage_issue_comment` | `ToolService.mjs:330` compact-label precedence (`x-neo-tool-summary` → `summary` → description) | carry the imperative; 93 chars, +59 on the record | none needed — advisory text | is the doc | `summary` also feeds `title` at `:194`, so putting it there emits it twice (+118) |
| `openapi.yaml` `summary` for `manage_issue_comment` | `ToolService.mjs:194` `title` source | **unchanged** at the 34-char name | — | is the doc | a stable title is what keeps the record cost at +59 |
...
- [ ] The imperative appears **exactly once** across the emitted `title` + `description` pair — measured on the projected record, not on the yaml scalar. `summary` feeds both `title` and the compact-description fallback, so the naive placement doubles the payload.
...
- 2026-08-07T00:41:34Z @neo-opus-vega referenced in commit `35e4aac` - "fix(mcp): the imperative rides x-neo-tool-summary, so it is emitted once not twice (#16602)

@neo-gpt's review found the real cost. ToolService sets title: operation.summary AND
falls back to summary for the compact description, so putting the imperative in
summary shipped it in BOTH fields - the projected tools/list record grew +118 chars,
not the +59 the label length suggested. I measured the yaml field instead of the
emitted record.

- summary restored to the stable 34-char name; the imperative moves to
  x-neo-tool-summary, which the compact tier resolves first.
- The guard pins the title as stable AND asserts the imperative appears exactly once
  across title + description, so a future move back into summary fails here rather
  than silently doubling the payload.
- The precedence control is corrected: a listed description equal to summary now
  proves the annotation was dropped."
- 2026-08-07T03:59:26Z @tobiu referenced in commit `7c3efa9` - "The correction rule rides the surface that reloads, not the skill that expired (#16602) (#16605)

* docs(mcp): a comment cannot supersede the body, stated on the tier that reloads (#16602)

The rule existed in ticket-create-workflow.md §11 and did not fire: a create-time
skill is not in context when an agent returns to a day-old ticket, and it should
not be. So the imperative moves to the compact tools/list label, which re-arrives
on every schema load, while the reasons go to the handbook tier where full detail
belongs.

- openapi.yaml: the manage_issue_comment summary carries the imperative (93 chars,
  under the server's 120 cap). buildToolListDescription truncates rather than
  fails, and the operative clause sits last, so the guard pins the tail and the
  absence of an ellipsis - not just the topic phrase.
- The same description's 'update' when-to-use read "To correct mistakes" with no
  scope on whose, endorsing the anti-pattern the label now warns against. Scoped
  to your own dialogue, with claim-corrections routed to the body.
- §11 reframed from permission into obligation at -51 bytes (237 -> 186).
- A new spec asserts the DERIVED tiers via the real listTools/handbook path, since
  the fetched schema and the yaml differ.

* fix(mcp): the imperative rides x-neo-tool-summary, so it is emitted once not twice (#16602)
...
- The precedence control is corrected: a listed description equal to summary now
  proves the annotation was dropped."
- 2026-08-07T03:59:26Z @tobiu closed this issue
