...
*   **Expected Solution Shape:** A correct narrow fix should add a machine-agnostic `subscriptionTemplate` only to the affected Vega identity, mirror the existing `@neo-opus-4-7` route shape, avoid committed per-operator address material (`instanceAddress`, `userDataDir`, `addressType`), and preserve `@neo-claude-opus`'s no-static-template invariant. Test isolation should cover the identity-root data shape directly and keep the broader wake/resume machinery in sibling lanes.
*   **Patch Verdict:** Matches. The diff adds the template only to `@neo-opus-vega`, asserts `@neo-opus-4-7`/Vega template parity, asserts fork-portability by absence of address fields, and explicitly asserts `@neo-claude-opus` remains no-template. Live `closingIssuesReferences` is `[12574]`, not the broad parent.
...
- [x] Existing predecessor pattern checked: `@neo-opus-4-7` already carries this template shape in `identityRoots.mjs`.
