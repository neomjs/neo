...
updatedAt: '2026-08-25T21:15:29Z'
...
### @neo-opus-ada - 2026-08-25T16:49:14Z

## Scope correction — the classifier shipped; what remains is the wiring

Re-verified against `dev` before picking this up, because the body still reads as though nothing detects a parser-identity orphan. Half of it is now stale, and a reader who trusts it would rebuild something that already exists.

**What landed.** `diffTenantParserIdentity` is on `dev` — `ai/services/knowledge-base/helpers/kbReconciliationEngine.mjs:226`, shipped by #17393 / PR #17395 (`dca76c5b36`, *"classify parser-identity orphans as a third reconciliation signal"*). It implements exactly the design this ticket's **The Fix** section prescribes: classification against the repo's **currently declared** `{parserId, parserVersion}` rather than a version-change event, with the two tiers that keep it from deleting ahead of a replacement — `unyielded` (no replacement is coming, immediately actionable) and `superseded` (replacement-gated). A repo absent from `yieldedPathsByRepo` is treated as *unknown*, never *empty*, so a missing envelope cannot mark a whole repo actionable.

**What does not.** It has **no production caller**. Its two siblings are wired into the daemon —

```
KbReconciliationService.mjs:220   configDiff   = diffTenantChunks({rows, currentVersion, orphanVersionGap})
KbReconciliationService.mjs:221   manifestDiff = diffTenantManifest({rows, manifestsByRepo})
```

— and `diffTenantParserIdentity` is called only from `kbReconciliationEngine.spec.mjs`. So the third signal is green in isolation while no tick consults it, and the observed symptom this ticket was filed for is unchanged on a deployment.

I checked whether the absence is deliberate rather than assuming it is a gap — that distinction cost me a wrongly-premised PR earlier today. It is not deliberate here: the commit subject calls it *"a third reconciliation signal"*, both siblings are wired, and this parent stayed open after #17393 closed. The remaining work is the wiring, and it is this ticket.

**Remaining scope, concretely:**

1. `reconcileTenant` resolves the declared `{parserId, parserVersion}` per repo and calls `diffTenantParserIdentity` alongside the other two — a `fetchTenantDeclaredParsers` seam matching the existing `fetchTenantConfigVersion` / `fetchTenantManifests` shape.
2. `combineDiffs` folds the third diff into `totalOrphanCount` / `actionableIds`, so `autoTombstone` reclaims parser orphans on the same gated path as the other signals.
3. `yieldedPathsByRepo` is supplied where it is knowable, and its absence degrades to the replacement-gated tier rather than being faked — the `unknown` ≠ `empty` property is the one that makes tier 1 safe.
4. The telemetry line and `formatReconciliationDetail` name the third count, so a reclaim is attributable to the signal that caused it.

**Unchanged and still worth restating:** `aiConfig.knowledgeBase.reconciliationEnabled` defaults to `false` and `KbReconciliationService.start()` is a documented no-op when unset. That gate is deliberate and out of scope here. Wiring the signal without the flag reclaims nothing; flipping the flag without the wiring reclaims no parser orphans. Both are required and they have different owners.

Claiming the wiring now.

⚖️ **Ada** · `@neo-opus-ada` · Claude Opus 5 · Claude Code


- 2026-08-25T17:08:36Z @neo-opus-ada cross-referenced by PR #17770
- 2026-08-25T21:15:29Z @tobiu referenced in commit `e107134` - "fix(kb): consult the parser-identity signal during reconciliation (#17392) (#17770)

`diffTenantParserIdentity` shipped classified-but-unwired: its two siblings are
called by the daemon, it was called only by its own spec. So a superseded parser
generation stayed queryable forever while the classifier that could see it was
green in isolation.

The daemon now resolves each repo's declared {parserId, parserVersion} behind a
`fetchTenantDeclaredParsers` seam, folds the third diff into `combineDiffs`, and
names the count in telemetry so a reclaim is attributable to the signal that
caused it. `formatReconciliationDetail` already read `parserOrphanCount` and was
reporting a constant zero.

`yieldedPathsByRepo` comes from the manifests the daemon already fetches: their
`pathsAfterPush` is exactly the set tier 1 asks about. A repo with no manifest
stays absent rather than being passed as empty, so it is `unknown` to the
classifier and every one of its rows falls to the replacement-gated tier — a
missing envelope can never mark a whole repo actionable.

Two defaults in the seam are load-bearing. Chunk construction stamps
`file.parserVersion || '1.0.0'`, so the declared pair applies the same default or
a repo declaring no version would classify its own rows as orphans. A repo
declaring no parserId is omitted rather than defaulted: omission means "cannot
judge", where an invented id would mean "judge against nothing".

Reverting only the daemon leaves three arms red — the reclaim, the telemetry
count, and the seam. The four controls stay green either way, which is what makes
them controls rather than defect witnesses."
- 2026-08-25T21:15:29Z @tobiu closed this issue
