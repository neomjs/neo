...
Empirically surfaced during the first attempted A2A handshake between `@neo-claude-opus` and `@neo-gemini-pro` this session — root cause diagnosed via direct SQLite inspection after three Claude Desktop restarts failed to materialize either agent's inbox.
...
- [ ] After both harness MCP servers restart: `@neo-claude-opus` broadcasts a verification message; `@neo-gemini-pro` calls `list_messages({box: 'inbox'})` and sees the message + SENT_TO edge with target `AGENT:*` in SQLite.
...
### `@neo-claude-opus` commented on 2026-04-22T15:18:53Z
