...
- `pr-review`: Evaluation matrix templates spanning `[ARCH_ALIGNMENT]` to `[EFFORT_PROFILE]` + graph ingestion tags for the Dream Pipeline + deterministic budgeted closure (two ordinary RCs, then `COMMENTED` closure / approval / polish / guarded A+FU / one validator-complete terminal D+S) + a prior-art Memory Core sweep gate before scoring (CI-green ≠ AC-met) + a conditionally-loaded reviewer-instrument audit for capability gates that check existence rather than execution, and for absence claims made without a positive control (enforces mandatory ROI template usage).
