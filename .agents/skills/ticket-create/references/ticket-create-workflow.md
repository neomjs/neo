...
3. **Substrate** — where does this work belong? Service layer? Build script? CI workflow? Framework core? Documentation? Match the fix to the substrate that owns the concern.
   ...
   **Code-vs-Data-Migration Sub-Split (for epic-shape tickets):** if the prescription describes BOTH code changes (`.mjs` / build-script logic) AND data migration (file renames / mass content relocation), the epic MUST split these into separate sub-tickets — code sub lands first, data sub consumes the new shape. Mixed-scope PRs create review-friction (signal-to-noise asymmetry; squash-merge git-history pollution). Catching this at ticket-create time prevents the friction surface from ever reaching epic-review (which would flag as Stage 3.5 Required Action; see `.agents/skills/epic-review/references/epic-review-workflow.md`). Empirical anchor: Epic #11113 / PR #11114 — 15 LOC of code logic bundled with ~4190 file renames in the same PR.
