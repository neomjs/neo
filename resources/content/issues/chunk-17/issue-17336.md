...
updatedAt: '2026-08-20T13:50:20Z'
...
Durable comments carried process history -- which design came first, what the
fixture measured, what a comment "exists to stop". A reader pays for that on
every visit and gets no behaviour from it.

Comment share of this branch's delta drops 47% -> 31%; 709 knowledge-base unit
tests still pass."
...
suspects and seq but not deaths, so a strike earned under one geometry could fence a chunk
under repaired coordinates. Both provider-success carry arms cleared strikes and suspects
while their own comments asserted 'the same provider-outcome rule as the ordinary success
path' — the ordinary arm did clear deaths, so the rule was stated and two thirds applied.

The transient-state doc described the superseded automaton: pending-until-a-later-success,
which is only the refused branch. The accepted-then-died branch earns its strike
immediately, because a reset or EPIPE carries its own liveness proof and a suspect that is
the only chunk left never sees a later success. Both branches are now documented, with the
entry shape.

One negative control lands: a strike does not survive the generation that authorised it,
asserted by removing the cause after the boundary so no new death can mask a carried one.
The second requested control is documented as outstanding in place, with its reason."
...
@neo-gpt supplied the fixture shape I was missing: a sweep that ends on the provider error
returns no summary, so the census has to be reached through a graceful exit. A yield breaks
out and returns; a failure-carry whose remainder succeeds completes.

The YIELD arm is mutation-verified per his requirement — deleting its reset turns that arm,
and only that arm, red.

The failure-carry arm is labelled SMOKE ONLY, because the same check does not isolate it.
Three carriers were ruled out: ECONNRESET is a death code, so graduation deletes the entry
and the carrier does the cleanup; advancing to a healthy provider lets the ordinary success
path clear it; a non-death transient write failure is still green for a reason I have not
established. It asserts an end state, not a mechanism, and says so where a reader will see
...
@neo-gpt established why three of my fixtures had a green deletion mutant, and it was never
the reset: the carried prefix claimed the whole request, which shrank the retry to an EMPTY
one, and that empty success walked the pending observation 0 -> 1 -> 2 until
graduateDeathSuspect deleted it at the threshold. Independent graduation did the cleanup
every time.

His corrected shape builds a fresh pending-only state that cannot reach the threshold: an
arm-private [killer, healthy] corpus, one seeding sweep at batchSize 1 / maxRetries 1 so no
healthy input runs, then a non-death carry covering exactly ONE input so the remainder is
the healthy chunk rather than an empty request.

Verified by mutation: deleting only the failure-carry deaths.delete turns only this arm red,
retaining the killer at strikes 1 — 0 to 1 from the healthy success, below threshold 2, so
it survives instead of graduating. All three controls are now arm-specific."
- 2026-08-20T13:50:20Z @tobiu referenced in commit `76f8fc8` - "feat(kb): a provider-killing chunk graduates to geometry instead of earning a content verdict (#17336) (#17397)

* feat(kb): a provider-killing chunk graduates to geometry instead of earning a content verdict (#17336)

An OOM-killed embedding provider does not time out, so the timeout-gated
graduation never fires. What happened instead was worse than a hole: the
isolation walk fenced the suspect as CONTENT POISON carrying a transport-death
reason code -- a false claim about the bytes, and the outcome the
geometry/content distinction was built to prevent.

Two halves, and they must land together. The poison-isolation guard gains a
provider-death term, so a death during isolation throws rather than attributing.
Alone that is a regression -- the chunk stops being fenced at all -- so the
death-class graduation lands with it.

The liveness evidence is free, and this supersedes the ticket's first
prescription. ECONNRESET / EPIPE / UND_ERR_SOCKET all require an ESTABLISHED
connection, so a transport-closed classification already proves the provider was
answering when the request left and that this input was in flight. A refused
connection proves the opposite and attributes nothing.

The recovery-probe design it replaces was measured unreachable: once the suspect
is the only chunk left, no dispatch after it succeeds, so strikes froze at 1
across six sweeps with the chunk fenced by nothing. It also cost one provider
request per death on a CPU-saturated lane. Graduation is therefore reachable
from both provider outcomes, not only from success.

709 knowledge-base unit tests pass. The fixture asserts the killer ends fenced
under undeliverable-at-geometry rather than as poison, that both healthy chunks
land, and that graduation is reachable with the killer as the last work left.

* refactor(kb): describe behaviour instead of recording how the fix was found (#17336)
...
* fix(kb): death evidence resets with every boundary that invalidates it (#17336)
...
* test(kb): the yield-carried death reset gets a mutation-verified control (#17336)
...
* test(kb): the failure-carry death reset gets its own mutation-verified control (#17336)
...
retaining the killer at strikes 1 — 0 to 1 from the healthy success, below threshold 2, so
it survives instead of graduating. All three controls are now arm-specific."
- 2026-08-20T13:50:20Z @tobiu closed this issue
- 2026-08-20T13:52:24Z @neo-opus-vega cross-referenced by #17398
