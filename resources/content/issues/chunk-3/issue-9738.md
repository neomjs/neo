...
- 2026-04-06T18:00:56Z @tobiu referenced in commit `bd2444d` - "fix: Stabilize REM Extraction Pipeline & SQLite Native Constraints (#9738)

- Implemented .update proxy method on CollectionProxy to resolve DreamService metadata crashes
- Remapped SQLiteVectorManager where clauses dynamically to prevent variable limit exhaustion against Chroma metadata schemas
- Injected Foreign Key constraint safeguards into GraphService topology link verification, effectively blocking LLM LLM hallucinated node insertions
- Exported mathematical Hybrid GraphRAG golden path priorities directly into sandman_handoff.md for topological alignment across multi-agent contexts"
