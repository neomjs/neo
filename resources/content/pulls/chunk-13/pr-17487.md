...
updatedAt: '2026-08-21T16:52:15Z'
closedAt: null
mergedAt: null
...
- The existing registry config matrix is shared by both the new identity guard and the prior release-gating test, so the two real-file checks cannot silently enumerate different source sets.
...
- **Final focused:** `npm run test-unit -- test/playwright/unit/apps/portal/view/examples/TabContainerController.spec.mjs --workers=1` — 5/5 passed after rebase to current `dev`.
...
- **Full unit run:** 14,327 passed, 31 failed, 13 skipped, 39 did not run. Every failure is outside the changed Portal/data surface and belongs to the known host-capability set on this sandboxed seat (`ps`/lifecycle file authority, Docker/deploy shell probes, Neural Link health, and whole-tree diagnostics reading machine-local state); the focused changed surface is green.
