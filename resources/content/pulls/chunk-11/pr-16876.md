...
updatedAt: '2026-08-10T12:55:10Z'
...
**The one failure is not mine, and I ran the control rather than asserting that.** `ChromaManager.spec.mjs:51 › connect marks the manager connected when heartbeat succeeds` fails **identically with this branch stashed**, and also **in isolation** (11 passed, 1 failed). Pre-existing on `dev`, not order-dependent, not introduced here.
