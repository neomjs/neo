...
updatedAt: '2026-08-16T20:10:58Z'
...
Every `npm i neo.mjs` downloads and unpacks 26.5 MiB of GitHub contributor rankings. Nothing in the framework reads them. The only consumer is the DevIndex app's own store, which fetches the file over HTTP at runtime from the deployed site — `apps/devindex/store/Contributors.mjs:50` resolves it from `Neo.config.basePath`, not from the package.

So the data is shipped to every framework consumer and read by none of them. It is roughly **27% of the unpacked package** for a payload with zero framework consumers.
