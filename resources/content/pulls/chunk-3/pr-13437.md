...
*   **`[TOOLING_GAP]`**: The dedicated external-NL whitebox remains blocked. My local live probe also could not register the AgentOSWidget worker: Neural Link health was up but stale on OpenAPI digest, the visible worker topology only showed an unrelated tenant-side worker, and raw Playwright REPL launch failed on host browser permissions. The focused Playwright e2e runner itself did work.
