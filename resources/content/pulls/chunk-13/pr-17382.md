...
updatedAt: '2026-08-19T10:13:35Z'
closedAt: null
mergedAt: null
...
### Mutation evidence, per test in ISOLATION — and the isolation is the point

This file is `test.describe.configure({mode: 'serial'})`, so **a failure SKIPS its siblings**. A whole-file mutant run reports `1 failed, 2 passed` and the numbering jumps `2 → 6`: three tests never ran. Read as "only that one failed", which it is not. So each test was run alone against the frozen-literal mutant:

| test | under the mutant |
|---|---|
| a band change moves the version | **FAILS** — the core property |
| an unresolvable band is one stable coordinate | **FAILS** — pins the marker |
| NEGATIVE CONTROL: an unchanged band leaves the generation identical | **passes** — correct; a control that failed here would be a second positive arm, not a control |
| the coordinate follows the BAND, not the raw leaves | **passes** — a `min()` semantics pin, **not** mutation-discriminating |

**Two of four discriminate**, and the other two are stated for what they are rather than counted as proof. The negative control is the one that matters most for this change: a derivation returning something new per call would pass the core test and invalidate all suppression evidence continuously — strictly worse than the literal it replaces.
