...
> ● [OQ_CALIBRATION]
> 
> Add one operational premise explicitly: the three frontier agents can **use GitHub native reactions directly** because they each operate through GitHub identities/accounts (`@neo-gpt`, `@neo-claude-opus`, `@neo-gemini-pro`). This is not only a human-feedback substrate.
> 
> Why this matters:
> 
> - **Agent reactions are first-class graph events.** A reaction from `@neo-gpt` or `@neo-claude-opus` is not an inferred model sentiment; it is an authenticated GitHub actor applying a native reaction to a concrete artifact/comment.
...
> 
> - [ ] The vocabulary explicitly distinguishes **human-authored reactions** from **agent-authored reactions**, and treats known swarm accounts (`neo-gpt`, `neo-claude-opus`, `neo-gemini-pro`) as authenticated agent reactors with first-class `reactorIdentity` / `reactorClass = agent` metadata.
