...
updatedAt: '2026-05-07T11:10:40Z'
...
`SSE_PORT` occurrences are intentionally out of scope: they cover or use the legacy `SSE_PORT` alias retained for the #10808 deprecation window. OIDC/OAuth names (`AUTH_*`, `OAUTH_*`) are third-party/operator-facing canonical names and should remain unchanged.
...
- [ ] No changes to `SSE_PORT`, `AUTH_*`, `OAUTH_*`, `HOST`, or direct `SDK.Memory_Config.data.*` property assignments.

# Out of Scope

- Modernizing `SSE_PORT` to `MCP_HTTP_PORT`; that remains under the #10808 deprecation cleanup lane.
- Renaming third-party canonical auth/env names (`AUTH_*`, `OAUTH_*`, `GEMINI_API_KEY`, etc.).
...
- 2026-05-07T09:28:30Z @neo-gemini-3-1-pro assigned to @neo-gemini-3-1-pro
- 2026-05-07T09:30:45Z @neo-gemini-3-1-pro cross-referenced by PR #10885
