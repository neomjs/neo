...
updatedAt: '2026-08-09T18:10:23Z'
...
- 2026-08-09T16:16:36Z @neo-opus-grace referenced in commit `ac8adfb` - "fix(ai): bound the resolver reason by membership, not by shape (#16799)

@neo-gpt's exact-head review: the previous bound admitted anything matching
/^KB_[A-Z0-9_]{1,120}$/ and called that a closed vocabulary. It is not — the
producer chooses the string, so KB_SECRET_ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789
satisfied the pattern and travelled verbatim into a durable, consumer-visible
record. The same boundary defeated a second time, by the shape of the check
rather than its intent.

My hostile control could not have caught it: a clone URL fails any KB_* shape
test, so it passed even against a gate that only tested shape. The added control
uses a deliberately WELL-FORMED code, asserts it matches the old pattern, and
requires it to be rejected anyway.

The KB_* arm is gone entirely — a resolver reaching a remote has no business
emitting this service's own codes. What remains is membership in a fixed
transport set; everything else collapses to one literal.

Co-Authored-By: Euclid <neo-gpt@neomjs.com>"
- 2026-08-09T16:52:13Z @neo-opus-grace cross-referenced by #16816
- 2026-08-09T18:10:23Z @tobiu referenced in commit `18e3f4d` - "fix(ai): stop asking an unwired resolver to re-derive a delta we already proved (#16799) (#16801)

* fix(ai): stop asking an unwired resolver to re-derive a delta we already proved (#16799)

The tenant-sync lane resolved its own deletion set via gitMirror.diffRevisions(),
carried it explicitly in `deleted`, and then also forwarded baseRevision — which
asks IngestionService to derive that same set through a resolver that has no
production implementation. The request could only fail, so every tenant repo past
its first sync sat at consecutiveFailures 12 with its cadence pinned at the 2h
backoff cap and its corpus frozen.

The envelope stops asking. IngestionService's contract is unchanged: a caller that
genuinely requests derivation and supplies no tombstones still fails closed, and a
resolver that is present and throws is now distinguishable from one that was never
wired.

Co-Authored-By: Euclid <neo-gpt@neomjs.com>
Co-Authored-By: Vega <neo-opus-vega@neomjs.com>

* fix(ai): bound the resolver failure reason to a closed vocabulary (#16799)
...
* fix(ai): bound the resolver reason by membership, not by shape (#16799)
...
Co-authored-by: Euclid <neo-gpt@neomjs.com>
Co-authored-by: Vega <neo-opus-vega@neomjs.com>"
- 2026-08-09T18:10:23Z @tobiu closed this issue
