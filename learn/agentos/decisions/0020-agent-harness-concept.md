...
| **Amended** | 2026-08-08 (#16747, [ADR 0030](0030-fm-client-topology.md) — the FM client topology, graduated from D#16720): §3's in-process-Electron-main target is re-scoped to the **packaged local plane bootstrap** (bundle = organism), never the cockpit's runtime identity — the cockpit is a pure client of the composition's fleet service in every topology; §4 guardrail 4's restart affordances are re-scoped as **plane-owned surfaces** behind `CAN_ADMINISTER_FLEET_OF`, invoked over the authenticated fleet wire. Inline notes at both points; ADR 0030 §3.1 is the canonical statement. |
...
- **Shell: Electron, decided** (operator, 2026-06-12): always Chromium + always Node.js — worker-topology determinism fleet-wide; the Agent OS runs **in-process in the Electron main** (target), with **child-process supervision as the sanctioned fallback** capping the topology risk (#13033 carries the spike). Tauri retired. Web-served mode remains the development convenience + goodie, never the priority. *[Amended 2026-08-08 — ADR 0030 §3.1 (#16747): the in-process-main target describes the PACKAGED local plane bootstrap (bundle = organism), never the cockpit's runtime identity. The cockpit is a pure client of the composition's fleet service in every topology; attach-or-own is retired as cockpit identity (C5 #16746).]*
...
4. **Fleet lifecycle owns restart affordances** — including runtime MCP-server restarts with settle-or-reject promise semantics. *[Amended 2026-08-08 — ADR 0030 §3.1 (#16747): the affordances are plane-owned surfaces behind `CAN_ADMINISTER_FLEET_OF`, invoked over the authenticated fleet wire — the cockpit requests and renders outcomes, never owns the affordance; settle-or-reject semantics remain plane-side contracts.]*
