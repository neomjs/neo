...
updatedAt: '2026-04-22T20:52:07Z'
...
- [ ] **Exception matrix codified** in `pull-request §6`:
  ...
  - **`7-day availability fallback`**: author self-attests in the PR body that no cross-family reviewer has been active on the repo for ≥ 7 days; verifiable via `query_raw_memories(query="<family-keyword>")` returning no hits within window. Fallback invocation MUST include the query result + timestamp in the attestation.
