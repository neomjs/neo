...
updatedAt: '2026-08-09T10:21:23Z'
...
- 2026-08-09T10:21:23Z @tobiu referenced in commit `5927ad4` - "feat(ai): ask sends a reasoning-effort control instead of thinking its budget away (#16768) (#16772)

* feat(ai): ask sends a reasoning-effort control instead of thinking its budget away (#16768)

summaryReasoningEffort and graphReasoningEffort have shipped since #13853/#13854 and
both default to 'none'. The ask path sent no control at all, so a reasoning-capable model
applied its own default: measured on a 26B-class local model with a ~7,900-token grounded
prompt, 'none' answered in 33.3s while sending nothing took 86.7s and returned an EMPTY
answer, with 297 of 299 completion tokens consumed by reasoning.

The empty answer is the defect; the latency merely accompanies it. A regression check
written against wall-clock would pass while a longer prompt still exhausted the budget.

The leaf lands in askSynthesis rather than beside its two siblings under localModels.chat,
because ask builds its model from THAT block ("the dedicated askSynthesis block, NOT the
global") and may point at an entirely different provider, model and endpoint. A leaf on
the chat model would tune a model the ask path never uses. The ticket prescribed the
sibling placement; reading the consumer corrected it.

Deliberately NOT added to askSynthesisGuard's required set: an overlay predating this leaf
must keep answering with today's behaviour rather than refuse. Refusing to answer is a
worse failure than answering slowly, and this leaf exists to improve answers.

miniSummary reads the existing summaryReasoningEffort leaf that SessionService already
consumes — both are summarization, so a second knob would be two names for one decision.

Specs are mutation-proven: removing the call-site line reddens all three substantive
assertions. An earlier revision of the empty-string test passed under that mutation and
was rewritten rather than shipped.

Co-Authored-By: Euclid <neo-gpt@neomjs.com>

* fix(ai): the reasoning-effort spec resolves the committed template, not the local overlay (#16768)
...
Co-authored-by: Euclid <neo-gpt@neomjs.com>"
- 2026-08-09T10:21:23Z @tobiu closed this issue
