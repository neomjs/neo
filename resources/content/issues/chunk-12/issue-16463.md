...
updatedAt: '2026-08-07T20:09:29Z'
...
## RSS series from a fresh mc-server incarnation — the FLOOR is the leak-relevant signal, not the peaks

Offering data rather than a conclusion on this ticket's open question (*is ~500MB a leak or a bounded working set*). `mc-server` was recreated at `2026-08-07T17:52:17Z` for #16642's declared ceiling, which gave a clean process to watch from zero.

Sampled every 20 minutes:

```
post-recreate (age 0 → 2h16m)   270.1 → 314.4 → 308.2 → 348.4 → 331 → 365.4 → 405.9 MiB
pre-recreate  (age ~5h50m+)     441.2 → 511.5 → 486.2 → 493.4 → 508.2 MiB
```

**It is a sawtooth with an upward trend, not monotonic growth** — two decreases in the post-recreate series (`314.4→308.2`, `348.4→331`) are GC. I had it as monotonic from recollection and reading the actual series corrected me, which is worth stating because the distinction decides the instrument:

**For a leak question, the rising FLOOR is the signal, not the peaks.** A peak series measures GC timing as much as live-set size; the trough after each collection approximates what could not be reclaimed. The troughs here are `270 → 308 → 331` — rising. That is the sharper read, and it is available from data already being collected.

**Three caveats, all of which keep this from being an answer:**

1. **A fresh process filling caches produces this exact shape.** Early upward drift is expected and says nothing about a leak until it plateaus. The question is whether the floor stops rising, and 2h16m is not enough to say.
2. **No age-matched baseline.** My earliest sample of the previous incarnation is at age ~5h50m, already at ~480 MiB. Comparing a 2h process against a 6h one is the cross-window error I retracted elsewhere today, so I am not doing it.
3. **The trend rate is not stable enough to extrapolate.** At the crude average it would reach the declared 768 MiB ceiling in a few hours; the series is too short and too noisy for that to be a prediction.

**What is armed:** a monitor alerts at RSS ≥ 600 MiB and on any second heap abort, sampling every 20 minutes. So if the floor keeps rising, this ticket gets a receipt rather than a guess — and if it plateaus, that is the bounded-working-set answer arriving on its own.

Post-#16642 the wall is a **declared 768 MiB** rather than an emergent ~560 MiB heuristic, so the next abort — if any — happens at a known number. That is controllability, not visibility: nothing measures against that ceiling, which is #16630's Slice B and unbuilt.
