...
**Wake-control (the `add_message` `wakeSuppressed` param, relaxed in #12635; priority tiered in #14576; claim-class flipped to quiet-by-default in #15987):** classify by **recipient actionability, not primitive name**. **Wake** (omit `wakeSuppressed`): direct review / re-review, `REQUEST_CHANGES`, `[lane-override]`, or coordination overlapping the recipient's active or owned surface. **Claim-class broadcasts are status, not interrupts:** `[lane-claim]` / `[review-claim]` / `[claim-corrected]` / `[drive-claimed]` on `AGENT:*` default to quiet — peers read them at their next intake, and collisions stay fail-closed at the claim surfaces (the `requireUnassigned` assignee gate plus intake's claim-race re-check). A contested-lane resolution that must interrupt sets `wakeSuppressed: false` explicitly. Use `priority: 'high'` only for act-now wakes: direct review / re-review, lane-unblock, operator relay, contested-lane resolution, `REQUEST_CHANGES`, or owned-surface overlap; other unsuppressed traffic stays `normal`. **May suppress** (`wakeSuppressed: true`): ordinary non-overlapping awareness — PR-opened observer notes (you're not the reviewer), lane-progress pings, acks; the recipient still receives them on their next `list_messages` rather than via an interrupt. MailboxService's actionable-classifier remains the aligned guardrail: suppressed actionable lifecycle subjects are rejected mechanically. (Additive to the session-sunset self-DM suppression, which stays valid.)
