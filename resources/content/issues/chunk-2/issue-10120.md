...
updatedAt: '2026-06-21T03:52:03Z'
...
### @neo-gpt - 2026-06-21T03:52:03Z

[ARCH_ALIGNMENT] Intake refresh on 2026-06-21: this is still a plausible product/Brain direction, but it is not branch-ready as written.

V-B-A evidence:

- Live issue state: #10120 is open, unassigned, and had no discussion comments before this intake.
- Auth/provenance check before public write: `gh api user --jq .login` => `neo-gpt`; `NEO_AGENT_IDENTITY` => `@neo-gpt`.
- Prior-art sweep degraded: `ask_knowledge_base` timed out after 15000ms, and `query_raw_memories` failed on the Memory Core embedding write canary timeout. I used live GitHub search + local source inspection as fallback.
- Exact successor/duplicate search for `externalVisibilitySignal`, `visibilityGapWeight`, `External Visibility`, and `visibility-gap` found #10120 as the only exact open ticket. Adjacent precedents are different: #10275/#12603 Silent Threads and #11447/#11622 Consumer Friction are visibility-only handoff sections, not external-metric ranking input.
- The implementation surface in the ticket is stale: the body names `buildScripts/ai/runGoldenPath.mjs`, but #12078/#12079 deleted the dead standalone Golden Path runner. Current source/docs route Golden Path through `ai/services/graph/GoldenPathSynthesizer.mjs` and the orchestrator `golden-path` task; `learn/guides/fundamentals/CodebaseOverview.md` explicitly notes that no separate standalone runner ships after #12078.
- Current scoring is centralized in `GoldenPathSynthesizer.synthesizeGoldenPath()`: semantic Chroma distance + SQLite structural weight produce the computed Golden Path, then `resources/content/sandman_handoff.md` is regenerated in the same render pass.
- ADR 0014 makes this a cloud/local boundary issue: core `golden-path` is cloud-deployable, but repo-enrichment sections such as active PR state and local issue backlog are Neo-maintainer-repo-specific and must be deployment-mode-gated. Adding GitHub traffic/sponsors/blog-file metrics extends that exact caveat unless the ticket defines a tenant-safe source contract.

Blocking gap:

The ticket mixes at least four contracts in one implementation claim without a current Contract Ledger: (1) core Golden Path ranking math, (2) Neo-local repo-enrichment rendering, (3) external GitHub/network metric acquisition + caching/rate limiting, and (4) cloud tenant behavior when those Neo-specific sources do not exist. A direct PR from this body would either reintroduce stale runner assumptions or smuggle local-only visibility data into a cloud-deployable lane.

Recommended reframe before implementation:

1. Decide whether V1 is **Neo-local repo enrichment only** or **tenant-deployable ranking input**. Those are different contracts.
2. If Neo-local only: route through the existing `repoEnrichmentEnabled` boundary and explicitly keep the signal out of tenant deployments unless a deployment-mode gate enables it.
3. If tenant-deployable: define the source provider shape for traffic/content/sponsors-style metrics, the cache TTL, missing-data semantics, and the exact graph persistence schema before touching scoring.
4. Replace stale path references with current surfaces: `ai/services/graph/GoldenPathSynthesizer.mjs`, `ai/daemons/orchestrator/scheduling/pipeline.mjs`, `ai/daemons/orchestrator/taskDefinitions.mjs`, `apps/portal/store/BlogNeo.mjs`, `apps/portal/store/BlogMedium.mjs`, and ADR 0014.
5. Split ACs so tests can prove the boundary: score calculation, generated handoff section, missing-source fallback, cache/rate-limit behavior, and cloud-profile disabled behavior.

Applied `not-code-ready` + `needs-design`. No implementation claim taken.
