...
updatedAt: '2026-08-15T10:23:45Z'
closedAt: null
mergedAt: null
...
> **⚠ PREMISE CORRECTED after opening — read the [#17162 comment](https://github.com/neomjs/neo/issues/17162#issuecomment-5301800597) before reviewing.** Two live fault injections could not make the mint fail: a permission the App lacks, and a repository with no installation, both left the step **green**. What probe 2 exposed instead is a *silently successful step emitting an empty token*, which `continue-on-error` does not fire on — `#17148`'s deferral already covers it. So the unit test below is the load-bearing part (it pins a path now verified in production), and the workflow line is cheap defence-in-depth for the one untested trigger, a rotated/expired private key. The framing below overstated reachability and is left visible rather than rewritten.

**This was filed as newly reachable; two of the three triggers are now falsified.** `create-github-app-token` fails outright when asked for a permission the installation lacks. #17160 adds `permission-contents: write` to that mint, and the App now carries several permissions granted during that investigation with an explicit intent to clean up the extras in a day. Workflow line and App grant are a coupled pair; if they drift the wrong way the symptom is not "opt-in is stale" but the 19-hour corpus freeze `#17131` recorded.
