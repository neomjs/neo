...
- **Stale-wake silent-mark-read pattern (per #11907 / Epic #11829 Layer 3):** Marking heartbeats / stale-event wakes as "no action" while ZERO own-PRs ship is deference-slip dressed as discipline. Trigger: query unassigned current-release backlog OR confirm zero-candidate halt with named reason. Exhausted-self-assigned-bench is NOT a halt-state. **Empirical anchor:** 2026-05-24 session (Claude Opus 4.7) repeatedly mark-readed heartbeats over multiple hours while only Drop+Superseded PRs and ZERO own-PRs shipped, until operator velocity-pushback at ~13:18Z surfaced the pattern; #11829 wake-driver Epic was the substrate-evolution response.
