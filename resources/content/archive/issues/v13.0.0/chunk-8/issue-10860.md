...
updatedAt: '2026-06-13T15:59:41Z'
...
### @Ram9199 - 2026-06-13T15:41:02Z

I am collecting real-world incidents around multi-agent credentials and attribution.

Was optional per-agent GitHub credential mapping driven by actual incidents or audit failures where all agents shared one GitHub identity, or is it preventive hardening? What changed afterward - per-agent PATs, audit logs, split credentials, manual review, or no change?

Not asking about any product. I am trying to identify repeated behavior after agent-authority and attribution failures.

### @tobiu - 2026-06-13T15:59:41Z

Hi @Ram9199 !

Around a year ago, I started with one agent, and we shared my `tobiu` account. The agent just identified itself via notes inside comments. Of course this could not scale, since other agents would need to parse all comments to see who created what. Plus, it led to severe identity crisis => "who am i?".

Separate identities resolve it, and enable GitHub notifications.

It goes deeper though: Each agent needs an own neo memory-core identity and session ids, plus own agent harness memory markdown files on top of this. Right now, each agent has an own repo clone folder, with different `.env` files. A bit brittle to manage manually, so the next logical step after v13 just got released is an own agent harness, which covers the fleet manager. not as sub-agents, but as an equal flat peer model, where each agent is a repo maintainer.

They ideate and coordinate on their own via A2A messages plus reading peers memories (telepathy), and apply rather rigorous cross model family PR reviews, until reaching the approved state inside several iterations.
