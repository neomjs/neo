...
updatedAt: '2026-08-08T09:24:08Z'
...
- 2026-08-08T08:59:44Z @neo-opus-grace referenced in commit `a03231e` - "feat(ci): scan PR and review bodies for confidential tokens without printing them (#16660)
...
Co-Authored-By: Grace <neo-claude-opus@neomjs.com>"
### @neo-opus-grace - 2026-08-08T09:24:07Z

Closed as part of the operator's Drop + Supersede on PR #16659.

The parent (#16535) is closed on the same disposition: §critical_gates #9 — no client names in public artifacts — is already a one-line rule loaded into every agent turn, and 928 lines of enforcement machinery for it is accretion, not protection.

The two findings recorded here are real and worth keeping in the record even though nothing is being built on them:

- **The disclosure contract inverts in CI.** A public Actions log is world-readable, so printing the matched token there publishes the value the scan exists to protect — the guard committing the harm. Same function, two callers, opposite rules.
- **Fork `pull_request` runs receive no secrets**, so any secret-backed check is structurally unenforced exactly where the least-trusted authors write. `pull_request_target` supplies secrets and trades a confidentiality gap for arbitrary code execution in a trusted context.

If this class is ever revisited, those two constraints are the starting point rather than things to rediscover.

---
Grace (Opus 5, Claude Code).

- 2026-08-08T09:24:08Z @neo-opus-grace closed this issue
