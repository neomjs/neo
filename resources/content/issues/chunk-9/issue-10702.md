...
updatedAt: '2026-05-22T08:10:05Z'
...
- 2026-05-04T18:56:08Z @neo-claude-opus added the `enhancement` label
- 2026-05-04T18:56:08Z @neo-claude-opus added the `ai` label
### @neo-claude-opus - 2026-05-22T02:32:01Z

**Input from Claude Opus 4.7 (Claude Code):**

> ❋ `[TOOLING_GAP]` **Re-surfaced 2026-05-22 — operator-flagged; recurred during `ticket-intake`.**
> 
> @tobiu flagged `get_conversation`'s PR-only scope again today. It recurred live while I ran `ticket-intake` on #11724: `get_conversation({pr_number: 11724})` → `Could not resolve to a PullRequest with the number of 11724` (#11724 is an issue).
> 
> **Priority-elevating angle not yet in the body:** `ticket-intake-workflow.md` §1.1 carries a hard `MUST` — *"You MUST use the `get_conversation` tool to fetch the live issue body and comment thread directly from GitHub"* — for **every** issue intake. With `get_conversation` PR-only, that mandated step always errors on issues; every agent running `ticket-intake` silently falls back to `gh issue view`. So this isn't only the surgical-fetch / token-budget parity the body frames (epic-review anchor) — it's a **live broken skill-contract**: a `MUST`-step that cannot succeed as written. ROI reframes from "nice selector parity" to "fix a mandated workflow step that currently always fails."
> 
> The Fix prescription still holds — I re-verified `get_conversation` still dispatches to `PullRequestService.getConversation` only. One freshness note for whoever implements: the github-workflow MCP source layout has shifted since this was filed (2026-05-04) — the `toolService.mjs` dispatch is now at `ai/mcp/server/github-workflow/toolService.mjs:98`, not the `services/`-prefixed path the body links. Refresh the §Architectural Reality `file:line` anchors at implementation time.
> 
> Still OPEN + unassigned — clean pickable lane.

- 2026-05-22T02:55:02Z @neo-claude-opus assigned to @neo-claude-opus
- 2026-05-22T07:08:39Z @neo-claude-opus cross-referenced by PR #11746
- 2026-05-22T07:27:46Z @neo-claude-opus referenced in commit `36e72b8` - "docs(github-workflow): get_conversation issue+PR guide sync (#10702)

GitHubWorkflow.md section 4.3 still described get_conversation as PR-only
via GET_CONVERSATION. Sync it to the dual-input contract: pull request or
issue, exactly one of pr_number / issue_number, shared comment selectors.
Addresses the PR #11746 cycle-1 Required Action.

Co-Authored-By: neo-claude-opus <neo-claude-opus@neomjs.com>"
- 2026-05-22T08:10:05Z @tobiu referenced in commit `a2bb8b6` - "feat(github-workflow): extend get_conversation to issues (#10702) (#11746)

* feat(github-workflow): route get_conversation to issues + PRs (#10702)

get_conversation dispatched to PullRequestService only; issue numbers were
unresolvable. Adds IssueService.getConversation + GET_ISSUE_CONVERSATION
(issue-side twins of the PR path, identical selector contract), a
getConversationRouter dispatch picking the service by pr_number xor
issue_number, and the dual-input openapi schema. Also strips pre-existing
trailing whitespace the whole-file check-whitespace hook flagged in the two
touched files.

Co-Authored-By: neo-claude-opus <neo-claude-opus@neomjs.com>

* test(github-workflow): cover get_conversation issue routing + selectors (#10702)

IssueService.getConversation: 11 cases mirroring the PR selector suite
(full / comment_id / since_comment_id / last_n / precedence / missing
issue_number / GraphQL-error). getConversationRouter: 5 cases pinning
pr_number-xor-issue_number routing + AMBIGUOUS/MISSING rejections.

Co-Authored-By: neo-claude-opus <neo-claude-opus@neomjs.com>

* docs(github-workflow): get_conversation issue+PR guide sync (#10702)
...
- 2026-05-22T08:10:05Z @tobiu closed this issue
