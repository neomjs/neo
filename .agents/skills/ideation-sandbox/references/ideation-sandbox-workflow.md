...
- **Double Diamond Divergence Guard (Mandatory for High-Blast-Radius):** If the proposal intends to graduate to an Epic, introduces a new Skill/Rule, or modifies substrate-level architecture, a **Divergent Options Matrix** is MANDATORY before graduation (recommended but optional for standalone `[GRADUATED_TO_TICKET]` work).
  - *Process Gate:* The matrix MUST be authored in the Discussion body *before* any `[RESOLVED_TO_AC]` tags are applied to prevent post-hoc rationalization.
  - *Empirical Depth:* Every rejected alternative in the matrix MUST cite at least 1 falsifying source (e.g., prior commit, precedent code, memory-mining hit).
  - *Substrate Decay:* A 6-month effectiveness measurement plan MUST be defined to evaluate if the new rule prevents rubber-stamping or just adds paperwork.
