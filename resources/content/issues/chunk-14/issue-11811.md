...
- The same command shows `normalizeAgentIdentityNodeId()` is exported from `ai/scripts/resumeHarness.mjs` and already unit-covered for `neo-claude-opus` -> `@neo-claude-opus`, prefixed identities, and whitespace trimming.
...
`NEO_AGENT_IDENTITY` can be configured in GitHub-login form (`neo-claude-opus`) while A2A identity surfaces are graph node ids (`@neo-claude-opus`). #11804 fixed this for `SwarmHeartbeatService`, but `KbAlertingService` still passes the raw env-var value as its alert sender.
...
|---|---|---|---|---|---|
| KbAlertingService A2A alert sender identity | AgentIdentity node-id convention + #11804 normalizer | `NEO_AGENT_IDENTITY=neo-claude-opus` emits alerts from `@neo-claude-opus`; already-prefixed values stay unchanged | If env var is unset, continue using `@system` | Inline JSDoc/summary remains enough; no user-facing docs needed | Unit test in `KbAlertingService.spec.mjs` + existing `resumeHarness.spec.mjs` normalizer tests |
...
- [ ] AC3 — Unit coverage proves unprefixed env input (`neo-claude-opus` or equivalent) produces a prefixed sender in the dispatched A2A alert payload.
...
> 4. Add a focused test in `test/playwright/unit/ai/daemons/KbAlertingService.spec.mjs` proving `NEO_AGENT_IDENTITY=neo-claude-opus` dispatches sender `@neo-claude-opus`.
