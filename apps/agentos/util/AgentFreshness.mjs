 ...
 * "we don't know" exactly like the sibling {@link module:apps/agentos/util/SourceHealth}
 * source contract.
 *
 ...
 *
 * @module apps/agentos/util/AgentFreshness
 */
...
/**
 * Static Fleet pane-freshness classification and presentation utilities.
 * @class AgentOS.util.AgentFreshness
 * @extends Neo.core.Base
 */
class AgentFreshness extends Base {
    /**
     ...
     * @type {String[]}
     */
    static FRESHNESS_CLASSES = Object.freeze(['fresh', 'stale', 'lost', 'unobserved'])

    static config = {
        /**
         * @member {String} className='AgentOS.util.AgentFreshness'
         * @protected
         */
        className: 'AgentOS.util.AgentFreshness'
    }
     ...
     */
    static classifyPaneFreshness(ledger, now) {
     ...
     */
    static describePaneFreshness({freshness, ageMs} = {}) {
        const relative = Number.isFinite(ageMs) ? AgentFreshness.formatAge(ageMs) : null;
     ...
     * ({@link module:apps/agentos/util/ViewerWakeTelltale}) must word relative ages identically —
...
export default Neo.setupClass(AgentFreshness);
