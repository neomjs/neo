...
updatedAt: '2026-08-21T20:08:28Z'
...
| `neo.harness.*` schema strings | `dockZone.v1`/`dockLayout.v2` contracts | UNCHANGED at runtime; disposition clause added | existing fail-closed validation | new ADR clause | 35 occurrences / 6 src files + consumer inventory |
...
- [ ] ADR file + H1 renamed; repo-wide grep for `0029-harness-docking-design` returns zero hits outside `resources/content/**` mirrors and git history; `lint-adr-seam-table` green.
- [ ] Model contract doc renamed to `DockZoneModel.md`; repo-wide grep for `HarnessDockZoneModel` returns zero hits outside mirrors/history; learn registry/portal routing verified by build or route check, not assertion.
...
- Re-homing the model contract doc out of `learn/agentos/` — the `#17500` extraction wave re-maps that tree wholesale within days; single-doc moves against it are churn. Revisit under the wave's doc plan.
