...
updatedAt: '2026-08-24T15:29:04Z'
...
3. A reached module with no `script-module` row is outside this classifier; it is not an `unregistered-module` blocker. Inventory membership/residue remains proof 1's job.
...
6. Retire the false aggregated `topology-edge-closure-unregistered-module` output from the current proof receipt without weakening the remaining topology blockers.
...
| ungoverned reached modules | ADR 0040 §2.4 proof-set separation | No `script-module` row means no module-custody claim in proof 2, not missing proof-1 authority. | Inventory disk/authority residue remains the fail-closed membership gate. | Reached dependency without a module row emits no unregistered blocker; inventory missing-row mutation remains red in its own suite. |
...
| paired proof receipt | #17533 + ADR 0040 §2.6 | Current source reports zero false unregistered-module findings and zero instrument errors; surviving findings remain exact and successor-owned. | No blanket suppression or count baseline. | Exact SHA-bound before/after receipt and class census. |
...
- [ ] A reached `ai/**` module with no `script-module` row does not emit `topology-edge-closure-unregistered-module`; proof 1 remains the sole membership/residue gate.
...
- [ ] The current paired proof is source-bound with zero instrument errors and zero aggregated unregistered-module findings; surviving topology findings remain exact and successor-owned.
...
- **Making the 290 identities disappear with a blanket suppression.** The fix is surface-aware classification, while Cloud-package/explicit-Cloud/unresolved controls remain red.
