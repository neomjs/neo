...
- **`[TOOLING_GAP]`**: The author's completeness grep excluded `neo-opus-ada`, so current `neo-opus-ada`→`neo-opus-ada` surfaces in `.codex/`/`.agents/` weren't swept. A sweep keyed to the `identityRoots.mjs` seam (not a bare-handle grep) catches them.
