...
updatedAt: '2026-08-09T20:04:38Z'
...
- [x] `shared-heap-observation-data` is declared in the compose `volumes:` block and mounted at `AiConfig.heapObservation.dir` in `kb-server`, `mc-server` and `orchestrator`. **Met** — @neo-gpt-emmy rendered exact-head `docker compose config`: KB/MC read-write, orchestrator read-only, Fleet unmounted.
...
- [x] A guard asserts the invariant from the compose source, with the writer roster **derived** from `getHeapObservationServiceKey()` declarations, and fails closed if that derivation returns an empty roster. **Met** — the acorn-based producer census refuses an override shape it cannot resolve. Convicted at head `94559749d7`: rewriting kb-server's override to the indirect form `{ const key = 'kb-server'; return key }` reds 2 tests rather than silently dropping it from the roster.
- [x] **Both guard halves are mutation-convicted:** removing a writer's mount reds, and making the reader's mount writable reds. A guard that cannot red against the shipped defect is not evidence. **Met** — four mutations, each verified to APPLY before reading its result: drop a writer mount (red), make the reader writable (red), indirect producer override (red, 2 failed), definition-level anchor move (red, 3 failed).
- [x] The directory segment is read from `ai/configBase.mjs` rather than restated, so renaming the leaf reds the guard instead of leaving the mount pointing at a directory nothing writes. **Met** — the guard pins the leaf anchor, its DEFINITION, `neoRootDir`, and planeConfig's relative root. Convicted: `planeDataRootDefault = resolvePlaneDataRoot({rootDir: neoRootDir})` → `= neoRootDir` reds 3 tests. A name-pin alone stayed green against this, which is why the definition link is separate.
- [x] Sibling compose specs stay green (`ParityPlaneVolumeScoping`, `DeclaredHeapCeilings`, `daemon.spec` `#15759` sole-owner-volume, `FleetServerComposition`, `ChromaPersistPathContract`, `DeploymentTrustDefaults`, `HostEdgePosture`, `LaneEnablementSignal`, `DeploymentRuntimeAccessService`, `DeployPipelineRevisionPin`). **Met** — 101 passed locally across all ten listed specs; exact-head CI green.
