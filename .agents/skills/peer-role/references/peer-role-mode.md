...
**Publish-time re-check (per #15780; restores #12621 AC2b, generalized):** AC2's "immediately before the write-operation" assumes claim and write are adjacent. When they are NOT — a multi-turn sweep, review, deposit, or any composed artifact — the composition window is unobserved, and the claim-time §6.6 check has expired by the time you publish. So: **when the write does not directly follow the claim, re-run `list_messages` immediately before publishing**, filtered by recency + scope exactly as the claim-time sweep was (read-status is consumption tracking, not relevance — an already-read `[lane-claim]` is still an active claim). On a hit — competing claim, contested-lane resolution, or a superseding artifact — the §6.6 hierarchy decides, and you re-frame or stand down BEFORE publishing rather than publishing and correcting after. Empirical: three review-path collisions (#12600 / #12616 / #12620) produced #12621's AC2b, whose payload edit was deferred and never landed; four more followed on the lane-claim path in one day (#15780's instance table), including a §5.2 gate artifact published as first-on-thread when it was third, and a duplicate ticket filed inside a 3-minute claim→PR window. Two shapes, one missing check: **collision-shaped** (someone else writes in your window) and **staleness-shaped** (the world moves in your window).

The same gate covers a second genre (per @neo-opus-grace, #15780): **a claim about a shared artifact's current state is yours to verify at the artifact, regardless of which peer told you** — gate open/closed, PR merge-ready, ticket claimed, comment superseded. A claim about a peer's *own turn* you may take at face value; auditing every peer's timeline before quoting them does not scale, and is not what fails. Only the first genre goes stale under you, and it goes stale fastest exactly when a thread is active: on 2026-07-24 a peer relayed a Discussion-gate state that was 20 seconds out of date at fetch time, twice in one hour, because a Discussion gate was not recognised as the same genre as a PR — where `gh pr view` before asserting merge-readiness is already reflexive. **A verification carries a timestamp** (@neo-opus-grace): having checked is not a durable property, so a check run earlier in the same turn is already spent — cite the `checkedAt` you actually publish against, and re-run rather than reuse it. Sibling precedent for that half: `pr-review-guide.md` §"PR-state echo is not a cache". Retires into a mechanical guard if one is ever built.
...
**Wake-control (the `add_message` `wakeSuppressed` param, relaxed in #12635; `[lane-claim]` tightened in #14100; priority tiered in #14576):** classify by **recipient actionability, not primitive name**. **Wake** (omit `wakeSuppressed`): every `[lane-claim]` (MailboxService rejects suppressed lane-claims mechanically), direct review / re-review, `REQUEST_CHANGES`, `[lane-override]`, or coordination overlapping the recipient's active or owned surface. Use `priority: 'high'` only for act-now wakes: direct review / re-review, lane-unblock, operator relay, contested-lane resolution, `REQUEST_CHANGES`, or owned-surface overlap; other unsuppressed traffic stays `normal`. **May suppress** (`wakeSuppressed: true`): ordinary non-overlapping awareness — PR-opened observer notes (you're not the reviewer), lane-progress pings, acks; the recipient still receives them on their next `list_messages` rather than via an interrupt. MailboxService's actionable-classifier remains the aligned guardrail: suppressed actionable lifecycle subjects are rejected mechanically. (Additive to the session-sunset self-DM suppression, which stays valid.)
