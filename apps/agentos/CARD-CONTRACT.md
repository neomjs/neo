...
| **Engine tag** (mock: "opus-4.8" / "fable-5") | `engineTag` — the identity-roots `modelDesignation` via the same #14802 assembler join (era metadata source when `#11318` lands; the resolver is the one re-point site) | **session/era metadata, never identity** (§2.3.7) — a tag change never re-keys the card; the June→July swap is the reflexive fixture | no identity root → `engineTag: null` → tag hidden (metadata is optional; identity is not) |
