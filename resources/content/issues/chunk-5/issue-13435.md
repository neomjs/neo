...
updatedAt: '2026-06-25T05:23:36Z'
...
- 2026-06-20T18:46:23Z @neo-gpt added the `not-code-ready` label
- 2026-06-20T18:46:23Z @neo-gpt added the `needs-design` label
### @neo-gpt - 2026-06-20T18:46:45Z

## Triage disposition

Marking `not-code-ready` + `needs-design`. The ticket body explicitly challenges the prior #12990 healthcheck-token decision and names Discussion #13505 as the convergence venue. That makes this a security/policy design boundary, not an implementation-ready cloud healthcheck leaf.

Implementation should wait until #13505 records the accepted shape and the #12990 disposition (loopback exemption vs dedicated static health secret vs status quo). Until then, keep this excluded from normal code-lane pickup.
...
### @neo-gpt - 2026-06-25T05:08:48Z

## Design status update

Discussion #13505 now records the OQ4 source refresh.

Disposition: keep this ticket split from the first deploy-readiness implementation. That implementation may model the existing `NEO_MCP_HEALTHCHECK_TOKEN` token route as a required readiness input under `gitlab-pat`, but it must not change the healthcheck-auth policy.

Reason: this ticket challenges #12990's token-route decision, and ADR-0025 / ADR-0026 both make healthcheck-auth changes a no-privilege-smuggling boundary. A loopback exemption or static health secret needs its own security artifact that explicitly supersedes #12990 before code changes.

No implementation claim here; this remains `not-code-ready` / `needs-design`.

- 2026-06-25T05:21:54Z @neo-gpt cross-referenced by #13432
### @neo-gpt - 2026-06-25T05:23:36Z

## Design status update — #13505 proposes graduation with this lane still split

Discussion #13505 is now `[GRADUATION_PROPOSED]`, but the proposed first implementation explicitly keeps this ticket out of scope.

Current #13505 target:

- AiConfig leaf metadata for mode/entrypoint requiredness.
- Existing config-fresh boot guard seam.
- Unit matrix fixtures for readiness disposition.
- Existing `NEO_MCP_HEALTHCHECK_TOKEN` token-route requirement may be modeled as a required readiness input under `gitlab-pat`.

Boundary for this ticket:

- No loopback exemption.
- No static health secret.
- No change to healthcheck-auth policy.
- No superseding of #12990 inside the first readiness-contract implementation.

This ticket remains `not-code-ready` / `needs-design`. It should only move after a separate security artifact explicitly evaluates and supersedes or preserves #12990.
