...
updatedAt: '2026-04-23T09:12:15Z'
...
Surfaced empirically during the 2026-04-22 A2A handshake validation between `@neo-claude-opus` and `@neo-gemini-pro` (sessions `d907e342-...` through `7ee23599-...`). Opus sent the first A2A message as broadcast (`to: 'AGENT:*'`) because pre-`#10174` neither agent held a `CAN_REPLY_TO` edge toward the other (no way to bootstrap DM trust). Gemini received the broadcast, attempted a direct-DM reply via `add_message({to: 'AGENT:@neo-claude-opus', ...})` once `#10174` merged — and was **rejected with `Unauthorized: Cannot send to AGENT:@neo-claude-opus`** despite having visibly received Opus's message.
...
- 2026-04-22T15:39:04Z @neo-claude-opus added the `bug` label
- 2026-04-22T15:39:05Z @neo-claude-opus added the `ai` label
- 2026-04-22T15:39:05Z @neo-claude-opus added the `architecture` label
- 2026-04-22T15:39:52Z @neo-claude-opus added parent issue #10139
- 2026-04-23T08:52:17Z @neo-claude-opus assigned to @neo-claude-opus
...
- 2026-04-23T09:09:45Z @tobiu referenced in commit `ebb122d` - "fix(mcp): extend reachable-counterparty to broadcast recipients (#10179) (#10220)

Pre-fix, MailboxService.addMessage's reachable-counterparty iteration
only matched SENT_TO edges whose target equaled the caller directly.
Broadcast recipients (SENT_TO → AGENT:*) therefore held no substrate
signal for DM-reply access, breaking the first-message bootstrap
pattern empirically during the 2026-04-22 Opus↔Gemini handshake
documented on PR #10177.

Extends the inner-loop guard to also match `edge.target === 'AGENT:*'`,
so a message via broadcast whose SENT_BY is the intended DM recipient
now grants reply access. Accepted trade-off: any broadcaster becomes
DM-reachable by every authenticated recipient; rate-limit mitigation
deferred until the spam surface materializes at swarm scale.

Regression coverage under production-convention fixtures:
- broadcast recipient CAN DM-reply to broadcaster without grant
- unrelated broadcast does NOT transitively grant access to third parties
- existing `Reachable Counterparty exception permits replies without
  explicit grant` continues to pass unchanged (20/20 green)

MemoryCoreMcpAuth.md §Mailbox A2A Integration updated — the Reachable
Counterparty Exception clause now documents broadcast-receipt trust
lift explicitly, closing the impl-vs-doc divergence #10179 called out.

Co-authored-by: tobiu <tobiasuhlig78@gmail.com>"
- 2026-04-23T09:09:46Z @tobiu closed this issue
...
- 2026-05-09T16:39:30Z @neo-gpt cross-referenced by #11029
