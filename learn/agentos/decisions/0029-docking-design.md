...
> Architectural Decision Record for the design tier **above** the landed `neo.dock.zone.v2` model contract: the multi-window layout model and its SharedWorker seam, single-workspace named layouts plus keyed multi-workspace topologies, cross-window drag as semantic operations (`transferItem`), grouped drag (`moveNode` / `transferNode`) and tab overflow, the core-lift disposition, the container contract embedded product surfaces consume, the auto-hide UI contract, and — per the §2.8 amendment (2026-07-16) — the multi-window choreography contracts: the gesture claim protocol, the gesture outcome machine, and vessel lifecycle/admission.

| Attribute | Value |
|---|---|
| **Status** | Accepted — 2026-07-02 (#14423; PR #14425 merged to `dev`). **Re-homed** in the same PR from `learn/agentos/HarnessDockingDesign.md` (contract-doc tier) to decision-record tier after the ADR-0005 `ADR_REQUIRED` audit (operator-flagged, review cycle 3) — see §1 Context for why the authority belongs here. **Renamed** 2026-08-21 from `0029-harness-docking-design.md` (#17503; §2.9): the subsystem is a generic Body capability — the harness misnomer is retired, persisted schema strings stay frozen. **Amended** 2026-08-22 (`#17541`; §2.1): the normative workspace host becomes the engine class `Neo.dashboard.dock.Workspace`. **Amended** 2026-08-31 (`#17969`; §2.7 state table): the *detached* row's `autoHidden` mutual-exclusion clause is retired as an error — detachment and auto-hide are orthogonal, and the exclusion holds structurally in the projection walk rather than as a commit rule. **Amended** 2026-09-04 (`#18308`; §§2.1/2.2/2.8.4/2.9): positional topology state is removed from `layout.v1`; keyed topology records and collections plus `topologyShape.v2` become the only multi-workspace wire. **Amended** 2026-09-05 (`#18284`; §§2.3/2.8.1): the cross-window drag registry gains a commit-authority axis — the logical topology Group of the window a surface renders in — kept separate from `sortGroup`; candidates are admitted by ownership before hit-test and claim, claims are raised among same-ownership candidates only, and the target's commit re-checks the axis. **Amended** 2026-09-09 (`#18528`; §§2.3/2.6): the catalog's `componentRef` is retired in favour of the engine's own `reference` vocabulary. The item key is the lookup identity and lowering derives no field from it; a record names a `reference` only to override that, and the projection stamps the resulting key onto the pane so `data-ref` and `getReference()` reach a docked surface for the first time. The field is removed from the allowlist rather than tolerated, so a document still carrying it fails validation loudly. ⚠️ That change left the wire string at `neo.dock.zone.v1`, which was the defect the next amendment repairs — corrected here rather than left standing. **Amended** 2026-09-09 (`#18537`; §2.1): the zone envelope becomes **`neo.dock.zone.v2`**, and `WorkspaceDocument.RETIRED_SCHEMAS` names its predecessor so a stored document at the old tag is recognised as **old** rather than reported as malformed. The allowlist rejects rather than drops, so a field-set change already made every earlier document unreadable; moving the tag only makes that legible. This is **not** backward compatibility — #18308's *"Zero backwards compatibility is explicit"* and its out-of-scope migration reader both stand — it is #18308's other trap, *"no silent redefinition"*, applied one envelope over. `neo.dock.topologyShape.v2` is the precedent: a dock tag bumped when its shape changed and enforced on read. |
...
```json
{
  ...
  "workspaces": {
    "main": {"schema": "neo.dock.zone.v2"},
    "popup:detail": {"schema": "neo.dock.zone.v2"}
  },
  "placementHints": {
    "popup:detail": {
      "dx": 240,
...
- `workspaces` is a record keyed by the holder's registered `workspaceKey`; each value is a normalized
  `neo.dock.zone.v2` document. No nested key field duplicates the map identity.
...
**Commit authority (optional member — amendment 2026-09-05, #18284):**

| Member | Kind | Obligation |
|---|---|---|
| `ownershipId` | property, optional | The commit authority this surface belongs to: the logical topology Group of the window it renders in (`Neo.manager.Transaction`), read at claim time so a host that learns its Group after construction is covered. Distinct from `sortGroup`, which answers protocol compatibility; the two are never concatenated. A surface that declares the member as `null` is participating but unresolved and is never a candidate. A surface that does not declare it is outside the Group world and remains a candidate only for a source that declares none. |

**Candidate admission by ownership (binding).** Before the claim pass, the hit-test, the preview and any embodiment, the coordinator removes from the source's `sortGroup` candidates every target whose ownership does not match the source's: declared ids must be equal; a declared `null` matches nothing; an undeclared target matches only an undeclared source. The filter compares two opaque strings the surfaces declared — the coordinator stays dock-blind, and this section's binding invariant holds. On the dock path the payload carries the source's ownership (`dockSourceOwnershipId` beside `dockSourceWorkspaceId`), and the target's commit re-checks it: an equal bare workspace identity across two Groups never means local.

Why absence is two-tiered rather than uniformly closed: `Neo.draggable.dashboard.SortZone` registers with the coordinator without any Group (`apps/colors` through `dashboard/Container.mjs`, `examples/dashboard/base/MainContainer.mjs` on the same path). A rule that cannot tell *not participating* from *failed to resolve* either disables those consumers' cross-window drag or admits the cross-root commit this amendment exists to stop, while reporting the same thing either way. This amendment fires the §1 escalation boundary by design (target resolution changes) and lands before the coordinator change that implements it.
...
|---|---|
| A keyed `panes` component config — the key is the lookup identity — or a `reference` the record names instead, for an advanced resolver keyed on its own vocabulary | Resolution through normal Container/Card creation, with the projected pane carrying that key as its `reference` so `getReference()`, the view-controller path and `data-ref` all reach it; live declared instances are **moved / re-parented, never destroyed** during ordinary moves and rail transitions — conditioned recreate exception below; explicit close ends the instance lifetime |
...
|---|---|
| Click a rail tab | Transient **reveal**: the pane renders as an **overlay** anchored to its edge — left/right rails reveal full-height, top/bottom rails full-width. The overlay's free dimension uses its owning edge descriptor's committed `extent`; only a never-sized edge uses the workspace fallback (default `0.25`, configurable). Overlay, never push: revealing MUST NOT re-layout the committed projection. Focus moves into the pane. Entry motion (amended 2026-09-02): the overlay root fades in where it settles and never overlaps its strip; its content slides in from the strip side as one panel — both children travel the root's own extent (container-query lengths against the root, never a percentage of their own boxes) — clipped by the root, on `--dock-transition-duration-reveal` (the panel tier of the motion vocabulary), collapsed under reduced motion at the token layer. |
...
- **Claims among same-ownership candidates only (amendment 2026-09-05, #18284).** The claim pass runs over the ownership-admitted candidates (§2.3), so a stable identity is unique within one gesture's claim set, and a target of another Group neither claims nor releases a claim in it; two roots with equal `stableTargetId`s never meet in one arbiter. The arbiter itself stays blind to ownership — the coordinator's admission is the scope.
...
| Concept | Identity |
|---|---|
| committed dock document | `neo.dock.zone.v2` |
