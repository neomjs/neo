...
updatedAt: '2026-08-22T01:20:07Z'
...
| `--json` receipt | same reconciliation result | stable SHA-bound machine envelope consumed by the paired-gate and move leaves | no partial-success envelope; failures remain typed and non-zero | exported typedef/JSDoc | schema assertions + deterministic ordering |
...
| plane-opener rows | `planePlacementCensus.mjs` | consume its exported census; resolve its three current unclassified rows in the extraction registry | never fork the opener detector or copy its count | sibling citation | composition fixture |
