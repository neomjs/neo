...
class VDomUpdate extends Collection {
     ...
     *
     * What was wrong is that only the payload learned about it. A sibling writing in the same window
     * asks {@link Neo.mixin.VdomLifecycle#isParentUpdating}, which reads the registry, is told the
     * parent is still scoped to itself, and opens its own flight. The parent then collects at -1 and
     * its dense tree carries that sibling's subtree too: one subtree in two independent batches,
     * applied twice with identical element ids.
     *
