...
updatedAt: '2026-07-17T00:06:19Z'
closedAt: null
mergedAt: null
...
- The next data-sync run realigns the 2,015 and restores the 27; the pipeline's diff shows exactly those entries changing plus 27 re-rendered artifacts.
- A subsequent run writes **nothing** and makes **no** network call for duplicates — both passes are idempotent, so the generated-content diff stays empty.
- `npm run ai:check-substrate-size`-style verdict for the corpus: `validateContentIntegrity` reports `ok: true` once the pipeline has run. Until then it reports `FAIL` with counts, which is the honest state rather than a silent one.
