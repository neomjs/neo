...
/**
 * The lazily resolved pane the repair arm counts. Its construction tally is the witness, not its
 * rendering: the contract is ONE instance per projected item, so a repair pass that re-resolves what
 * the failed pass already materialized shows up here as two.
 */
class LazyRepairPane extends Container {
    /**
     * Constructions of this class, reset by each arm that reads it.
    ...
    static config = {
        className: 'Test.Unit.Dashboard.DockWorkspace.LazyRepairPane'
    }

    construct(config) {
        super.construct(config);
        LazyRepairPane.instances++
    }
}

Neo.setupClass(LazyRepairPane);

/**
 * A REAL repair, unlike {@link RepairWitnessWorkspace}: `refreshDockWorkspace` is left alone, so the
 * re-projection `onDockProjectionFailed` schedules actually runs. That is the only way a second
 * construction can appear at all — a stubbed repair records that one was scheduled and can never
 * observe what it then does.
 *
 * One item resolves lazily; the rest keep the plain config path, so the arm reads a single pane's
 * tally rather than a whole projection's.
 */
class LazyRepairWorkspace extends PlainWorkspace {
    static config = {
        className: 'Test.Unit.Dashboard.DockWorkspace.LazyRepairWorkspace'
    }

    resolvePane(itemId, item) {
        return itemId === 'terminal'
            ? {module: () => Promise.resolve({default: LazyRepairPane})}
            : super.resolvePane(itemId, item)
    }
}

Neo.setupClass(LazyRepairWorkspace);
...
test.describe('Neo.dashboard.dock.Workspace', () => {
    ...
    test.describe('#18275 the repair pass does not re-construct what the failed pass already built', () => {
        /**
         * Rejects one update flight of the NEXT projection, then lets the real repair run.
         *
         * The rejection has to land after the reconcile loop inserted its items — both typed-failure
         * sites in the reconciler wrap `host.promiseUpdate()`, which is the flight that follows the
         * inserts. Rejecting earlier fails the pass before anything is constructed and the arm
         * witnesses nothing.
         * @param {Object} workspace
         * @param {Number} rejectOnCall
         * @returns {Promise<Number>} the number of flights the pass actually took
         */
        const failOneFlight = async (workspace, rejectOnCall) => {
            const host            = workspace.getDockHost(),
                  originalFlight  = host.promiseUpdate.bind(host),
                  originalProject = DockProjectionReconciler.reconcileProjection;

            let calls = 0;

            // The stub is installed around the RECONCILER call, not around the whole refresh: the
            // workspace makes its own `promiseUpdate` flights outside the reconciler (the unmounted
            // host sync, for one), and a raw rejection there is never typed `isDockProjectionFailure`,
            // so it escapes as a plain error instead of routing to the repair this arm exists to test.
            DockProjectionReconciler.reconcileProjection = function (projection) {
                host.promiseUpdate = () => {
                    calls++;
                ...
                return originalProject.call(this, projection).finally(() => {
                    host.promiseUpdate = originalFlight
                })
            };

            try {
                await workspace.refreshDockWorkspace(null, createLazyActiveDocument(), {});
                await workspace.refreshPromise
            } finally {
                DockProjectionReconciler.reconcileProjection = originalProject;
                host.promiseUpdate                          = originalFlight
            }
        ...
        /**
         * `terminal` is the ACTIVE card of its node here, unlike {@link createDocument}. A parked
         * lazy item is never loaded at all — correct behaviour, and useless as a witness: the arm
         * needs the item materialized before the failure so a repair's construction is a duplicate
         * rather than a first load.
         * @returns {Object}
         */
        const createLazyActiveDocument = () => {
            const document = createDocument();
        ...
        test('one rejected flight leaves ONE construction of the lazily resolved pane', async () => {
            LazyRepairPane.instances = 0;

            // The item must be INTRODUCED by the failing pass, not pre-exist it. The reconciler seeds
            // `liveItems` from the current tabs, so an item already on screen is found there and never
            // re-resolved — a pre-existing item makes this arm green without witnessing anything. The
            // observed CI shape is the other one: the failing pass inserts the item, the recovery
            // retires the staged shell along with it, and the repair pass resolves it from scratch.
            workspace = Neo.create(LazyRepairWorkspace, {dockModel: createDocument()});

            await workspace.refreshDockWorkspace(null, createDocument(), {});
            await workspace.refreshPromise;

            expect(LazyRepairPane.instances, 'parked behind an active sibling, nothing loaded yet').toBe(0);

            await failOneFlight(workspace, 3);

            expect(LazyRepairPane.instances, 'the repair re-used what the failed pass built').toBe(1)
        })
    });
