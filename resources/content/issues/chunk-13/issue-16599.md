...
updatedAt: '2026-08-07T05:54:59Z'
...
## The writer fence — required for the refusal to mean anything (design settled 2026-08-07)
...
### Acceptance criteria for the fence

- [ ] The merge import holds a lease across first scan read → last upsert, and refuses rather than proceeding if it cannot obtain one.
- [ ] `ingest_source_files` acquires the same lease at operation level and returns a structured `retryable` refusal on `held`, with lease owner and expiry — never a silent block and never an internal wait.
- [ ] A `viaMcp: false` bulk path already under the heavy lease **inherits** it; a spec asserts no second acquisition occurs.
- [ ] A test proves exclusion by observation: a concurrent writer attempting an upsert during a merge's protected span is refused, and the merge's scan verdict is still accurate at its last write.
- [ ] **The negative control:** with no contention, both paths behave exactly as today — the fence must not become a serialisation tax on ordinary ingestion.

**Until this lands, the detector's guarantee is bounded to "no divergence at scan time."** PR #16607 states that bound in its body; prose narrowing does not discharge the missing concurrency guarantee, which is why the reviewer's `CHANGES_REQUESTED` stays the gate rather than being answered by wording.
