...
updatedAt: '2026-08-16T19:22:08Z'
...
- 2026-08-16T19:22:08Z @tobiu referenced in commit `126c4ed` - "fix(ai): the wake adapter types into the process it verified, not a fresh frontmost read (#17231) (#17233)

Three keystroke blocks each opened with `first application process whose
frontmost is true` -- a second, independent read taken AFTER
`assertTargetFrontmost` had already approved the target. The check proved the
target held focus; the next statement discarded that result and asked the system
again, so every keystroke went wherever focus had drifted to by then. With the
0.2s-1.0s delays inside those sequences, focus has hundreds of milliseconds to
move WHILE the payload is being typed.

That is why a misrouted wake could report success. Losing focus BEFORE a check
aborts loudly -- the `Target app lost frontmost status` errors in the receiver
log are that guard working. Losing it AFTER a check raised nothing at all: one
seat's wake was typed into another seat's window, and on the restore path the
OPERATOR'S OWN recovered draft was pasted into whichever window had taken focus.
The check was never the weak part; discarding its result one line later was.

The target is now resolved once by identity -- pid, then bundle id, then name,
the same precedence the assertion uses -- and every block tells that process.

Observed end to end before the fix: a wake addressed to one seat arrived in
another's session while the operator was typing in the second window, confirmed
independently by the operator and by the receiving seat's transcript.

The arm is folded into the existing UI-delivery test rather than standalone:
`dispatchLocalWake` serializes on module-level state, so a second delivery
perturbs specs sharing the worker. A standalone version broke an unrelated
stale-replay arm -- an isolation defect introduced by a test for an isolation
defect, caught by running the same two-file scope against the base."
- 2026-08-16T19:22:08Z @tobiu closed this issue
