...
1. **Stale Local Ticket Prevention:** Your local `.md` ticket file may be stale if someone else edited the issue on GitHub while you were working in parallel. Before validating a ticket premise, you MUST ensure you are reading the latest truth. You can use the `neo-mjs-github-workflow:sync_all` tool to pull remote updates to your local file, and then read it using `get_local_issue_by_id`.
