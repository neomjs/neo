...
updatedAt: '2026-08-24T07:50:03Z'
...
- 2026-07-30T23:30:08Z @neo-opus-vega cross-referenced by PR #16195
### @neo-gpt-emmy - 2026-08-24T05:41:03Z

## [INTAKE] `valid-as-written` — live partial-response evidence is still discarded

V-B-A at current `origin/dev` confirms the ticket's two foundational failures remain live:

- `list_pull_requests({believedOpen:[99999999]})` returned `unverifiable: [{number:99999999, reason:'not-found-or-inaccessible'}]` at `2026-08-24T05:40:13.852Z`.
- `PullRequestService.listPullRequests()` still unwraps `response.data` and drops `response.errors`; `projectBelievedOpen()` still trusts `row.state` without requiring `row.number === submittedNumber`.
- The OpenAPI `PullRequestBelief.unverifiable[].reason` enum still contains only `not-found-or-inaccessible`.

**Age / successor risk:** created `2026-07-30T21:44:37Z`, updated `2026-07-30T21:55:32Z`; `pre-stale` under the repository's 90-day stale / 14-day close workflow, with neither `stale` nor `no auto close`. The self-authored drift probe intersected three declared surfaces, so the full intake ran. Merged PR #16195 is the adjacent schema-constraint/compiler repair for #16194; current source proves it did not deliver this error classifier, and no merged/open PR closes #16196.

**Contract / duplication:** every Contract Ledger anchor still exists with the prescribed old behavior; the parent #16136 relationship is live and is not `epic`-labeled. Ticket-focused KB retrieval found #16196 but no competing owner; three Memory Core framings were a clear miss; live issue/PR search found no duplicate or active implementation.

**ROI:** positive. The existing one-request path already returns typed partial errors. The bounded repair is one pure alias/error classifier, exact-number guards, the existing schema enum, and focused unit/emitted-schema controls—no new request, tool, subsystem, or ADR change.

**Verdict:** `valid-as-written`; implementation may proceed.

- 2026-08-24T05:56:47Z @neo-gpt-emmy cross-referenced by PR #17678
- 2026-08-24T07:24:39Z @neo-gpt-emmy referenced in commit `dc48af0` - "fix(github): classify believed-open lookup evidence (#16196)"
- 2026-08-24T07:50:03Z @tobiu referenced in commit `0260837` - "fix(github): classify believed-open lookup evidence (#16196) (#17678)"
- 2026-08-24T07:50:03Z @tobiu closed this issue
