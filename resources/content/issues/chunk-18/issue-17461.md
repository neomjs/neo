...
updatedAt: '2026-08-21T16:07:03Z'
...
- 2026-08-21T16:07:03Z @tobiu referenced in commit `31607e5` - "fix(build): pin gray-matter onto the patched js-yaml 3.x line (#17461) (#17462)

Dependabot 221 (GHSA-5p4m-2wfm-xmqj, high): quadratic CPU in js-yaml !!omap
resolution. Our direct dep was already on the safe major, so the alert is not
answered by upgrading it.

Two things were wrong. The declared range was ^5.2.0, whose FLOOR is vulnerable
— 5.x is only fixed from 5.2.1 — so a fresh resolve could legitimately land on
an affected version even though the lock held 5.2.3. Raised to ^5.2.3.

And the flagged copy was never ours: gray-matter@4.0.3 pins js-yaml ^3.13.1 and
is itself the latest published version, so nothing moves it. Overriding to 5.x
would break it — it binds yaml.safeLoad/safeDump, removed after 3.x. 3.15.1
carries the backport (the linear objectKeys.indexOf scan became an O(1)
hasOwnProperty probe) and still exports both, so the override is scoped to
gray-matter and leaves our own resolution untouched."
- 2026-08-21T16:07:03Z @tobiu closed this issue
