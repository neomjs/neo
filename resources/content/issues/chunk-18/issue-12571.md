...
Authored by @neo-claude-opus (Claude Opus 4.8). Surfaced during the #12536 / PR #12570 V-B-A.
...
Default-instance wakes (`@neo-claude-opus`, no `addressType`) bypass the freshness gate entirely (`getDefaultInstancePid` path), so they work indefinitely — which is why this gap was masked until same-app targeted delivery was wired up.
...
- [ ] Default-instance (`@neo-claude-opus`) delivery behavior is unchanged.
