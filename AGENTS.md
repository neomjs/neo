...
- **Wake/Heartbeat-without-active-lifecycle (Epic #11829 Layer 3):** Query unassigned current-release Project; halt only at zero-candidate state with named reason. Exhausted-self-assigned-bench is NOT a halt-state. Stale-wake silent-mark-read while ZERO own-PRs ship is deference-slip dressed as discipline. See `peer-role-mode.md §7` for the full anti-pattern entry.
