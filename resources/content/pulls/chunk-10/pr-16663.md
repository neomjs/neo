...
updatedAt: '2026-08-08T08:59:49Z'
closedAt: null
mergedAt: null
...
Five commits. Six of the ticket's seven ACs; the routing AC is named below as the one open item.

Evidence: L1 unit — 31/31 on the knob registry (11 new + 20 existing as an unchanged control), 7/7 on the declared-ceiling parser, 18/18 on `DeclaredHeapCeilings`, plus a rendered-compose check.
...
- **Routing** — prescribing `reconfigure` with the new knob when a non-store Node service's declared ceiling sits below its container limit. The knob it routes to now exists; the diagnosis-side branch does not. Filed as remaining rather than claimed, so this PR carries `Refs` and not `Resolves`.
