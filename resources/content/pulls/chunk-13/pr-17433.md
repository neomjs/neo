...
updatedAt: '2026-08-21T11:39:49Z'
closedAt: null
mergedAt: null
...
**1839 memory-core specs green at `3c26bcb9c3`**, three consecutive clean runs. One intermittent
...
| batch admission ceiling = full budget | `Expected: <= 3 Received: 4` — 372ms |
...
**The non-deadlocking instrument.** The exact-peak-4 probe releases on `open >= targetPeak || requestInputs.length === expectedSpans`, so the single-worker mutant **completes and fails by assertion** rather than timing out at the probe deadline. Round 1 was right that a timeout has many causes and an assertion has one; the earlier body paragraph conceding an "ugly red" is obsolete and has been removed rather than left standing.
