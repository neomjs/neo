...
- **Stale-wake silent-mark-read pattern:** Marking heartbeats / stale-event wakes as "no action" while ZERO own-PRs ship is deference-slip dressed as discipline. Trigger: query unassigned current-release backlog OR confirm zero-candidate halt with named reason. Exhausted-self-assigned-bench is NOT a halt-state. Treat each heartbeat as the next-lifecycle-event prompt rather than passive notice.
