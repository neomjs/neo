...
7. **No tracked file modification without a self-assigned ticket.** Self-assign + broadcast `[lane-claim]` to `AGENT:*` before any git-tracked edit. Enforcement: `pull-request-workflow.md §1.2`, `ticket-create-workflow.md §10`. *(Exception: Maintainer Polish Fast Path for mechanical/metadata fixes when the Review Compression Circuit Breaker fires, per `pull-request-workflow.md §10`)*.
