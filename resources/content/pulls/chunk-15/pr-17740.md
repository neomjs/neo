...
| AC-9 | #14829 semantics survive — data-only capture, provenance, replay, replay-mark | Neural-link specs green; `InstanceService`'s call sites updated for the new failure taxonomy; `nlRelocationComposition.spec.mjs` round-trips save → read → replay-mark through a real graph. **Fresh-session replay is L3 and runs on the nightly e2e lane** — see the Evidence line |
