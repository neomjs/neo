...
updatedAt: '2026-08-10T08:35:06Z'
...
- [x] A declared leaf names the cap; read at the use site on every admission. — `ai/configBase.mjs` ollama group; `#tryAcquireOllamaEmbeddingSlot` reads `aiConfig.ollama.maxInFlightEmbeddings` per call rather than capturing it once, so a raised cap can wake waiters it now has room for.
