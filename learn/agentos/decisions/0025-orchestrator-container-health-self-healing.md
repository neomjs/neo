...
| **Amended** | 2026-06-26 (#14191, full-self-heal) — the **escalate-with-diagnosis (page)** terminal (the §2.3 envelope, the alarm-only line, the §2.4 diagnose-map's `config-drift` / `data-integrity` routes, AC-7) is superseded by **record-with-diagnosis (durable async-audit to the heal-event ledger `healEventLedgerStore`, #14163) + autonomous action**: an operatorless cloud has no human to page (@tobiu's directive — epic #14039 / #14132). `data-integrity` routing is now governed by ADR-0027 (autonomous data-recovery). The never-loop discipline, the persisted anti-thrash envelope, and the two-worlds config-lifecycle-only safety (AC-5) are **unchanged**. **Amended 2026-08-03 (#16462, restart-churn detect signal)** — §2.4 gains a **restart-churn** detect signal, the first signal in the model that is a property *across* container incarnations rather than a point-in-time observation. Additive: non-authoritative, `record`-terminal, no new recovery class, no new privilege, no action-set change; AC-1 through AC-7 unchanged. |
...
- **Detect signals, restart churn** *(amendment 2026-08-03, #16462 — the detect model extends from point-in-time observation into observation across container generations):* every signal above is **instantaneous** — a state, a sample, a probe verdict. That is a blind spot with a precise shape: **a container can restart indefinitely while every point-in-time signal is honestly green**, because each incarnation genuinely is alive. Observed on the maintainer plane at **977 restarts over ~30 hours**, reporting `running` + `healthy` with a 28-second-old process and no diagnostic facts. No probe of any kind closes this, whatever it probes — restart churn is a property *across* incarnations, and a healthcheck by definition evaluates the current one. The signal is therefore a counter read against a **persisted, generation-keyed baseline**: restarts above a threshold within a window, on one container generation, minus restarts the caller itself initiated. Generation-keying is what makes a recreate a new baseline rather than a fault (and stops a stale baseline making a fresh container look sick); subtracting planned restarts is what stops a deploy raising it, because an alarm that fires on every deploy is disabled within a week and leaves the blind spot plus a dead alarm. **The fact is non-authoritative and its action class is `record`** — per the coverage-drift precedent below, and because the only action available is `restart`, which a container's own restart history has already falsified. It introduces no new recovery class (`ambiguous`, the established record-never-auto-restart class), no new privilege, and no action-set change.
