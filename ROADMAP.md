...
**⚠️ The gating core decision — not yet made: what the harness *is*.** The critical path below is the *machinery*; it does not yet define the **product** — which views, widgets, and navigation the harness provides, *what's where* (the view structure), what a human user can do, and what an agent can do. [Discussion #13436](https://github.com/orgs/neomjs/discussions/13436) opened this but did not resolve it. Until a **top-level harness-UI-definition epic** lands that definition, the nodes below risk random work — docking with no definition-of-done, or scaffolding like the EvidencePane that no final product keeps. **This is the next core decision; the roadmap updates again once it is delivered.**
...
| **Cockpit UX/IA** — first-open → set up agent identities (Accounts) · run the fleet (Session) → chat → live pane | cockpit-UX epic (graduating from [Discussion #13436](https://github.com/orgs/neomjs/discussions/13436)) | converging |
| **Beautiful design** — Claude-Desktop-class visual polish (the install-earning floor); from the current PoC styling to a designed product | cockpit-UX epic — Design lane ([#13436](https://github.com/orgs/neomjs/discussions/13436) Fork D) | not done |
