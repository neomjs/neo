...
- **Antigravity 2.x:** workspace rules and skills use `.agents/rules/` and `.agents/skills/`; the [CLI migration layer](https://antigravity.google/docs/gcli-migration) also recognizes existing root `AGENTS.md` / `GEMINI.md` instructions. Neo's canonical per-turn substrate remains root `AGENTS.md`. MCP definitions are a separate concern: choose either the global `~/.gemini/config/mcp_config.json` authority or the workspace `.agents/mcp_config.json` authority from the [Antigravity MCP contract](https://antigravity.google/docs/mcp), and do not duplicate the same server across both.
- **Gemini CLI:** consumer free / Google AI Pro / Ultra service [transitioned to Antigravity CLI on 2026-06-18](https://developers.googleblog.com/an-important-update-transitioning-gemini-cli-to-antigravity-cli/). Enterprise and explicit API-key use remain distinct supported modes; they do not make a repository workspace settings template an Antigravity authority.
...
It copies `ai/config.mjs` plus the per-server `config.mjs` overlays discovered by `ai/scripts/setup/initServerConfigs.mjs` from the main checkout (resolved via `git worktree list --porcelain`) and — with `--link-data` — creates only the approved granular data links under `.neo-ai-data/` plus gitignored single-file handoffs such as `resources/content/sandman_handoff.md`. It does **not** replace the `.neo-ai-data/` parent. Process-control dirs (`orchestrator-daemon/`, `embed-daemon/`) stay clone-local by design; canonical orchestrator state/logs are exposed through a read-only alias at `.neo-ai-data/orchestrator-daemon-canonical/` without sharing the daemon PID directory. Independent sibling clones (Codex / Antigravity style) cannot infer the canonical checkout from `git worktree list`; pass `--canonical-root <canonical-checkout>` or set `NEO_AI_CANONICAL_ROOT`.

The CLI is the full interactive bootstrap: after config/data hydration it installs missing dependencies as needed and always runs `build-all`. There are no `--install` or `--build-all` switches. Code that already owns its install/build lifecycle can import `hydrateCurrentWorktree({mainCheckout, projectRoot})` for the build-free hydration seam; no separate build-free CLI mode exists.

**Why `--link-data` matters (per #10224):** without it, each worktree gets its own empty `.neo-ai-data/sqlite/memory-core-graph.sqlite`, which means AgentIdentity nodes seeded in the main checkout are invisible to the worktree's MCP server. `bindAgentIdentity('neo-opus-ada')` returns null, the mailbox throws `"no agent identity context bound"`, and A2A handshakes silently fail — the #10184 symptom from a different root cause than cache coherence. The granular links unify approved Memory Core substrates (SQLite + Chroma + concepts + backups) so ADR 0001's "one SQLite file shared across N processes" assumption holds across worktrees.
...
- **If the healthcheck is successful:** The Memory Core is active.
    ...
    - **Establish Context (Mandatory):** You **MUST** call these tools at boot:
        ...
        6. **Seat Memory-Layer Anchor Check (seats with a generated memory layer):** verify your harness's load proof before identity-bearing artifacts — Kimi: the `<seat-memory-layer source="…" trigger="session-boot|post-compact-reload">` marker plus the `MEMORY.md` / `identity.md` sections; OpenCode: the same two files' content via `opencode.jsonc → instructions` (no marker wrapper exists on that path). Proof absent ⇒ the layer is NOT loaded — diagnose per harness (route: the context-recovery identity quarantine). `turnPresence.fresh` proves a sibling hook, never this layer's load. Retirement: a native harness memory slot retires the mechanism and this check.
