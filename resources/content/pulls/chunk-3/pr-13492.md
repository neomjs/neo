...
*   **Inputs Read Before Patch:** #13474 (body + Contract Ledger + ACs), the existing `ai/mcp/server/shared/logger.mjs` (`createLogger` rotation), the three MCP server config templates (KB/MC/NL), and the tenant live-deploy compose (the MCP servers run there → cloud-relevance).
