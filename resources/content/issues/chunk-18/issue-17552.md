...
updatedAt: '2026-08-22T15:58:30Z'
...
4. **Reduced motion:** the inserted rule gains a companion `@media (prefers-reduced-motion: reduce) { #<ownerId> .neo-list-item { transition: none } }`, sharing the insert/delete lifecycle.
...
| `minItemWidth_` (NEW plugin config) | this ticket | fluid mode: columns + per-item width derived from the owner rect | null → fixed mode | docblock | unit witness incl. the measured fixture (903px region, margin 9, minItemWidth 420 → 2 × 447px) |
...
| transition CSS rule | `updateTransitionDetails` | + reduced-motion companion rule | n/a | inline | rule-payload witness |
...
- [ ] Fluid-mode math witnessed against the measured cockpit fixture (903 / 9 / 420 → 2 × 447) plus a 1-col floor case and a 3-col case.
...
- [ ] The reduced-motion companion rule inserts and deletes with the base rule (CssUtil payload witness).
