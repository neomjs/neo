...
| `close_window` | Close an owner-granted topology-known popup through its private native route and observe terminal disappearance. | `RuntimeService` -> window operation bridge. | `write-locked` | Runtime window mutation; not transaction-captured. | Unsupported without an owner grant; otherwise verified terminal result or `{error}`. | Trusted controller/e2e only; never bypass a product semantic return/disposal contract. | Fixture wrapper: `closeWindow`. |
