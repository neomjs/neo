...
updatedAt: '2026-08-09T16:09:33Z'
...
- [ ] `shared-heap-observation-data` is declared in the compose `volumes:` block and mounted at `AiConfig.heapObservation.dir` in `kb-server`, `mc-server` and `orchestrator`.
- [ ] The writer mounts are read-write and the orchestrator mount is `:ro`.
- [ ] A guard asserts the invariant from the compose source, with the writer roster **derived** from `getHeapObservationServiceKey()` declarations, and fails closed if that derivation returns an empty roster.
...
- [ ] Sibling compose specs stay green (`ParityPlaneVolumeScoping`, `DeclaredHeapCeilings`, `daemon.spec` `#15759` sole-owner-volume, `FleetServerComposition`, `ChromaPersistPathContract`, `DeploymentTrustDefaults`, `HostEdgePosture`, `LaneEnablementSignal`, `DeploymentRuntimeAccessService`, `DeployPipelineRevisionPin`).
- [ ] **Evidence level: L2.** This ticket closes on the topology change and its guard. The live reading belongs to `#16763` AC-9 and is **deliberately not** this ticket's close condition — the plane must first be recreated with the new volume, which no implementation PR can supply.
