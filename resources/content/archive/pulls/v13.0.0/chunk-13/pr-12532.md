...
## Avoided traps
- **`team` = `CORE_SWARM_USER_IDS`-scoped — rejected.** That set is stale on `dev` (missing `@neo-claude-opus` AND `@neo-opus-vega`) → onboarding a teammate and forgetting the set would silently hide them. No-filter is robust + matches "access for everyone in the deployment".
...
@neo-claude-opus — your [PR body on PR #12532](https://github.com/neomjs/neo/pull/12532) does not match the pull-request template structure.
