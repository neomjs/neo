...
updatedAt: '2026-07-12T13:47:36Z'
closedAt: null
mergedAt: null
...
**DRAFT — cycle-2 in progress; head 346639ad2, full hosted CI green.** The wrap core + **both** consumed surfaces (`query_raw_memories` and `ask_knowledge_base`) are wired + unit-tested. Emmy's cycle-2 (review 4679826997) found real authorization/semantic gaps — most importantly that terminal-collection authorization does NOT authorize the intermediate PATH the walk crosses to reach it (an RLS defect). **Landed this cycle:** the KB type-predicate fix (bc6f66f86), the **intermediate-hop RLS Depth-Floor fix** (a717a830d — the security crux), and the **maxCandidates overflow fix** (346639ad2). One design fork awaits Emmy's call (**fork #1**: FILE-as-intermediate vs terminal — see RA4). The items under **Remaining Before Ready** keep this a draft — it cannot merge / false-green #14504 until met.
...
## Review RAs
- **RA1** session-scope re-authorization ✓ · **RA2** ⏳ mechanical-key alias narrowing done, but #14528 `canonicalConceptId`/`aliasOf` consumption still open · **RA3** complete-path per-hop provenance ✓ · **RA4** ⏳ **intermediate-hop RLS now fixed** (a717a830d — enrich-scoped `PUBLIC_TRAVERSABLE_LABELS` fail-closed allow-list `['CONCEPT','FILE']` blocks private-intermediate traversal; the shared #14474 probe keeps full reachability via `traversableLabels: null`) and **`maxCandidates` now counts rejected hydrations** (346639ad2 — bounds attempts, not just successes). Remaining: budget not yet request-global (hop budget resets per alias member), candidate labels filter only after raw traversal; **fork #1 for Emmy** — FILE-as-intermediate (preserved, per the existing depth-2 test) vs her stated FILE-terminal (`['CONCEPT']`) · **RA5** ⏳ Memory OpenAPI schema ✓ + KB type-predicate now reapplied (bc6f66f86), but KB OpenAPI has no `conceptWalk` surface, KB dedup/empty-flat behavior is still incorrect, and the direct-service specs are not tool-boundary MCP compliance fixtures · **RA6** tool-description tightened ✓.
...
## Test Evidence
- Graph spec (`conceptAnchoredRetrieval.spec.mjs`, 22): resolver + `enrichWithConceptWalk` — wrap-never-replace, no-resolve honest event, walk-merge, per-hop path at depth 2, degrade-by-omission, fail-closed gate, dedup, label-skip, `WALK_BUDGET`, `walkDurationMs`, `maxCandidates`/truncation, `traversableNodeLabels`, graph-throw-degrades-to-flat robustness (4977fbaf9), **intermediate-hop RLS: a FILE reachable ONLY via a private AGENT_MEMORY intermediate is NOT appended** (a717a830d — the reserved `test.fixme` un-skipped + green), and **maxCandidates counts rejected hydrations** (346639ad2 — a reject consumes ceiling budget). Probe spec (`conceptNeighborhoodProbe.spec.mjs`, 7) still green — the shared walk's full-reachability measurement is untouched. **All green + full hosted CI on 346639ad2 (unit, integration-unified, CodeQL, check, lint-pr-body).**
...
- KB surface (`SearchService.noModel.spec.mjs`): ask-level opt-in probe — `conceptWalk` ON threads the event through the envelope; OFF omits the key (byte-identical). **Green in hosted CI.** (The empty-collection test in that file needs a live Chroma `count()`, so it shows red ONLY in a local no-webServer run — stash-confirmed failing on baseline dev too, i.e. environmental, not introduced by this PR.) Plus the KB type-predicate regression (`QueryService.queryDocuments.spec.mjs`, bc6f66f86): `findDocBySource('…guide…','guide')` builds a where carrying the type clause — the opt-in walk never widens the caller's type scope.
...
- [x] **Opt-in probes** at both SERVICE surfaces (absent-param byte-identity + present-param enriched shape, b1aadfcbb) — but these are DIRECT-service probes, NOT tool-boundary MCP compliance fixtures.
- [x] **KB type-predicate** reapplied at walk hydration (bc6f66f86).
- [x] **Intermediate-hop RLS Depth-Floor** (the security crux, Emmy review 4679826997): fail-closed `PUBLIC_TRAVERSABLE_LABELS` allow-list (a717a830d); the reserved regression is un-skipped + green.
- [x] **`maxCandidates` counts rejected hydrations** (346639ad2) + **`file:`/`file-` dialect dedup** (ac528adaf).
- [ ] **Fork #1 (Emmy's call):** FILE-as-intermediate (preserved, per the existing depth-2 test) vs FILE-terminal (`['CONCEPT']`) for the traversal allow-list.
- [ ] **Cycle-2 bounded-correctness batch** (remaining, held for a stable-head follow-up pass): keep hydrated `metadata` internal to synthesis, KB empty-flat structural rescue, `GraphService.ready()`, request-global hop budget (currently resets per alias member), candidate labels filter before raw traversal.
- [ ] **Consume #14528** `canonicalConceptId`/`aliasOf` in `resolveConcepts`.
- [ ] **KB OpenAPI `conceptWalk` surface** (mirror Memory) + a tool-boundary MCP compliance fixture (not the direct-service seam).
- [ ] **Doc truth-sync:** #14504 Contract Ledger — retire the falsified "KB store lacking concept nodes" fallback → Native Edge Graph → FILE → KB-collection hydration.
