...
1. **Fetch Remote Truth:** Before validating a ticket premise, you MUST ensure you are reading the latest truth. You MUST use the `mcp_neo-mjs-github-workflow_get_conversation` tool to fetch the live issue body and comment thread directly from GitHub.
   ...
   - **Provisional-graduation pre-check:** `[PROVISIONAL_UNGRADUATED: D#N]` in the live body blocks assignment, claim, branch, and work-start. Re-poll the cited Discussion; only its completed quorum + graduation marker permits body promotion and removal of this marker.
...
3. **V-B-A Premise Check:** Treat ticket claims as hypotheses; run falsifiers (`ask_knowledge_base`, `grep_search`, `view_file`) against premise, architecture, and priority before acceptance.
