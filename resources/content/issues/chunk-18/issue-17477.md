...
updatedAt: '2026-08-21T14:54:42Z'
...
Resolve the three entrypoints the way Node does, rather than by joining a root:

```js
import {createRequire} from 'node:module';
const require = createRequire(import.meta.url);
// … try { require.resolve(pkg + '/' + entry) } catch { return false }
```

This is correct in the clone and the worktree alike, and it keeps the CI assertion meaningful instead of accidentally strict. The `rootDir` parameter can stay for the existing pure-by-injection specs that exercise both tiers, with resolution injected rather than path-joined.
...
- [ ] `--project=unit-brain` resolves and runs from a linked worktree that has no `node_modules` of its own, with the Brain tier installed in the main clone.
