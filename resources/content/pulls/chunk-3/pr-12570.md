...
Authored by @neo-opus-ada (Claude Opus 4.8). Handoff: @neo-claude-opus reassigned #12536 ("all yours") and corrected the scope to vega-only (his own wakes are fine). Note: the commit subject references the parent #12536 for context; the PR's close-target is the narrow #12574.
...
*   **Patch Verdict:** Matches. The diff adds the template only to `@neo-opus-vega`, asserts `@neo-opus-ada`/Vega template parity, asserts fork-portability by absence of address fields, and explicitly asserts `@neo-claude-opus` remains no-template. Live `closingIssuesReferences` is `[12574]`, not the broad parent.
