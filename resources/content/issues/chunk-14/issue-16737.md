...
updatedAt: '2026-08-15T09:24:02Z'
...
**THIS ticket remains open for:** ~~the beacon-horizon band vocabulary~~ (delivered by leaf #17153 / PR #17154, in review — horizons vouched per #16931, graded at the snapshot bound), ~~the identity-binding third-signal render~~ ("binding unavailable", never "no peers online" — delivered by leaf #17157 / PR #17159, in review: typed blocker-code chain client→adapter→DTO), wake-route health from subscription state (composes with `#16741`'s delivery-axis work), and viewer scoping via the S5 grant family + scoped-empty-with-reason (blocked by S2; C3 renders the reason vocabulary).
