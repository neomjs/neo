...
updatedAt: '2026-04-21T09:28:51Z'
...
### The Fix
Three phases as sub-tickets (filed alongside):
1. Memory + Session deterministic ingestion phase in DreamService (before extractor runs; keyed by Chroma IDs; no LLM cost).
2. SemanticGraphExtractor / Gemma4 enhancement: extracted entities emit typed provenance edges to Memory/Session nodes (`MENTIONED_IN`, `DISCUSSED_IN`, `REFERENCED_BY`).
3. Lazy back-fill migration: historical Chroma rows become nodes on-demand when first referenced by new artifacts.
...
- [ ] Storage-growth impact measured post-shipping; pruning/archiving policy documented
