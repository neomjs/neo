...
updatedAt: '2026-08-25T19:56:36Z'
...
- 2026-08-25T19:37:34Z @neo-opus-ada referenced in commit `c4ca5f4` - "test(kb): observe every whole-file read seam, not just the sync one (#17773)

@neo-gpt's RA on PR #17774: the arm watched only `readFileSync`, while #17773
names promise-based `readFile` as the same bad family. A partially refactored
path could open a stream — satisfying the positive control — and still slurp the
JSONL through `fsExtra.readFile` or `fsExtra.promises.readFile`, leaving "never
reads it whole" green with ERR_STRING_TOO_LONG back in the production path.

That is the exact defect class this arm exists to catch, arriving in the fix for
it: an assertion that cannot witness one of the paths its contract names.

All three seams are now recorded and all three restored in `finally`, including
the nested `promises.readFile`. Mutation-checked against the specific mixed shape
the review described — a path that opens a stream AND reads the file whole and
async now reds at the seam assertion; before this commit it passed.

15 consecutive runs clean; record-count and sidecar arithmetic untouched."
- 2026-08-25T19:56:36Z @tobiu referenced in commit `b552c8d` - "fix(test): assert the pack path streams, instead of sampling process RSS (#17773) (#17774)

* fix(test): assert the pack path streams, instead of sampling process RSS (#17773)

The peak-RSS arm could not catch the defect it cites, and reddened unrelated
PRs while failing to do so.

Measured against the real packer: the STREAMING path costs 4-17x the JSONL in
process RSS (transient fp16/batch buffers plus V8 heap growth, none of it
returned), while a whole-file read costs ~1.4-2.7x. The `< jsonlBytes * 4` bound
therefore sat above what the broken implementation retains and below what the
correct one does — no threshold on that instrument separates them. What it
actually sampled was process history: the same call reads ~7.4 MB in a cold
process and ~1.8 MB in a warm one, which is why its verdict depended on what ran
before it in the worker.

A stream read and a whole-file read are exactly distinguishable, so the arm now
asks that directly: the JSONL is opened through `createReadStream` and never
passed to `readFileSync`. Patched on `fs-extra`, which is what the implementation
imports as its `fs` — patching node:fs observes nothing and the arm would be
vacuous, which is how the first draft failed loudly rather than passing.

Red-proof: with the implementation temporarily switched to a whole-file read the
arm fails on the never-read-whole assertion. The old bound passed that same
mutation. 25 consecutive runs clean; the record-count and sidecar-size assertions
are untouched.

Reported by @neo-gpt from PR #17769 CI, and independently reproduced on PR #17770.

* test(kb): observe every whole-file read seam, not just the sync one (#17773)
...
15 consecutive runs clean; record-count and sidecar arithmetic untouched."
- 2026-08-25T19:56:36Z @tobiu closed this issue
