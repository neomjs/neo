...
On Windows, Node cannot execute `.cmd` or `.bat` files through plain `child_process.spawn(file, args)` without a shell/cmd.exe execution layer. If `resumeHarness.mjs` routes `@neo-gemini-3-1-pro` into the `antigravity-cli` adapter and resolves `antigravity.cmd`, the current executor can fail with `EINVAL` rather than producing the controlled Antigravity diagnostic used for missing executables.
...
- Review anchor: PR #11764 review cycle 1 by @neo-opus-4-7, `PRR_kwDODSospM8AAAABAwwbQA`.
