...
| **Amended** | 2026-06-26 (#14191, full-self-heal) — the **escalate-with-diagnosis (page)** terminal (the §2.3 envelope, the alarm-only line, the §2.4 diagnose-map's `config-drift` / `data-integrity` routes, AC-7) is superseded by **record-with-diagnosis (durable async-audit) + autonomous action**: an operatorless cloud has no human to page (@tobiu's directive — epic #14039 / #14132). `data-integrity` routing is now governed by ADR-0027 (autonomous data-recovery). The never-loop discipline, the persisted anti-thrash envelope, and the two-worlds config-lifecycle-only safety (AC-5) are **unchanged**. |
