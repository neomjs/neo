...
| PR body missing FAIR-band stance declaration (or declaration mismatches live `gh search prs` query) | **`pull-request-workflow.md §1.3` FAIR-Band Pre-Flight Gate violated** — declaration is the bypass-resistant choke-point that enforces FAIR-band author-lane discipline regardless of which lane-discovery skill path the author took. Required Action: amend PR body with the canonical declaration shape (in-band / under-target / over-target-with-rationale); over-target-yield-candidate FORBIDS PR-open until author-yield A2A is sent. Reviewer verifies declaration accuracy via `gh search prs --merged --limit 30 --sort updated --json author` (±1 PR race-condition tolerance). Empirical anchor: operator-surfaced gap 2026-05-15 post PR #11432 merge. |
