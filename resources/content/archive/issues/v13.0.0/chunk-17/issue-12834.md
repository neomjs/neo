...
Per @tobiu: Fable runs as a fully isolated Claude instance (own --user-data-dir, repo clone, Claude memory, MC identity), so its wake route self-registers at runtime like @neo-opus-grace — no committed static subscriptionTemplate (the distinct user-data-dir IS the per-instance address; a committed static route would be unnecessary and a cross-leak risk). Spec asserts @neo-fable carries no static template. Corrects the static-template safe-default in 418fc03ff."
