...
[#9492's design-lock comment](https://github.com/neomjs/neo/issues/9492#issuecomment-4644149298) carries the design source: `Container` normalizes the event seam, `View` becomes the state master, the bodies become render/event delegates, and cross-body keyboard/range behavior uses a flattened visible-column coordinate space. That design is no longer the release ceiling. The release boundary now has shipped foundation plus open gates: #12754 moves body-scroll synchronization into `grid.View`, #12708 proves the `SortZone` body-resolution path, while #9491, #9872, #9492, and #12758 remain open for index remapping, remaining orchestration follow-through, and centralized selection.

Two parts of that gate have moved from theory into shipped evidence. [#12754](https://github.com/neomjs/neo/pull/12754) moves `syncBodies()` into `grid.View`, delegates the legacy `Container.syncBodies()` seam, and preserves split-body render / scroll behavior. [#12708](https://github.com/neomjs/neo/pull/12708) resolves the [#12707](https://github.com/neomjs/neo/issues/12707) SortZone regression by resolving the grid through `owner.gridContainer` and the active body through the toolbar `layoutLock` region: `bodyStart`, `body`, or `bodyEnd`. That mirrors `grid.header.Toolbar`, gives locked-start and locked-end column drags the correct region body, and adds focused unit coverage for the body-resolution contract.

```mermaid
flowchart LR
    A["#12754 landed: View body-scroll orchestration"] --> C["multi-body foundation"]
    B["#12708 landed: region body resolution"] --> C
    C -. "next" .-> D["#9491 global / region-local indexes"]
    D -. "next" .-> E["#9872 remaining orchestration follow-through"]
    E -. "next" .-> F["#12758 / #9492 View-owned selection model"]
```

The remaining release boundary is the larger Body/Grid claim. [#9491](https://github.com/neomjs/neo/issues/9491) must still settle global-to-region-local column indexes and cross-toolbar resorting semantics, [#9872](https://github.com/neomjs/neo/issues/9872) remains the broader orchestration tracker, and [#12758](https://github.com/neomjs/neo/issues/12758) / #9492 must deliver the centralized selection model. The final v13 prose should describe #12754 and #12708 as landed Body evidence without implying that the View-owned multi-body grid architecture has fully shipped.

Representative anchors:

- [#12754](https://github.com/neomjs/neo/pull/12754) - `grid.View` owns body-scroll synchronization while preserving split-body render / scroll behavior.
...
Body/Grid guardrail: [#12754](https://github.com/neomjs/neo/pull/12754) and [#12708](https://github.com/neomjs/neo/pull/12708) are landed evidence. The release note must not claim the full View-owned multi-body grid architecture until [#9491](https://github.com/neomjs/neo/issues/9491), [#9872](https://github.com/neomjs/neo/issues/9872), [#9492](https://github.com/neomjs/neo/issues/9492), and [#12758](https://github.com/neomjs/neo/issues/12758) are terminal.
...
Current evidence refreshed on 2026-06-08T16:34Z:
