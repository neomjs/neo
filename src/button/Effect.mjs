...
const vdomClsOwner = Symbol('button.Effect.vdomCls');
...
class EffectButton extends Component {
    ...
    createVdomEffect() {
        return new Effect(() => {
            let me   = this,
                vdom = me.getVdomConfig(),
                cls  = vdom.cls;

            // Root classes generated by the effect are one logical contribution. Authored, base and
            // config-owned classes are projected by component.Base and must not become effect inputs.
            vdom.cls = [];
            me._vdom = vdom;

            if (!me.setClsContribution(vdomClsOwner, cls)) {
                me.syncClassNodes()
            }
        })
    }
