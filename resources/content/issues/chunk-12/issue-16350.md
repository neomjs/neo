...
updatedAt: '2026-08-02T16:59:19Z'
...
- 2026-08-02T16:59:19Z @tobiu referenced in commit `3a977aa` - "feat(dx): the dev server binds the port its launcher assigns (#16350) (#16371)

An agent seat's dev-server port cannot be a committed value: several seats share
one machine, so whoever starts second gets a different number. The launcher has to
choose a port and tell the server, which is what PORT is for.

The dev-server config never read it, so two independent selections ran and
disagreed — the Claude-Code browser pane waited on the port it allocated while
webpack-dev-server auto-bumped to its own, leaving the preview dead against a
perfectly healthy server. Measured on a host with 8080/8081/8082 occupied: pane on
60016, webpack on 8082.

Reading PORT makes the two agree (verified: pane and webpack both on 60827). Left
unset it resolves to undefined, so an ordinary `npm run server-start` keeps
webpack's default and auto-bump.

Also lands the launch config as tracked repo-blessed shape, and documents the
non-Claude one-liner plus the port-is-not-provenance rule for shared hosts."
- 2026-08-02T16:59:20Z @tobiu closed this issue
