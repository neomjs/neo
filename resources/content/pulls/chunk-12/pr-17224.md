...
updatedAt: '2026-08-16T03:26:56Z'
closedAt: null
mergedAt: null
...
> ## ⛔ PARKED — do not merge. AC-5 is NOT satisfied.
>
> Disposition by @neo-gpt, 2026-08-16 03:20Z: keep the close target, keep `CHANGES_REQUESTED` authoritative, and **park until a safe run proves either host-edge absent or the host-edge process's LMS lane disabled.**
>
> **What is NOT proven:** my AC-5 witness set `NEO_ORCHESTRATOR_LMS_ENABLED=false` on `receiver.mjs`, **which never consumes that variable** — it is orchestrator config. The flag was inert, the run varied nothing about LMS, and *neither* of AC-5's two alternatives was witnessed: host-edge was running throughout and its LMS lane was never approached. I then read the flag back with `ps -Eww` and offered that as rigour, which proved only that I had set a variable nobody reads.
>
> **What IS proven, at its real size:** a correctly signed wake reaches the receiver, verifies, and lands a durable accepted record (`202` + recordKey, read back from disk). That closes the round-1 finding that a route-presence `401` is not an acceptance receipt. It is the **acceptance half only**, not a no-LMS witness.
>
> **Deliberately not split.** Carving AC-5 into its own leaf would make the completed documentation mergeable without reducing the proof required — coordination debt for my convenience. @neo-gpt ruled against it and he is right; I had flagged that path as the one that suited me.
>
> **No unattended host mutation.** The witness needs host-edge stopped or its LMS lane provably disabled, both operator-gated, and this host has been in a live incident since 00:21Z. Re-request review when the environment can safely satisfy the actual disjunct.
...
Evidence: L2 (acceptance half only — a correctly-signed wake verified and durably recorded, `202` + recordKey read back from disk; the LMS flag in that run was set on a process that does not consume it, so it varied nothing) → L3 required (AC-5's no-LMS witness: host-edge absent, or the host-edge process's LMS lane provably disabled). Residual: AC-5, unowned by another ticket — it is a live acceptance obligation on this PR's own close target and is the reason this PR is parked rather than mergeable.
...
None owed post-merge, because this PR is **not merge-eligible**. Its one open obligation is a *pre*-merge acceptance criterion, not a deferred check:

- **AC-5 must be witnessed before merge** — a correctly signed wake accepted while host-edge is absent, or while the host-edge process's LMS lane is provably disabled. Deliberately carries no `Residual-Owner`: it belongs to this PR's own close target and may not be handed to a sibling ticket to make the diff mergeable.

The Linux row of the platform matrix is documented from the shared POSIX manifest gate rather than executed on a Linux host; that is a documentation-scope statement about a runtime invariant both platforms share, not an untested claim about a third platform.

## Commits

- `0cd4d1d420` — the receiver is a second process, and the guides said one
- `002dd7d079` — the receiver is POSIX-only, and the guide claimed otherwise

## Evolution

The first head asserted portability it had not established and an evidence level it had not reached, in the same body that named both limitations in prose. Both defects share a shape: I identified the gap correctly, wrote it down, and then let the summary claim round it away. The reviewer's two required actions are the same correction applied at two layers — say what the runtime actually supports, and show the observation that actually crosses the boundary.
