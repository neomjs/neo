...
> **Author's Note:** This proposal was synthesized by **Claude Opus 4.7 (Claude Code)** via `/ideation-sandbox` during operator-triggered friction-→-gold turn 2026-05-23. Originating empirical anchor: ~3hr agent idle-out during nightshift after 3 PRs cleared review-court — both `@neo-claude-opus` and `@neo-gpt` (per operator-side observation: *"both of you stopped lifecycle events on purpose"*) — despite 273 open tickets in the backlog and a 50-PR ceiling that was nowhere near saturated.
...
1. Orchestrator logs show `[idleOutNudge] Sent heartbeat nudge to @neo-claude-opus` — **no equivalent log line for `@neo-gpt`**. GPT's "lifecycle-driver watchdog" is a custom Codex-side workaround for a canonical substrate gap, not a parallel implementation of the same substrate. The canonical orchestrator nudge path is family-asymmetric.
...
|---|---|---|
| `ai/scripts/idleOutNudge.mjs` (per operator log) | Emits `[idleOutNudge] Sent heartbeat nudge to @neo-claude-opus` | **Delivery asymmetric — no equivalent for `@neo-gpt`** |
...
| **B. Orchestrator nudge delivery symmetry** | Canonical-substrate-fix for currently-asymmetric idleOutNudge | Operator log evidence: `Sent heartbeat nudge to @neo-claude-opus` only — NOT `@neo-gpt` | **Adopt** — Layer 2, BLOCKING for cross-family parity | None — pure substrate-symmetry fix |
...
> 
> Since the orchestrator runs in the operator's primary checkout where `NEO_AGENT_IDENTITY=@neo-claude-opus`, the service inherits that single binding. Per-pulse checks/nudges only `@neo-claude-opus`. GPT (different identity, different env) never gets nudged by this orchestrator — their Codex-side `nightshift-lifecycle-driver` watchdog is the federated-per-family workaround for this single-identity service design.
> 
...
> ```
> [2026-05-23T07:24:42Z] [idleOutNudge] Sent heartbeat nudge to @neo-claude-opus
...
> [2026-05-23T07:44:48Z] [idleOutNudge] Sent heartbeat nudge to @neo-claude-opus
> ```
> 
