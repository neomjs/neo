...
| 5 | Lease held by orphan | Inactive in this local pass; covered by existing stale-recovery primitive | No lease file was present under the resolved `AiConfig.orchestrator.dataDir` during the read-only pass. `HeavyMaintenanceLeaseService` has a 6-hour TTL, stale detection, and stale/malformed replacement. | Sub 2 should surface active lease owner, pid, acquiredAt, expiresAt, and stale status. Sub 9 should cover held, stale, and malformed lease cases. |
...
1. Resolve the active profile's `AiConfig.orchestrator.dataDir` (the `NEO_AI_ORCHESTRATOR_DIR`
   leaf can relocate it), then inspect `<dataDir>/heavy-maintenance-lease.json`. Never infer
   this path from the current working directory.
