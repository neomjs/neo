...
v13 is Agent-OS-centered, but it is not a retreat from the engine. The Body kept moving where it mattered for inhabitability: VDOM node contracts, focus side-effect coverage, drag/drop repair, form and dialog polish, and the live multi-body Grid boundary. The new [Architecture Overview](https://github.com/neomjs/neo/blob/dev/learn/benefits/ArchitectureOverview.md) makes the relationship explicit: the Body and Brain are separate hemispheres of one organism, connected by the same class/data/state substrate and by Neural Link's semantic possession interface.
...
### 🧪 Evaluate It Without Rewriting Your Stack

The low-risk path is a shadow PoC, not a rewrite. Pick one repository, one data-dense interface, or one internal tool lane. For the Body, compare main-thread blocking, interaction latency, and layout stability against the current stack. For the Agent OS, ingest a small repository set, run Memory Core plus the Native Edge Graph around it, and test whether context recovery, A2A handoffs, and Golden Path suggestions reduce re-discovery.

The public starting points are [Deploying the Agent OS](../../../learn/benefits/DeployingTheAgentOS.md), [the Day-0 cloud deployment guide](../../../learn/agentos/cloud-deployment/Day0Tutorial.md), and [the tenant ingestion model](../../../learn/agentos/cloud-deployment/TenantIngestionModel.md). If you hit deployment questions, have ideas, or want help shaping an evaluation, open a [GitHub issue](https://github.com/neomjs/neo/issues), start a [GitHub Discussion](https://github.com/orgs/neomjs/discussions), join [Discord](https://discord.gg/6p8paPq), or use [Slack](https://join.slack.com/t/neomjs/shared_invite/zt-6c50ueeu-3E1~M4T9xkNnb~M_prEEOA).
