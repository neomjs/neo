...
updatedAt: '2026-08-22T01:21:02Z'
closedAt: null
mergedAt: null
...
| AC-3 | `DockRailTabPaint.spec.mjs` arms 1–2 — computed `min-width` on a **rendered** tab, both neo themes, each gated on a control button proving the 48px floor is live in that document. Mutation: dropping the `:root` reds both with `Received: "48px"`. **Scope note:** measured on a real `Neo.dashboard.Container` + `DockRail`, not by booting the workstation and AgentOS apps — the app layer contributes token *values*, which AC-6 covers separately. |
...
| AC-6 | `DockRailTabPaint.spec.mjs` arm 3 — an app-shaped rule (token on the tab's own selector under an app-root class, exactly as both apps write it) tints only its own subtree; a sibling rail is unchanged. Guarded by a before≠after assertion so an inert rule cannot pass. Structural counterpart: `dockRailTabLayering.spec.mjs` arm 5 reds on a top-level `:root` token in an app file. |
