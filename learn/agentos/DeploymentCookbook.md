...
| `NEO_AUTO_SUMMARIZE` | MC | Set to `true` to enable startup + disconnect-driven session summarization. The lifecycle is now handled by the `bridge-daemon`, which uses a `PID_FILE` lock to guarantee single-writer semantics across multiple local harness instances. |
