...
updatedAt: '2026-08-05T13:13:05Z'
...
- 2026-08-05T13:13:05Z @tobiu referenced in commit `8882c23` - "refactor(ai): remove the checkout-relative graph-path resolver (#16544) (#16545)

resolveMemoryCoreGraphPath lost its only caller when PR #16527 routed the
turn-presence hook over MCP. Verified against merged dev by walking every
.mjs/.json/.md/.yaml blob in the tree: the sole file containing the symbol was the
one defining it, and no spec referenced it.

A dead export matters more than usual here, because this function IS the pattern
#16513 was filed to remove - path.resolve(rootDir, '.neo-ai-data/...') where rootDir
came from the caller's own module location. That produced 7192 turn-presence
intervals from 9 agents written into maintainer checkouts no reader ever queried,
invisible because a writable SQLite file accepts writes happily. Leaving it exported
leaves a working, importable, apparently-sanctioned helper for exactly that mistake.

MEMORY_CORE_GRAPH_DB_ENV is kept - configBase.mjs consumes it for the graphProd leaf.
Only the function and the node:path import it stranded are removed.

Verified: zero references tree-wide; the configBase consumer still resolves;
resolveTurnPresenceRuntimeConfig unchanged; 272 turn-presence + hook specs green."
- 2026-08-05T13:13:05Z @tobiu closed this issue
