...
updatedAt: '2026-07-19T09:09:31Z'
...
## Visual-continuity AC — backfilled from PR #15327 review (Grace RA-3, applied by Clio takeover)

Projecting a native attribute Neo previously synthesized via class activates a dormant user-agent cascade (`button:disabled`); presence assertions cannot see it. The contract therefore also requires:

- [x] The `neo-disabled` class remains the **sole styling authority**: the rendered disabled paint with the native attribute present is identical to the class-only baseline. Owned via the `&:disabled` pin in `resources/scss/src/button/Base.scss` (measured leak before the pin: Chromium paints the element `color` as disabled GrayText, `rgba(16,16,16,0.3)` vs baseline `rgb(0,0,0)`), witnessed by the render-equivalence assertion in `test/playwright/component/button/Base.spec.mjs` (attribute-on vs attribute-off computed styles, same node — red before the pin, green after).

Durable rule this encodes for the next native-attribute projection (`readonly`, `required`, `checked`, …): the functional oracle (attribute present) and the visual oracle (render unchanged) are different oracles — audit or own the UA pseudo-class the attribute activates.
