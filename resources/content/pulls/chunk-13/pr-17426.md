...
updatedAt: '2026-08-20T17:50:23Z'
closedAt: null
mergedAt: null
...
**P3 — the residual had no executable owner.** It pointed at epic #17411, which had no child for the migration. **#17428** is filed and linked, and investigating it turned up something better than a new mechanism: **the guaranteed trigger already exists.** `generationElectionStore.mjs:7` states that an input-strategy change *"invalidates every existing vector"*, and `EMBEDDING_POISON_STRATEGY_FAMILY = 'kb-embedding-input-v1'` is the one-token lever, with a docblock describing exactly this case.

But it is **global while the damage is partial** — neo's own corpus was never affected, so a bump rebuilds a large correct corpus to repair a smaller broken one, on a lane measured at ~0.19 chunks/min. The targeted alternative needs a detector that **does not exist**: `upsert` writes no `documents`, and metadata cannot date a row (an affected row and a freshly-correct one carry identical `kind`-without-`type`). So #17428 escalates that cost fork rather than deciding it, and its first AC is the operator resolving it.

Also corrected on his instruction: **"repair naturally" is gone.** The accurate statement is that `parserVersion` *is* a hash input, so an advance would re-embed — and nothing schedules one.
