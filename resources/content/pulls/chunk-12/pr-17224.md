...
updatedAt: '2026-08-16T03:11:44Z'
closedAt: null
mergedAt: null
...
Evidence: L3 (a correctly-signed wake accepted with `202` and a durable record on disk, against a receiver started with `NEO_ORCHESTRATOR_LMS_ENABLED=false`) → L3 required (AC-5's no-LMS witness). Residual: AC-5 under a genuinely *absent* host edge, Residual-Owner: #17227 — this host runs one, and stopping it during a live incident was operator-vetoed.
...
- [ ] Confirm on a Linux host that the newly documented receiver invocation runs as written — this checkout is macOS, so the Linux row is documented from the shared POSIX gate rather than executed.
  Residual-Owner: #17227
