...
updatedAt: '2026-07-12T15:56:02Z'
closedAt: null
mergedAt: null
...
**DRAFT — cycle-3; head 2ad8442cb.** Exact-head CI: earlier heads were green; 2ad8442cb's hosted run is the authoritative gate (not asserting green for an unverified head). The wrap core + **both** consumed surfaces (`query_raw_memories`, `ask_knowledge_base`) are wired + unit-tested. Emmy's cycle-2/3 reviews found real authorization/semantic/contract gaps; this cycle closed the tractable ones. The **six-gate matrix** below is the live status; gates #1/#2 (heavy, architectural) are the convergence-owned future pairing/Mnemo decision. Draft until the matrix + L3 are met.
...
1. **Path-level tenant/edge RLS** (GraphService-owned seam + per-consumer policy) — **OPEN, heavy/architectural.** Landed: enrich-scoped `PUBLIC_TRAVERSABLE_LABELS` fail-closed allow-list `['CONCEPT','FILE']` (a717a830d) blocks private-intermediate traversal; the shared #14474 probe keeps full reachability via `traversableLabels: null`. Remaining: full tenant/edge RLS via a GraphService-owned raw seam + per-consumer policy (FILE-intermediate for Memory, terminal for KB). → convergence-owned future pairing/Mnemo.
2. **Request-global traversal budget + honest truncation** — **OPEN, heavy.** Hop budget currently resets per alias member; `walk.truncated` not surfaced. → future pairing/Mnemo.
...
4. **GraphService.ready() lifecycle** — **ADDRESSED / contested (Emmy's fork):** both enrich graph-read sites already fail-safe degrade (resolveConcepts try/catch→[], spec :88-93 pins a throwing not-ready graph→[]; the raw-edge walk likewise), so a not-ready graph contributes nothing and the flat path stays byte-identical. `await ready()` would only convert a transient-init no-op into a wait — argued redundant for opt-in enrichment. Emmy's call.
5. **KB response contract** — **CLOSED:** explicit `conceptWalk` event schema + references `via`/`conceptPath`/nullable-score (141a49100); a generated-call fixture proving default-off vs opt-in envelope at the MCP boundary (2ad8442cb).
