...
1.  **Internal Power:** Agents can now seamlessly use our three robust, built-in servers via the standard protocol:
    *   `github-workflow`: Full project management (issues, PRs, releases).
    *   `knowledge-base`: Semantic search over the entire codebase and docs.
    *   `memory-core`: Long-term memory and session recall.
2.  **External Reach:** Agents can connect to the broader ecosystem of third-party MCP servers (e.g., PostgreSQL, Google Drive, Slack), configured simply via `servers: ['my-external-server']`.

This hybrid approach gives agents the best of both worlds: deep integration with the Neo.mjs OS and unlimited extensibility.
