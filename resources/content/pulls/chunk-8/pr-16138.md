...
updatedAt: '2026-07-29T18:27:26Z'
closedAt: null
mergedAt: null
...
Headed E2E runs now select a compositor-presenting Chrome profile by default. The existing
GPU-intent and frame-limit-disabled arguments remain available only through the exact
`NEO_E2E_ENGINE_PROFILE=1` opt-in for engine and benchmark investigations.

Evidence: L1 (focused selector matrix and source-wired pixel witnesses) → L3 required (headed
Workstation popup pixels on an unrestricted macOS seat). The local headed attempt stopped before
Chrome created a page because the harness sandbox denied Crashpad access; it is not counted as
product evidence.
...
- `NEO_FILM_TAKE=1` remains a pacing/recording signal and no longer decides whether Chrome presents
  pixels.
- Exact film plus engine sentinels fail before browser launch because capture under the known
  non-presenting engine profile is contradictory.
- The live loader census found one inherited-environment path: package, direct Playwright, and
  nightly entrypoints all reach the same config; CI and the matrix/visual configs do not consume
  this selector.
- No current OffscreenCanvas correctness spec requires the frame-limit override. Published
  Harness Endurance benchmark reproduction now opts into the engine profile explicitly.
- Workstation Canvas, dock-preview, and re-entry screenshot receipts now follow presenting
  capability instead of film mode, so ordinary headed runs retain pixel artifacts.

## Test Evidence

- `npm run test-unit -- test/playwright/unit/e2e/glState.spec.mjs` — 8/8 passed.
- Unit matrix covers unset, `0`, `false`, arbitrary text, film-only, engine-only, and both-set
  rejection; it also pins proportional GL demand.
- `node --check` for the selector and focused unit spec — passed.
- Staged archaeology, JSDoc, parse, whitespace, shorthand, derived-domain, and block-alignment
  hooks — passed.
- Headed Workstation attempt with both sentinels unset — infrastructure stop before page creation:
  Chrome Crashpad `Operation not permitted` plus reporter `uv_uptime EPERM`. No product assertion
  was reached and no visual claim is derived from that run.

## Post-Merge Validation

- [ ] On an unrestricted macOS seat, run the same-gesture Workstation tear-out/re-entry witness
      headed with both profile sentinels unset and confirm retained non-empty PNG artifacts.
...
The recurring failure was encoded in the default: semantic truth could remain green behind empty
native windows, while a maintainer had to remember a film-only sentinel to see pixels. The repair
makes presentation unsurprising and turns the specialized non-presenting profile into an explicit
choice. The intake also converted two migration risks into executable contracts: a complete loader
census and a fail-fast sentinel conflict.
