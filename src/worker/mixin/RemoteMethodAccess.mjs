...
class RemoteMethodAccess extends Base {
    ...
    generateRemote(remote, method) {
        ...
        const proxy = function(data, buffer) {
            ...
            // the Message it actually sent, so waiting can never be queued inside it. Once ready, straight through.
            if (origin === 'canvas' && me.canvasGroups && !me.canvasGroups.isReady(data?.windowId)) {
                return me.canvasGroups.whenReady(data?.windowId).then(() => me.promiseMessage(opts.destination, opts, buffer))
            }
