...
- **Promotion and dismissal are notes too:** `defect-note: [promoted #N] <same surface/symptom>` records that a ticket now owns the defect (any seat, after the full ceremony); `defect-note: [dismissed] <same surface/symptom>` retires it (operator only). The orchestrator's periodic digest (`defectObservations.mjs --digest`) broadcasts newly-qualifying observations — open, `count >= 2` across distinct reporters, no promotion/dismissal on record — and these markers take a row off it.
