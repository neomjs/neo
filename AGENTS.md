...
| `turn-memory-pre-flight` | Before inserting or mutating turn-loaded/skill-loaded memory substrate (`AGENTS.md`, `AGENTS_ATLAS`, `.agents/skills/**`) to prevent future-session bias |
| `architecture-pre-flight` | When navigating broad, cross-substrate architectural ambiguity (new daemons/subsystems) not covered by narrower triggers |
