...
updatedAt: '2026-08-21T22:50:26Z'
...
- 2026-08-21T22:17:15Z @neo-opus-ada referenced in commit `111c12f` - "fix(ai): the pipe diagnosis needs the byte, not a resemblance (#17284)

Emmy's RA-1, and she is right about the class: the repair reintroduced the
defect it exists to remove, one layer along.

`collapsePipesAndSpace` removes SPACES as well as pipes, so "observed partial"
and "observedpartial" compare equal with no pipe anywhere in either string.
Similarity alone was enough to reach the pipe branch, which then told an author
to escape a pipe their text does not contain -- an unfollowable instruction,
produced by the diagnosis rather than by the rule.

The expected text's pipe is now a required cause witness. Similarity cannot
prove which byte the parser lost; only the presence of that byte can.

The discriminating half of the new arm is the SECOND assertion. Refusing is
correct either way -- the strings genuinely differ -- so an arm that only checked
for rejection would have passed against the broken code. It asserts the refusal
does NOT name the escape, and falls through to the ordinary verbatim demand.
Mutation-verified: removing the witness reproduces Emmy's exact message,
`Escape every pipe in: "Record the observed partial contract"`.

ONE CORRECTION to the review: `extractDispositionRows` did not LOSE a docblock.
It never had one -- `git show origin/dev` confirms it sits bare there too, so the
insertion orphaned nothing. The gap is real and the Contextual Completeness gate
wants it closed, so it now carries one; but it is a pre-existing gap this diff
walked past, not damage this diff did.

unit-brain 181 passed."
- 2026-08-21T22:39:45Z @neo-opus-ada referenced in commit `7ddde3a` - "fix(ai): two orphaned docblocks reattach to the functions they describe (#17284)

Emmy was right and my correction to her was wrong. `extractDispositionRows`
DID have a docblock; it sat orphaned above `collapsePipesAndSpace`, stacked in
front of that function's own block.

MY PROBE ANSWERED A NARROWER QUESTION THAN THE CLAIM. I ran `git show
origin/dev | grep -B 16 "function extractDispositionRows"` and read "nothing
attached" as "never had one". Those are different questions, and the second is
not answerable by looking immediately above the function -- which is exactly
where an orphan is NOT.

One correction to the correction, for provenance: the orphan predates this PR.
It is on dev too, stranded above a different function there. So the insertion
did not create it; it moved which function it sits in front of.

Rather than delete the old block, its content is folded into the attached one:
the second-to-last-cell rule and the separator-row handling are real parsing
contract that my replacement did not carry. Nothing is lost, and there is now
one docblock instead of two.

SECOND ORPHAN, same class, found while fixing the first: a Micro-Delta
validation-failure block stranded ~90 lines above
`getMicroDeltaPrReviewTemplateValidationFailure`, which had none of its own.
That one MOVES rather than merges -- it is the only copy, so deleting it would
have destroyed the documentation instead of relocating it. The distinction is
the same one I got wrong above, checked this time before acting.

The file now has zero stacked docblock pairs, verified mechanically rather than
by eye.

unit-brain 181 passed."
- 2026-08-21T22:50:26Z @tobiu referenced in commit `473dac5` - "fix(ai): the Round-2 cell parse stops losing the bytes it demands verbatim (#17284) (#17498)

* fix(ai): the Round-2 cell parse stops losing the bytes it demands verbatim (#17284)

Two defects in extractDispositionRows, one function, one symptom: the gate tells
an author to carry text verbatim that they already carried verbatim.

split('|') splits on EVERY pipe, so a pipe inside a cell becomes a boundary --
the byte is deleted, the halves rejoin across a space, and every later column
shifts left. Escaping was worse than not escaping: \| was cut too, leaving the
backslash in the compared text, so the author doing the correct markdown thing
got a stranger mismatch and no wording got them out. @neo-gpt was blocked on
exactly this posting Round 2 for PR #17475, whose RA-2 carries observed|partial.

The split now honours escapes and restores them, so an escaped pipe round-trips
byte-for-byte. A raw pipe still yields an ambiguous row, because markdown cannot
express it -- what changed is that escaping works.

The label detector accepted digits only, so RA-1a and §0 were folded into the
carried action. Widened to a bare ordinal token, deliberately still a SHAPE and
not "whatever sits in column one": anything carrying whitespace or prose stays
part of the action, which is what stops the repair from discharging every row.

And the refusal now names the mechanism. When two strings differ only where a
pipe sits, the row is reporting our reconstruction rather than the author's
text; saying "carry it verbatim" there is an instruction nobody can follow. The
byte-verbatim rule itself is untouched -- this diagnoses its failure, never
relaxes it.

* test(ai): the pipe fixture carries TWO pipes, so the escape assertion means what it says (#17284)
...
* fix(ai): the pipe diagnosis needs the byte, not a resemblance (#17284)
...
* fix(ai): two orphaned docblocks reattach to the functions they describe (#17284)
...
unit-brain 181 passed."
- 2026-08-21T22:50:27Z @tobiu closed this issue
