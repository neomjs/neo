...
If your new `.mjs` file has a clear sibling pattern in the chosen directory — same lifecycle category, same architectural role, same naming convention — you are inside `§23 Sibling-File-Lift` territory and the fast-path applies.
...
> *"Pre-Flight (structural fast-path): authoring `<path>` matches sibling pattern of `<sibling-path>` in `<dir>`; §23 sibling-file-lift applies; no novel directory choice."*
...
Mirrors `§23` / `§22.1 Mailbox Check` / `pr-review-guide §9.4 Cold-Cache Exception`. Explicit reasoning-statement before authoring:
