...
updatedAt: '2026-08-19T22:44:29Z'
closedAt: null
mergedAt: null
...
Fixed both ways: the run measures block extent (contiguous added comment lines, tags included — a reader faces the whole block), and the red-proof runs the tool over named commits. `prose` still excludes tags, because the two axes measure different objects on purpose.
...
`test/playwright/unit/buildScripts/checkCommentDensity.spec.mjs` — **35 passed**.

- **Red-proof, run axis, measured by the tool:** `a17ade4264` at 19% share / 45-line block — the case the share axis provably cannot flag. **Silent arm:** `087114e8ab` at 24% / 11.
