...
updatedAt: '2026-08-25T16:56:34Z'
closedAt: null
mergedAt: null
...
Evidence: the repair arms are red against a store of stale rows and green after; helper + runner specs 36 passed; knowledge-base and `ai/scripts` suites clean apart from reds proven pre-existing below.
...
- `staleEmbeddingRepair.spec.mjs` — **19 passed** (8 round-trip fidelity arms, the null-reversal arm, selection/idempotence).
- `repairStaleEmbeddings.spec.mjs` — **17 passed** (red-proof, single-write, metadata preservation, partial-run, flag safety, page walk over 4,500 rows).
...
- `ai/scripts/migrations/repairStaleEmbeddings.mjs` — new runner; dry-run default, `--apply` writes, before/after census from one instrument.
