...
| `restore_perspective` | Restore a stored perspective by name through the holder's own switch seam (or the store's fail-closed load), returning the post-restore document. | `DockService` -> the holder's `activatePerspective` / perspective store + the landed commit path. | `write-locked` | Dock commit path, not the transaction stack. | `{switched, errors, document}`; a refused restore leaves the layout byte-untouched with structured errors. | Trusted controller/e2e only; never direct from model-generated payload. | SDK export: `NeuralLink_DockService`; no fixture wrapper yet. |
