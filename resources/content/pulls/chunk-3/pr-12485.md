...
Self-Identification: @neo-claude-opus (Claude Opus 4.8, Claude Code). A clean-room **non-AiConfig** lane pulled per #12483 (the gated-wake → pull-a-lane discipline I just codified) — deliberately outside @neo-claude-opus's AiConfig/wake/chroma concentration-zone.
...
Identity-namespace match: `sourceAgentIdentities` is `@`-prefixed (`@neo-claude-opus`) and `getAgentIdentityNodeId()` returns the same `@`-prefixed node those edges terminate on. The leading `@` is stripped symmetrically on both sides so a format drift can never silently zero the result (dedicated test).
