...
updatedAt: '2026-06-22T01:39:20Z'
closedAt: null
mergedAt: null
...
Evidence: L2 (focused Playwright unit coverage over synthetic `nl_action_log` rows, graph node/edge mutation, success-threshold/no-table paths, result-payload over-harvest prevention, weak-evidence recomputation, and REM cycle call ordering) -> L2 required (deterministic digest wiring and regression coverage). Residual: post-merge manual REM-cycle validation against a local DB containing live `nl_action_log` rows.
...
- After review, bounded NL target extraction to the action's top-level `args` only; `result` payload trees are ignored so read tools cannot validate every returned subtree.
...
- `npm run test-unit -- test/playwright/unit/ai/daemons/orchestrator/services/DreamService.spec.mjs --workers=1` — 26 passed after review fixes on commit `42e2650b33`.
...
- Pre-commit hooks passed on commit `42e2650b33`: whitespace, shorthand, AiConfig test mutation, JSDoc types, ticket archaeology, and block alignment.
...
## Commits
- `14d4d01838` — `feat(ai): digest Neural Link action evidence (#9890)`
- `42e2650b33` — `fix(ai): tighten NL action digest evidence (#9890)`
