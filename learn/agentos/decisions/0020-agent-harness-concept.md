...
- **Shell: Electron, decided** (operator, 2026-06-12): always Chromium + always Node.js — worker-topology determinism fleet-wide; the packaged product **bootstraps the local Agent OS plane from the Electron main** — in-process as the target arm, **child-process supervision as the sanctioned fallback** capping the topology risk (#13033 carries the spike) — as host machinery beneath the fleet wire, while **the cockpit's runtime identity is a pure client** of the (bundled or remote) plane's fleet service. Tauri retired. Web-served mode remains the development convenience + goodie, never the priority. *[Sentence amended 2026-08-08 — ADR 0038 §3.1 (#16747); previously "the Agent OS runs in-process in the Electron main (target), with child-process supervision as the sanctioned fallback". The #13033 hosting-arm decision is unchanged; what changed is WHOSE identity the hosting is — plane bootstrap, not cockpit. Attach-or-own is retired as cockpit identity (C5 #16746); ADR 0034 carries the shell-specific amendment at §2.1/§2.6.]*
- **Source placement preserves the hemispheres:** harness UI in `apps/` (**replacing the `apps/agentos` early PoC**); Node-side fleet machinery in `ai/` (beside orchestrator/wake daemons); Neural Link / MCP is the bridge. No top-level source mixing — the Electron **packaging root** (own `package.json`) wraps and serves the operator-selected source-mode Body during development, and later packages the built Body + Brain, without owning or moving either source tree. **Credentials (PATs) live Brain-side only** — they never transit the browser.
...
- **Multi-window choreography:** the magic **already ships** (`apps/colors` + the agentos prototype): one continuous drag migrates a pane's *embodiment* — pane → OS popup → pane — across apps with live slide-resort, the component staying one live heap object throughout (`Neo.manager.Window` God-View + `getWindowAt(x,y)` + `main.addon.WindowPosition`; the `#8164`→`#9498` lineage). The named gaps are leaves: popup terminal drop (#13025), OS-window drag reintegration (#13028), QT-grade docking (#13030). The docking model contract lives in [HarnessDockZoneModel.md](../HarnessDockZoneModel.md); it is a contract pointer, not an ADR amendment. ADR-0029 §2.8.5 defines the adjacent generic Possession Interface: Neural Link may inspect and physically operate a topology-known popup through a one-time private identity join, with close separately granted by the semantic owner and every product meaning retained above that seam.
...
4. **Fleet lifecycle owns restart affordances** — including runtime MCP-server restarts with settle-or-reject promise semantics. *[Amended 2026-08-08 — ADR 0038 §3.1 (#16747): the affordances are plane-owned surfaces behind `CAN_ADMINISTER_FLEET_OF`, invoked over the authenticated fleet wire — the cockpit requests and renders outcomes, never owns the affordance; settle-or-reject semantics remain plane-side contracts.]*
