...
stage one captures `panes` and `zones` once; stage two adds `perspectives` — a map of named `zones` arrangements or
already-lowered documents (validated like a supplied `dockModel` and never registered, so a baseline may hold a pane a
sibling workspace owns), captured once the same way, with a `zones`-only workspace declaring one under a reserved name — and the reactive
