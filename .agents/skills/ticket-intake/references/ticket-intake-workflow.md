...
1. **Fetch Remote Truth:** Before validating a ticket premise, you MUST ensure you are reading the latest truth. You MUST use the `mcp_neo-mjs-github-workflow_get_conversation` tool to fetch the live issue body and comment thread directly from GitHub.
   - **Sandman Handoff Pre-Flight Guard:** Before proceeding with intake, verify that `resources/content/sandman_handoff.md` does not contain a blocking `lane-state` (e.g., `AWAITING_REVIEW`, `AWAITING_HUMAN`). If a blocking state is active, you are explicitly forbidden from initiating new work until it is resolved.
