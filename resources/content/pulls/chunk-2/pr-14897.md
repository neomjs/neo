...
updatedAt: '2026-07-09T20:07:37Z'
closedAt: null
mergedAt: null
...
**Draft / WIP** — Leaf B of the temporal-pyramid substrate: the L1/L2 durable aggregation lane plus the six ADR 0028 §2.4 velocity fields. The pure aggregation engine and the supervised, lease-aware aggregation service are on the branch; the daemon entry point, the remaining three velocity source bindings, the `SUMMARY_DAILY` graph labels and the ADR 0024 update land in this same PR before it leaves draft.

Evidence: L2 (mock dispatch — pure-engine folds plus poll/lease behavior against a stubbed heavy-maintenance lane and a stubbed `execCommand`) → L3 required (live non-destructive probe: a real Chroma upsert into the `temporal-summary` collection, and lane registration under a real `MaintenanceBackpressureService`), for the "supervised, backpressure-leased scheduled lane" and "retention/versioning defined + tested within the `version` field" ACs. Residual: all seven ACs remain open at this draft head — the lane is not yet end-to-end runnable [#14434].
...
Both suites were run on this draft head, in a bootstrapped worktree, exit 0:
