...
Use when applying an identity change (a new tagline, a corrected fact, a new capability that outgrew the framing). This file is the entry point; it routes to the three payloads below as each step needs them:
...
State in one line WHAT is changing and whether it is a **FACT** (single value) or **FRAMING** (positioning). Most changes are one or the other; a mixed sentence ("the first multi-worker engine") is both — split it: the claim is framing, "first/2019" is a fact.

## Step 1 — Enumerate the affected surfaces
Open `references/affected-areas-map.md`. For the thing you're changing, list EVERY surface across all 5 classes that carries it. Do not stop at the README. Grep to confirm occurrences:
```bash
...
- **FACT** → fix the SSOT (see `references/facts-ledger.md`), then derive / coherence-check every other occurrence. Never hand-edit a derived copy and call it done.
- **FRAMING** → see `references/framing-governance.md`. Check each surface for compatibility with the apex (not equality — audience-segmentation is deliberate). If a surface *contradicts* the apex, classify drift-vs-divergence; escalate divergence to the operator.
...
Before writing any "fastest / only / first / Nth" into a surface, run the check in `references/framing-governance.md` § claim gate. Drop unbounded superlatives or hedge them. Regenerate every number from its SSOT at this step.
