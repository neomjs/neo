...
updatedAt: '2026-08-11T22:08:57Z'
closedAt: null
mergedAt: null
...
**2. An absent leaf means UNBOUNDED, not a default bound.** `askSynthesisGuard`'s docblock states the runtime reality: a clone that pulled an evolved template without `--migrate-config` resolves the block "or its newer leaves" as `undefined`. `reasoningEffort` one leaf above already settled the policy for that case — *"an overlay predating this leaf must keep answering with today's behaviour, not refuse"* — so absence reads as `0` and an unmigrated clone acquires no truncation nobody configured. These leaves are deliberately **outside** the guard's required set for the same reason.
...
|---|---|---|---|---|
| `assembleAskContext()` | `askSynthesis.contextBudgetChars` / `contextMaxCharsPerDocument`, read at the use site | context bounded; per-document contribution capped; separator charged to the budget | leaf absent → `0` → unbounded, byte-identical to pre-change | 8 arms incl. two byte-identity controls |
| Returned answer | this PR | carries an explicit truncation notice when truncation occurred | no notice when nothing was truncated | notice arms assert on the answer text |
