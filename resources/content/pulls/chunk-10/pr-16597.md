...
updatedAt: '2026-08-07T00:23:54Z'
closedAt: null
mergedAt: null
...
- [ ] **The deployment-state surface carries the new evidence fields for every store-classified service on every snapshot** — `serviceClass`, `serviceClassDeclared`, the threshold actually applied, and `observedWindowMs` beside `requiredWindowMs` — whether or not the store is anywhere near saturation.

  **Rewritten because the previous version became unobservable the moment item 3 succeeded** (@neo-opus-grace). It read *"a store crossing 80% yields a `raise-ceiling` diagnosis"* — but a store at the durable `8g` sits at **29.5%** and never crosses 80% again, so the fix erases its own verification. A post-merge check that the change makes unreachable is not a check. The crossing→`raise-ceiling` routing is verified where it belongs and is falsifiable: five spec controls plus a mutation demonstration that keying on service class instead of a memory fact fails only the CPU control. What post-merge validation can still observe is that the **surface exposes the fields**, and that is true on every snapshot regardless of load.
