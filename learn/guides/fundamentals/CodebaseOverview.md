...
> For a visual architectural map showing how the Runtime Engine and Agent OS connect as a closed feedback loop, see [Architecture Overview](../../benefits/ArchitectureOverview.md). For the autonomous sub-agent delegation model (profiles, capability gating, cost control), see [Swarm Intelligence](../../agentos/SwarmIntelligence.md). For the offline forecasting engine that scores and prioritizes work, see [The Dream Pipeline & Golden Path](../../agentos/DreamPipeline.md).
