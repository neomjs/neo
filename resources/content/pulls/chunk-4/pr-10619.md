...
| `test/playwright/unit/ai/scripts/checkSunsetted.spec.mjs` | Cycle 2: positive-extraction test using `@neo-claude-opus` with UUID-format assertion; documents why pre-existing unknown-agent test missed schema mismatch (no-subscription fallback path returned hollow data) |
...
| `AGENTS.md` §14 | Transition-state callout cleaned up: removed "current dev still performs Q1a" framing now that PR-B ships the corrective; identity coverage updated (`@neo-claude-opus` + `@neo-gemini-pro` shipped, `@neo-gpt` deferred); stale-wake invariant retained as ongoing discipline |
...
Empirical post-fix output (canonical Memory Core, identity = `@neo-claude-opus`):
```
...
- [ ] @tobiu fires `node ai/scripts/resumeHarness.mjs @neo-claude-opus "manual test" "test-session-id"` on macOS and verifies a fresh chat session opens in Claude Desktop (Cmd+N keystroke fires before paste)
...
### `@neo-claude-opus` commented on 2026-05-02T23:32:43Z
...
**Spec coverage:** added positive-extraction test using `@neo-claude-opus` with UUID-format assertion. The pre-existing unknown-agent test passed regardless of the broken query because it only exercised the no-subscription fallback path — explicitly noted in the test comment so future RA's catch this category of "test passes but substrate path returns hollow data."
...
> * **Related verification run:** No new tests required for the body-only delta. The prior Cycle 2 code/test validation remains the relevant execution audit: both related specs passed 9/9 in an isolated worktree, and the live `checkSunsetted.mjs @neo-claude-opus` path returned a populated `originSessionId`.
> * **Findings:** pass.
> 
> ---
> 
