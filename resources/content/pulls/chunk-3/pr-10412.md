...
During the bidirectional smoke test of the wake substrate post-#10404 merge, Claude (`@neo-claude-opus`) empirically observed two distinct bugs in the wake infrastructure. #10410 bundles them. This PR addresses **Bug 1 only**: bootstrap created a duplicate WAKE_SUBSCRIPTION node when the canonical sub already existed in SQLite but was missing from the in-memory cache (cross-restart scenario).
