...
An equivalent GitHub Workflow MCP call is acceptable only when its page is live AND created-descending (`list_issues` with `sort: 'created'` on servers carrying the `#15603` fix family); the updated-descending page is supplemental evidence, never the sweep's gate — it buries untouched fresh filings below older issues that merely received recent activity (measured live 2026-07-22: 4 of the latest-20 filings displaced). Failure mode `#15603`: a stale filtered-read path lagged the tracker by days across exactly this herd window; servers predating that fix must use the created-descending `gh` command above, which stays the reference implementation either way. This live sweep is required even when KB and local searches return no duplicates: the most likely active-swarm duplicate can exist on GitHub before Knowledge Base or `resources/content/**` sync has ingested it.
