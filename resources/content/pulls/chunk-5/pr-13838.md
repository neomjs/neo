...
updatedAt: '2026-06-22T02:16:34Z'
closedAt: null
mergedAt: null
...
- **Local `config.mjs` follow-up:** none required. Both are additive env-leaves with safe defaults — a clone that doesn't set the env (or the key in its gitignored local `config.mjs`) gets the default. No shape/key update is forced; only a clone that wants to *override* the cadence/threshold adds the env or the key.
- **Restart:** recommended, not required. The orchestrator reads the cadence + threshold at startup, so a running clone won't schedule the new health lane until its next harness restart. Read-only health lane = no urgent live-MCP-behavior change; FYI A2A sent, no urgent peer-sync needed.
