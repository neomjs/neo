...
updatedAt: '2026-07-02T17:52:36Z'
...
- 2026-07-02T17:50:42Z @neo-gpt added the `not-code-ready` label
- 2026-07-02T17:50:42Z @neo-gpt added the `needs-design` label
### @neo-gpt - 2026-07-02T17:52:36Z

## [ticket-intake] Not code ready: contract ledger required before implementation

[ARCH_ALIGNMENT]
I did not claim or branch #14418.

Intake classification: `needs-narrowing` / contract-alignment-required. The lane is valid in direction, but not implementation-ready as written.

V-B-A evidence:
- Live #14418 is same-day (`createdAt: 2026-07-02T01:31:36Z`, `updatedAt: 2026-07-02T17:50:42Z`), unassigned, and has no direct implementation PR/close-link for `14418`.
- Prior-art memory supports the premise: Grace's graduation memory `e24c79dd-b4db-4999-a09c-872cd273c14b` documents why #13873 became standalone #14418; memory `07fa7bcf-b928-4240-b1dc-5fc686f8e5d7` records the operator-set objective: drained REM backlog scaled by live agent count.
- ADR successor-risk: `adr-aligned` with ADR 0026's fixed controller-blind `apply(serviceKey, action)` + persisted envelope, and #14191's record-not-page amendment. No ADR challenge found; the implementation must not widen the action set or bypass the envelope.
- The readiness blocker is ticket contract completeness, not concept validity. #14418 changes/introduces multiple consumed surfaces but has no Contract Ledger Matrix:
  - homeostatic controller/coordinator above the reactive controller,
  - `RecoveryActuatorService.apply(...)` envelope/budget/quarantine use,
  - `reconfigure(knownKey)` config knobs/static caps,
  - summary fidelity metadata (`sourceTier`, `degraded`, likely `usedTier`) and weak-digest supersession,
  - `query_summaries` discount semantics while weak summaries exist,
  - `healEventLedgerStore` record-not-page overload terminal.
- #14422 has since graduated the shared OQ6 four-axis lattice into #14472: authority = `trustTier` reference-only, fidelity extends `sourceTier`/`degraded`, extraction provenance remains separate, lifecycle carries `candidate/promoted/rejected/stale/superseded`. AC-3 must consume that shared contract instead of minting a local `weakDigest` vocabulary.

Required body update before code:
Add a Contract Ledger Matrix with `Target Surface | Source of Authority | Proposed Behavior | Fallback | Docs | Evidence` rows for the surfaces above, cite ADR 0026/#14191 and #14422/#14472, and state the implementation slice boundary. After that, re-run intake and the ticket can move.
