...
3. **Duplication Check:** Semantic search is significantly more powerful than string matching. First, prioritize using `ask_knowledge_base` or `query_documents` (with `type='ticket'`) to semantically search for overlapping active or archived initiatives. If you require exact keyword verification, fallback to using `grep_search` explicitly targeting `resources/content/issues`, `resources/content/issue-archive`, and `resources/content/discussions`.
