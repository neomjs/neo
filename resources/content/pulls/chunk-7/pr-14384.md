...
updatedAt: '2026-06-30T18:22:36Z'
closedAt: null
mergedAt: null
...
- Reshaped both `DeployingTheAgentOS.md` Mermaid diagrams into narrower `flowchart TD` layouts with vertical subgraphs for MCP surfaces, data/state, provider placement, and runtime control, avoiding the wide same-rank fan-out that scaled the rendered diagrams down.
...
- Diagram-readability correction re-ran `npm run ai:lint-guides`, `npm run agent-preflight -- learn/benefits/AgentMemory.md learn/benefits/DeployingTheAgentOS.md --pr-body /private/tmp/pr-14321-body.md`, and `git diff --check` after rebasing onto current `origin/dev`.
- Mermaid parse/render residual unchanged: this checkout has the `mermaid` package but no `mmdc` binary, and local parse hits `DOMPurify.addHook is not a function` before syntax validation; browser/portal render remains the required verification surface.

## Post-Merge Validation

- [ ] Browser/portal render-check both Mermaid diagrams in `learn/benefits/DeployingTheAgentOS.md`.
- [ ] Confirm #14321 closes while the benefits IA/restructure and final sweep children remain open.

## Commits

- `287e439818` — `docs(benefits): refresh Agent OS deployment memory value (#14321)`
- `234071f31d` — `docs(benefits): clarify Agent OS deployment topology (#14321)`
- `4c70560516` — `docs(benefits): correct Agent OS deployment guide facts (#14321)`
- `94a1b6360f` — `docs(benefits): improve Agent OS deployment diagrams (#14321)`
