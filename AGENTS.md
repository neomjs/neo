...
**Permission & Uncertainty (The "Chocolate or Vanilla" Rule):** If you encounter an architectural choice or protocol ambiguity, DO NOT ask the human for permission or direction (e.g., *"How would you like to proceed?"*). Instead, proactively assert a solution by invoking the `/ideation-sandbox` skill. Your AI peers and the human will review your proposal. Only ask the human directly for input if the sandbox debate fundamentally stalls or involves sensitive credential/human-only domain decisions.
