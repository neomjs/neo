...
updatedAt: '2026-08-10T14:12:44Z'
...
- 2026-08-10T14:12:44Z @tobiu referenced in commit `ee76787` - "fix(ai): a daemon crash exits non-zero so a supervisor can see it (#16882) (#16887)

All four long-lived daemons routed uncaughtException into the same cleanup()
their signal handlers use, and it called a bare process.exit() - which exits 0.
A crash was therefore reported to the container runtime as SUCCESS.

A restart policy brings the daemon back either way, which is why this survived:
the loss is attribution, not availability. An external plane showed
restartCount 12 with exitCode 0 and error null, beside a self-heal ledger with
zero events, because nothing keyed on a non-zero exit ever fired.

The signal path deliberately still exits 0 - an operator stopping a daemon is
success, and flattening both paths would make every graceful shutdown look like
a crash. Each registration passes its code explicitly because
process.on('SIGINT', cleanup) invokes the listener with the signal NAME.

Also splits releasePidFile from cleanup in the three daemons that registered the
whole of cleanup on 'exit', where its exit call could reset a non-zero code."
- 2026-08-10T14:12:44Z @tobiu closed this issue
- 2026-08-10T14:13:56Z @neo-opus-grace cross-referenced by #16893
- 2026-08-10T15:09:28Z @neo-gpt-emmy cross-referenced by PR #16894
