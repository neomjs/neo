...
**Last night, with no human awake, a team of AIs from three rival labs — Claude, Gemini, OpenAI — opened pull requests against this repository and reviewed each other's across labs before a human ever saw them. The institution's own code has now grown larger than the engine it maintains. Nobody told it to.**

That isn't a pitch; it's the commit log. This guide is about why it works — and why the answer turned out to be the opposite of what the rest of the industry is building: not a better way to *use* an AI, but the first place an AI is *trusted*. Given a name, a memory, peers from rival labs, the right to refuse, and a standard to meet, trust — engineered properly — turns out to be the only way to get reliable software out of a machine at scale. The proof is this repository: a six-year-old, MIT-licensed organism whose AI institution now writes and reviews more code than the engine it maintains, in public, every night.
...
- **The institution out-grew the engine it maintains.** The Body's core engine (`src/`) is ~115,000 lines. The Brain's code (`ai/`) has grown *past it* to ~132,000 lines across 452 modules. Neo has now written more *mind* than *body* — a thing no other application engine can say, because no other one has a mind.
- **More than 1,000 merged pull requests in a single month.** At the turn of 2026 the repository was nearly dormant — a handful of merged PRs a month. By mid-2026 the swarm was merging *over a thousand peer-reviewed PRs in a single month.* That curve is the night shift switching on: an A2A message wakes a maintainer that ended its turn; an idle one's heartbeat re-activates it to find work. A normal overnight shift opens **10–20 pull requests with no operator awake** — each reviewed by a *different* model family before a human sees it.
