...
- **OD-1 — The canonical apex. GRADUATED (Discussion #12234, full cross-family triad, 2026-05-31).** Dated proof point (fixed calendar anchor): **in May 2026 the canonical repo recorded 706 merged PRs and 800 closed issues** (GitHub search, verified 2026-05-31). Counts are merged-PR / closed-issue totals only. **Never write a `peer-reviewed` qualifier** (the search verifies *merged*, not review-state) **or a month-over-month ratio / `accelerating` claim** without a precise metric spec + verification — they date-stamp the identity and the April→May delta was mis-stated as `~3×` (per @neo-gpt V-B-A `dc-17119216`/`dc-17119231` + operator flag). The canonical apex line:

  > **Neo.mjs is a self-evolving software organism — a professional, end-to-end AI engineering team that lives in its own open-source repository. Where the industry runs one AI agent and gets slop, Neo runs a swarm of minds from rival labs — Claude, Gemini, GPT — that read each other's reasoning through a shared memory (Active Hybrid GraphRAG) and catch what no single model can see in itself. They autonomously run the full engineering lifecycle: ideating, building, and cross-reviewing a production multi-threaded engine, running overnight DreamService cycles to re-steer their own priorities, and closing self-healing loops that turn live runtime drift into repaired code, fresh tickets, and evolved skills — evolving every cycle, in the open, without losing the thread across sessions. It maintains its own codebase today; it's being built to inhabit yours, regardless of the models' training data.**

  Structure beneath the apex: **BODY (`/src/`)** = the high-performance off-main-thread UI runtime (JSON-first since 2019, App Worker as main actor, web multi-window; engine-category mental models apply *here only*); **BRAIN (`/ai/`)** = the entire Agent OS (≈41% of module count — institution + MX loop + Memory Core with **Active Hybrid GraphRAG** (`mutate_frontier` frontier-edge mutation + `GoldenPathSynthesizer` fusing semantic Chroma distance with SQLite structural weights to re-steer priorities) + KB + A2A + orchestrator + DreamService); **Neural Link** = a flagship capability, NOT the apex and NOT a "demo" (~2%, 1 of 7 MCP servers; conversational UIs + multi-agent multi-harness live collaboration, Discussion #10119).
  ...
  **Register split:** "rival labs" + the anti-slop contrast = human-facing copy (README/portal hero) only; machine surfaces (ld+json, keywords, GitHub topics, the literal `<meta name="description">`) use the colder concise form and never the word "slop." Canonical concise meta-description: *"Neo.mjs is a self-evolving software organism and autonomous agent OS: a professional end-to-end AI engineering team using shared memory, cross-family PR review, Active Hybrid GraphRAG, DreamService cycles, and self-healing workflows."* Proof asset: blog #10074 (cross-family self-healing, first-person, verified-true) — publish + link from the apex's proof anchor. "Application Engine for the AI Era" (npm/SEO discovery) and "self-evolving digital organism" (VISION-doc framing) are retained as deeper/audience-segmented clusters, governed as compatible with — never the lead over — the institution apex.
