...
updatedAt: '2026-08-21T17:52:03Z'
...
- 2026-08-21T17:52:03Z @tobiu referenced in commit `da19ff6` - "fix(build): escape backslash and quote in template string chunks (#17484) (#17485)

`part.replace(/'/g, "\'")` is an identity replacement — `"\'"` in a
double-quoted JS string is a bare `'` — so static template chunks were
wrapped in single quotes without ever being escaped. The comment said
"Escape single quotes"; the code replaced each apostrophe with itself.

An apostrophe in static text adjacent to an interpolation emitted
`'it's ' + (count) + ' here'`, a SyntaxError. Backslashes were unhandled
too, and are the more dangerous half: a trailing `\` escapes the closing
delimiter, while an interior `a\b` is reinterpreted as the `\b` escape and
silently becomes a backspace — that one parses, so it reaches production.

Both call sites — the text-node chain and the attribute chain — now route
through one `toQuotedStringLiteral` helper that escapes backslashes before
quotes. The defect existed in duplicate because the expression was inlined
at both sites; sharing the helper is what stops a third chain inheriting it.

Coverage asserts the emitted expression *parses* and *round-trips*, not
that it matches a string — a string-equality assertion greens for any
consistent-but-wrong escaping, including the identity replacement itself.
Mutation diagonal: reverting the text site reddens its 4 arms and leaves
the attribute arm green; reverting the attribute site reddens only its own.

Closes CodeQL alerts 41 and 42 (js/identity-replacement)."
- 2026-08-21T17:52:04Z @tobiu closed this issue
