...
description: Closeout protocol for parent epics — answers "we resolved all epic subs, are we done now?" with a structured matrix + verdict recommendation (close / close partial with follow ups / keep open / create missing subs / retire-supersede). Sibling to epic-review (which handles entry); this is the exit gate.
