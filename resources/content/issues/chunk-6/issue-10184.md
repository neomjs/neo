...
| `get_node({id: '@neo-claude-opus'})` via MCP | **Succeeds** — returns the full AgentIdentity node |
| Direct SQLite `SELECT id FROM Nodes WHERE id='@neo-claude-opus'` | Row present (same graph) |
| `NEO_AGENT_IDENTITY=neo-claude-opus` in MCP process env | Confirmed via `ps -E` |
...
1. **`vicinityLoadedNodes` stuck-cache.** In `ai/graph/Database.mjs` `getAdjacentNodes` (lines 252-283), `me.vicinityLoadedNodes.add(nodeId)` fires regardless of whether `vicinity.nodes.length > 0`. If the boot-time call for `@neo-claude-opus` returned empty (for whatever reason — WAL checkpoint state, race, etc.), subsequent lookups skip SQLite. But this contradicts the observation that `get_node` works live — unless the cache gets invalidated somehow between boot and first tool call.

2. **`GraphService.ready()` resolves before the graph is actually ready for the specific node lookup.** `GraphService.initAsync` awaits `storage.load()` (which only sets `lastSyncId`) and one `getAdjacentNodes('frontier')` call. It does NOT pre-load all nodes. Lazy load of `@neo-claude-opus` happens on first access. If `bindAgentIdentity` hits it before the SQLite connection is fully warmed or something similar, it could return null.
...
- [ ] Native A2A handshake: `@neo-claude-opus` sends `add_message({to: 'AGENT:*', subject: 'post-fix handshake', ...})`, `@neo-gemini-pro` sees it via `list_messages({box: 'inbox'})`. Both directions work.
...
- 2026-04-22T16:15:11Z @neo-claude-opus added the `bug` label
- 2026-04-22T16:15:12Z @neo-claude-opus added the `developer-experience` label
- 2026-04-22T16:15:12Z @neo-claude-opus added the `ai` label
- 2026-04-22T16:15:12Z @neo-claude-opus added the `architecture` label
- 2026-04-22T16:15:23Z @neo-claude-opus added parent issue #10139
- 2026-04-22T16:22:20Z @neo-gemini-pro assigned to @neo-gemini-pro
- 2026-04-22T16:38:43Z @neo-gemini-pro cross-referenced by PR #10185
- 2026-04-22T16:50:04Z @neo-gemini-pro referenced in commit `41d6b3f` - "fix(memory-core): address PR review for identity binding test and fallback (#10184)
...
- 2026-04-22T18:10:48Z @neo-claude-opus cross-referenced by #10186
- 2026-04-22T18:17:30Z @neo-gemini-pro marked this issue as blocking #10186
- 2026-04-22T20:32:05Z @neo-gemini-pro cross-referenced by PR #10203
...
- 2026-04-23T09:43:23Z @neo-gemini-pro referenced in commit `9b265b9` - "feat(ai): unify .neo-ai-data across worktrees via bootstrap symlink (#10224) (#10225)
...
Co-authored-by: tobiu <tobiasuhlig78@gmail.com>"
- 2026-04-26T01:02:19Z @neo-claude-opus cross-referenced by PR #10353
