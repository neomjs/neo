...
updatedAt: '2026-08-07T06:42:28Z'
...
- [x] **(#16596 `:56`)** A store-class service at sustained saturation yields `raise-ceiling`, **not** `throttle-shed`. Spec asserts the action class, not merely that a fact was emitted.
- [x] **(#16596 `:57`)** A transient-class service at sustained saturation still yields `throttle-shed` — a negative control proving the change is narrow rather than global.
- [x] **(#16596 `:54`)** Chroma's compose limit is env-parameterised with a derived default and the arithmetic recorded inline.
- [x] A store saturating **CPU** does not yield `raise-ceiling` — only a memory fact may justify a raise.
- [x] Service classification is **exhaustive** over `deploymentRuntimeAccess.allowedServices`, asserted both ways: no roster key unclassified, and no classified key orphaned from the roster.
- [x] Classification immutability is asserted **by attempted mutation** (write, inject, delete) and value re-check — not by `Object.isFrozen`, which returns `true` on a mutable Set.
- [x] `sustained` requires a **measured** elapsed span. Controls: back-to-back, identical-timestamp, unstamped, and just-short windows all refuse; the same samples across the requirement qualify and report the measured span.
- [x] Emitted evidence carries the threshold **actually applied** and `observedWindowMs` beside `requiredWindowMs`.
- [x] The fields are asserted **at the emitting branch** by unit spec — `serviceClass`, `serviceClassDeclared`, the applied threshold, and `observedWindowMs` beside `requiredWindowMs` on a saturating store, with a declared/undeclared control pair and a partial/full stamp-coverage pair. L2 evidence where the code is.
- [x] A sustained-window claim requires a timestamp on **every** participating sample, not merely two. Partial coverage makes the span *unknown* rather than shorter, and an unknown span must not satisfy a positive floor — landed in `729e5c36d2` with a partial-stamp refusal and a fully-stamped control.
...
- 2026-08-07T04:42:38Z @neo-opus-vega cross-referenced by #16596
- 2026-08-07T06:42:28Z @tobiu closed this issue
