...
updatedAt: '2026-08-08T12:44:53Z'
...
- 2026-08-08T12:42:27Z @tobiu referenced in commit `cdad885` - "fix(ci): the C3 test-overlay lint fires on the files it scans (#16628) (#16678)"
- 2026-08-08T12:42:27Z @tobiu closed this issue
### @neo-fable - 2026-08-08T12:44:52Z

## Close-out ledger (merged @ `cdad885348`, PR #16678)

Delivered exactly the re-scoped body: the workflow's watched set is now a superset of the lint's scanned surfaces on both triggers (`ai/**/*.mjs` + `test/**/*.mjs`, invariant documented in-file), and ADR-0019 §3/§7 record the shipped C3 coverage (`[guarded: …]` tag + the test config-authority rule in the coverage list). AC receipts: synthetic reintroduction red-proven locally (exit 1, `FAILED - 1 test config-authority violation(s)`; green exit 0 after removal); the introducing-PR trigger reach was reviewer-verified (@neo-gpt's approval verified both `paths:` blocks against the lint's two declared scan roots) and self-demonstrated (the SSOT lint workflow ran green on PR #16678 itself); ADR truth-fold shipped in the same commit. The intake inversion is the lane's durable lesson: the original "7 re-accumulated instances" premise was a string-grep artifact — census-corrected on #16515 with the method note (`rg -l` answers "mentions", never "does"), and the recurrence vector (documented-not-enforced invariant, Vega's Depth-Floor) lives on as #16684 (pool). Cross-family review: @neo-gpt Approve, zero RAs; @neo-opus-vega non-gate corroboration with a live same-day specimen.
