...
### 2.3 The four sub-decisions (each a map-infidelity, with its fix)
...
**V-B-A note (do not enshrine unverified):** the curated-59 count is verified (`nodes.jsonl` = 59 lines = 59 `.md` files, 1:1) + the gap-inference weight-gate (`GapInferenceEngine.guideGapWeightThreshold`); the ADR-node embedding is **roadmapped, not built** (the 23 ADR nodes are un-embedded). **Open V-B-A:** whether `nodes.jsonl` is generated from the `.md` content SSOT or independently maintained — the 1:1 count suggests sync, but the generation mechanism is unverified (drift risk; the `content-should-be-SSOT` redundancy is the tell).
