...
1. **Canonical apex** — one framing is the source of authority for "what Neo is" (recommended: the `README.md` self-evolving-digital-organism / Four-Pillars frame; see §2.7 OD-1).
...
- **OD-1 — The canonical apex.** *Recommendation:* the `README.md` "self-evolving digital organism / Four Pillars (Brain / Swarm / Body / Evolution)" frame is THE apex all framing coherence-checks against. The "Application Engine for the AI Era" cluster stays as a deliberate discovery-optimized segment (engineers searching npm don't search "organism") — kept, governed as a coupled cluster, **not** pulled up to "organism".
...
- **OD-3 — The structuring metaphor.** README uses "Four Pillars"; `ArchitectureOverview.md` uses "Two Hemispheres"; both are deliberate. *Recommendation:* Four Pillars is the canonical top-level scaffold (it's in the apex); Two Hemispheres is a valid engineering-altitude *projection* (Body + Brain = the two hemispheres; Swarm + Evolution are cross-cutting). Document the mapping so they stop reading as competing.
