...
| Approving substrate touching multi-loaded agent-memory files (`AGENTS.md`, `.agents/skills/**/*.md`, `.claude/CLAUDE.md`, `.codex/CODEX.md`, `.agents/ANTIGRAVITY_RULES.md`) by FILE-COMPLETENESS dimension only without auditing RUNTIME-LOAD EFFECT (does the content load once or twice per turn?) | **Loading-runtime-effect substitution** — distinct from rubber-stamping: DIMENSION failure (effect-surface unaudited) not ENGAGEMENT failure (content-surface was reviewed). Substantive feedback can be given across multiple cycles while the load-effect dimension stays invisible. Empirical anchor: PR #11244 (2026-05-12) — 3 reviewers (Gemini-author + Opus Cycle 1+2 + GPT Cycle 1+2) missed runtime-load duplication across 4 review cycles + 2 fix cycles = 6 total; operator surfaced via direct V-B-A on Gemini's MCP `context.fileName` config + Codex hook script (`.codex/hooks.json` + `.codex/hooks/codex-context.mjs`). Proactive companion: `/turn-memory-pre-flight` (Epic #11256) fires at substrate-creation time. Reviewer mechanical pre-flight: `cat .codex/hooks.json` + `cat .codex/hooks/codex-context.mjs` + check harness MCP `context.fileName` config + `readlink .claude/CLAUDE.md` — then mentally execute per-turn loading effect. |
