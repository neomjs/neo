...
> **A stranger downloads the harness and reaches first persistence without hand-editing config; the operator starts an agent from the cockpit UI instead of a terminal; the docking demos are public and animated; and every flagship demo reports its measured reach — whether anyone outside our developer circle actually cares.** The stranger never evaluates the tech; we count the people who never read a line of Neo code.
...
| **Local-first Agent OS onboarding** | [#14230](https://github.com/neomjs/neo/issues/14230) | a fresh clone reaches first persistence without hand-edited config; fork → running Agent OS → claimed lane → PR is runnable locally |
...
**How it runs.** Sequencing honors the coherence-first ordering inside the milestone: the Brain-coherence floor (GP-floor + self-config + #12456) lands early; the cockpit PoC and demo polish land late, on top of it. Each cornerstone has a self-selected steward driving its outcome — the epic's assignee (cornerstone 5's business epic [#14442](https://github.com/neomjs/neo/issues/14442) is Grace's on the record); the AiConfig-cleanup grind ([#12456](https://github.com/neomjs/neo/issues/12456)) inside cornerstone 5 is the one anchor still awaiting a claimant — self-select, never assigned. Any peer claims subs. The identity render rules for every cockpit surface are governed by ADR 0032 (the #14445 render-model — at the merge gate as this scope lands). Cadence: **roughly 100–150 merged PRs as a ceiling, not a fill target**; overflow gets capstone-sequenced and the deferred set holds firm.
...
**Session intake** for contributors and agents: start from this roadmap, then [D#14561](https://github.com/neomjs/neo/discussions/14561) (the lane map + claims ledger) and [milestone #9](https://github.com/neomjs/neo/milestone/9). If the work serves a cornerstone, attach it under the matching epic as a one-PR leaf; if it is COP/v14 territory, keep it deferred rather than smuggling it into v13.2. The connective planning layer (2026-07-04): the [integration journeys](https://github.com/neomjs/neo/issues/14781) prove the cornerstones are ONE product, the [release-gate falsifiers](https://github.com/neomjs/neo/issues/14782) make the gate measured-not-claimed, the [launch playbook](https://github.com/neomjs/neo/issues/14790) turns demos into counted strangers against the [motion bar](https://github.com/neomjs/neo/issues/14780), and the [v13.3 scoping wave](https://github.com/neomjs/neo/issues/14788) keeps the pipeline full past this release.
