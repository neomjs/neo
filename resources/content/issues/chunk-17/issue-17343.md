...
updatedAt: '2026-08-18T12:18:20Z'
...
- 2026-08-18T11:57:34Z @neo-opus-vega referenced in commit `b998272` - "fix(ai): an unresolvable band refuses to split instead of cutting to the sibling (#17343)

@neo-opus-grace found that the contract this PR argues for was not honoured by
its own splitter. The `?? guardrail.safeProcessingLimitTokens` fallback I added
meant a declared-invalid ceiling cut against the 28,672 band admission had just
stopped trusting — the exact value the change exists to remove, restored
silently on the one path where nothing was left to validate it.

Fixed at the source rather than only at her call site. splitOversizedEmbeddingChunk
now returns the chunk whole when the band does not resolve: a split planned
against an unresolvable band is planned against nothing, and whole is the only
honest output, since the callers' unmeasurable branches and the pre-invocation
boundary behind them refuse it with a reason attached.

Adds the symmetry she asked for in IngestionService.filterEmbeddingInputBudget,
which routed an unresolved band straight into the splitter while
VectorService.expandOversizedEmbeddingChunks already guarded the same case. An
unresolvable band is a configuration defect, not an oversized chunk; it now
records the oversized skip and leaves the chunk whole.

Spec case added per RA-1, and mutation-verified rather than merely green:
restoring the `??` fallback makes the new case FAIL while its siblings pass, so
it discriminates against her defect specifically. 449 specs across the 23 files
importing the three changed modules pass."
- 2026-08-18T12:09:12Z @neo-opus-vega cross-referenced by #17349
- 2026-08-18T12:18:20Z @tobiu referenced in commit `6fe1067` - "fix(ai): admission keys on the engine slot, in the unit the engine counts (#17343) (#17347)
...
* fix(ai): an unresolvable band refuses to split instead of cutting to the sibling (#17343)
...
it discriminates against her defect specifically. 449 specs across the 23 files
importing the three changed modules pass."
- 2026-08-18T12:18:20Z @tobiu closed this issue
