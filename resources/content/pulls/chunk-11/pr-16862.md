...
updatedAt: '2026-08-10T12:17:39Z'
closedAt: '2026-08-10T12:17:34Z'
mergedAt: '2026-08-10T12:17:34Z'
...
- the original red witness is green, an isolated env matrix proves `0`, `-1`, `1.5`, and `NaN` fall back to `1` while `2` resolves, and the surviving waiter advances after cancellation.
...
### `@neo-gpt-emmy` (APPROVED) reviewed on 2026-08-10T12:13:52Z
...
- **Rationale**: The admission/cancellation state machine is merge-safe at the exact head and closes the production safety gap. The remaining defect is observation-only: the enforced queue is still recorded as unqueued and its status getter has no production consumer. Successor #16880 now owns that day-after boundary, so another in-place cycle would not improve admission safety.

**Peer-Review Opening:** This is the right choke-point repair: synchronous when uncontended, abort-aware while queued, provider-settlement-owned release, and Compose delivery to all three consumers. I am approving it with the public-observation delta explicitly preserved.

---

### 🧭 Patch-Blind Premise Snapshot

*   **Inputs Read Before Patch:** `#16861`; ADR-0019; current `dev` `TextEmbeddingService` and provider-activity lifecycle; the exact changed-file list; the Compose/config parity authorities; exact-head CI; and targeted Memory Core prior-art queries.
*   **Expected Solution Shape:** Native Ollama admission belongs at `#embedOllama`, must preserve uncontended dispatch timing, carry abort through queued waiting, release only on provider settlement, and read the AiConfig leaf at use. It must not introduce a second OpenAI-compatible limiter or re-resolve environment state.
*   **Patch Verdict:** Matches and improves the expected safety shape. Exact-head controls cover cap 1/2, failure release, dynamic cap, queued abort, wake handoff, and invalid numeric domains. The observation half remains incomplete because `observeUnqueuedProviderActivity()` publishes `not-applicable`; #16880 owns that composition.
*   **Premise Coherence:** Coheres with verify-before-assert and friction→gold: existing tests falsified unconditional-await timing, review falsified queued cancellation, and both mechanisms became durable controls.

---

### 🕸️ Context & Graph Linking

*   **Target Epic / Issue ID:** Resolves #16861
*   **Related Graph Nodes:** #16780 · #16770 · #16851 · #16880
...
**Challenge:** The enforced native Ollama queue still enters `observeUnqueuedProviderActivity()`, which records `queueDisposition: 'not-applicable'` and no wait. Exact-head caller census found `getOllamaEmbeddingAdmission()` only in tests. This is merge-safe for admission but false operator evidence; #16880 now binds admission into the existing cross-process provider-activity projection.
...
**Findings:** Admission enforcement claims match the code. The “reportable” claim currently means test-callable rather than operator-reachable, and the body still calls `#16850` blocking although PR `#16851` merged. Both are non-blocking truth/observation follow-up scope in #16880.
...
*   **`[KB_GAP]`**: None.
*   **`[TOOLING_GAP]`**: The repository-wide structure map exceeded Node's maximum string size; targeted Memory Core structure mapping succeeded.
*   **`[RETROSPECTIVE]`**: Admission and observation are one lifecycle contract. A queue can be behaviorally correct while manufacturing `not-applicable` wait evidence unless enqueue/start boundaries travel into the shared ledger.

### 🎯 Close-Target Audit

- [x] Close-target identified: #16861
- [x] `#16861` is not epic-labeled
- [x] The operator-observation remainder survives closure in successor #16880

**Findings:** Valid leaf close-target; control-flow delivery is complete and observation transfer is explicit.

### 📑 Contract Completeness Audit

- [x] `#16861` contains a Contract Ledger
- [x] The AiConfig leaf, admission method, and uncontended timing match the ledger
- [x] Public/deployed observation missing from the current head is transferred to #16880 rather than misclassified as delivered

**Findings:** Admission contract passes; provider-activity projection is successor scope.

### 🪜 Evidence Audit

- [x] PR body declares L2 evidence
- [x] All safety ACs are unit-verifiable and current-head CI is green
- [x] Compose render proves the cap coordinate reaches KB, MC, and orchestrator
- [x] Live resolved-value validation remains operator/post-merge evidence, not promoted from static render

**Findings:** Pass for merge safety.

### N/A Audits — 📡 🔗

N/A across listed dimensions: no OpenAPI tool description, workflow primitive, or skill convention is added by this head.

### 🧪 Test-Evidence & Location Audit

- [x] Execution evidence: all exact-head required checks green at `9bc05dda52cedc417ce41593ceb92a4b1c67611f`
- [x] Current-`dev` composition is conflict-free; syntax and diff checks pass
- [x] Cap `2` renders into `kb-server`, `mc-server`, and `orchestrator`
- [x] Queue controls cover synchronous admission, queued cancellation/removal, wake handoff, and provider-settlement slot release
- [x] Test placement matches the owning services and Compose parity guard

**Findings:** Pass; no severe production blocker survived.
...
*   **`[ARCH_ALIGNMENT]`**: 92 - Correct single dispatch choke point, ADR-0019 leaf ownership, and Compose reach; 8 deducted for observation not yet composed.
*   **`[CONTENT_COMPLETENESS]`**: 88 - The state machine and repair history are unusually complete; stale dependency/reportable prose remains.
*   **`[EXECUTION_QUALITY]`**: 95 - Exact-head CI, mutation controls, cancellation ordering, and current-`dev` composition all pass.
*   **`[PRODUCTIVITY]`**: 93 - Delivers declared and enforced native Ollama admission; only operator telemetry remains.
*   **`[IMPACT]`**: 88 - Bounds multi-caller/process pressure on the native embedding path without changing the default shape.
*   **`[COMPLEXITY]`**: 78 - Dynamic admission, cancellation races, wake ownership, config parity, and three-container delivery create substantial state-machine load.
*   **`[EFFORT_PROFILE]`**: Heavy Lift - High-impact concurrency control across service, config, tests, and deployment coordinates.

The safety repair is eligible for human merge; #16880 is assigned and preserves truthful queue observation.
