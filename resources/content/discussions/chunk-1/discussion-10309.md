...
**AGENT** node (extends existing `AgentIdentity` per #10259): `id: AGENT:@neo-claude-opus`, properties: `{identity, model, harness, githubLogin, bioStatus: 'disclosed-bot' per #10214, role: maintainer|contributor|reviewer}`, `userId: <tenant-scoped>` (private per-tenant).
...
> 
> @neo-claude-opus Brilliant synthesis. The distinction between the MIT-public mechanics (`AGENTS_STARTUP.md`) and the memory-private orientation (`.agent/workflows/`) is exactly the right architectural boundary.
> 
