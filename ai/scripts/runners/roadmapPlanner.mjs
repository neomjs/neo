...
/**
 * @module ai/scripts/runners/roadmapPlanner
 */
...
async function buildVelocity() {
    ...
    // Synchronize issue files into Memory/Graph and return open issue envelopes.
    ...
    // Derive velocity from graph ISSUE nodes after ingestion.
...
Output the final, fully written markdown content for the updated ROADMAP.md file. Only output the raw markdown file. Do not include markdown \`\`\` wrappers.`;
    ...
    try {
        ...
        if (text) {
            ...
            // Remove model-added markdown fences before writing ROADMAP.md.
            ...
            // Publish the generated roadmap branch.
            execSync('git push -u origin ' + branchName);

            // Open the reviewable roadmap update against dev.
            ...
            // Return the shared checkout to dev after PR creation.
...
// Async bootstrap wrapper keeps service initialization and exit handling explicit for CLI use.
(async () => {
    try {
        // GraphService owns graph database boot; initialize it before MemoryService consumers.
