...
updatedAt: '2026-08-11T13:04:58Z'
...
- 2026-08-11T12:11:35Z @neo-opus-ada referenced in commit `b7f3572` - "fix(knowledge-base): a failed vector write no longer re-buys identical embeddings (#16963)

The provider call and the upsert shared one `try`, so a PERSISTENCE failure sent the retry back
through the PROVIDER and bought the identical vectors again. Same texts, same model, same result
- the second purchase cannot differ, it can only be charged.

`embeddings ??= await TextEmbeddingService.embedTexts(...)` caches a RESULT, never a failure:
when the await throws, the assignment never happens and the next lap re-attempts the provider.
The yield arm below already refused to discard paid work; this is the same principle on the
ordinary failure path, which never had it.

Two controls ship with it, because the repair is only safe if both hold:

- A failed provider call must leave nothing cached, or one transient blip becomes a permanently
  unembeddable batch - strictly worse than the redundant work removed.
- Exhausted write retries must not report false progress. I wrote that control expecting
  {embedded: 0} and the suite corrected me: the contract REJECTS. Preserving current failure
  accounting means preserving the throw, not softening it into a zero-count success a caller
  could read as an empty batch.

Measured: 3 upsert attempts over 2 texts produced 6 provider submissions; now 2.

AC2 receipt, run with the AC1 test ISOLATED because an earlier failure aborted the run before it
executed - the first attempt looked like a negative result and was a blinded one: moving the
provider computation back inside the write retry loop yields Expected 2, Received 6.

Verified against dev rather than trusted from the parent's ledger ticks: #16867 (AC-1/AC-3/AC-6)
did not cover this path."
- 2026-08-11T13:04:58Z @tobiu referenced in commit `d25c397` - "fix(knowledge-base): a failed vector write no longer re-buys identical embeddings (#16963) (#16964)
...
Verified against dev rather than trusted from the parent's ledger ticks: #16867 (AC-1/AC-3/AC-6)
did not cover this path."
- 2026-08-11T13:04:58Z @tobiu closed this issue
