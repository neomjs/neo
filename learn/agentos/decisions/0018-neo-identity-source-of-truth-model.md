...
| MCP-server count | frontier | `package.json` scripts matching `ai:mcp-server-*` (NOT a naive `ls ai/mcp/server/` — that returns 7: it must exclude `shared/` infra **and** `gitlab-workflow/` PoC, yielding 5) | derive into prose/JSON via build or lint coherence-check; the script manifest is the single place the exclusion rule lives | 11 surfaces understate to 4/3/2 |
