...
updatedAt: '2026-08-07T18:01:01Z'
closedAt: null
mergedAt: null
...
The `ai/services.mjs` barrel is the canonical SDK entry point, and it could not be loaded in the Body install tier: two eagerly-imported leaves reached `chromadb`, which ships only in the Brain tier. Merely *importing* the SDK required a package that is only needed once something talks to Chroma. That took the hourly Data Sync stage down for twelve consecutive runs, froze `resources/content/issues` and `discussions`, and was bridged by the deliberately temporary, self-expiring SDK-boundary exception (#16495 / PR #16496).
...
- `ready()` no longer implies a client, so every reader that relied on that now ensures.
...
1. **The ticket's own table was wrong.** It lists `chromaClientPrimitives.mjs` as *"deferrable as-is, cleanly"*; both managers called that function at **module scope**, so the barrel still reached `chromadb` through it. Deferring required making it async and awaiting all 7 sites.
2. **The ticket's premise had decayed** — its title claimed Data Sync *"still cannot run"* with 12 consecutive failures; the pipeline had been green for days behind the #16495 exception. Truth-folded in place with a marked correction.
3. **`initAsync()` deferral is NOT demand-lazy for an eager singleton.** `Neo.setupClass()` instantiates the singleton at module load and `core.Base` schedules `initAsync()` on the very next microtask. An earlier revision of this PR moved the import there and **the failure simply changed phase**. Caught by @neo-gpt's cycle-1 falsifier, not by me.
4. **"No consumer of `ai/services.mjs` was modified" was FALSE** in an earlier revision of this body. `ai/examples/smart-search.mjs` imports the barrel and is changed here. The ticket's AC saying no consumer changes is therefore not met as literally written; the export surface is unchanged, which is the property that actually matters, and the ticket is folded to say so.
5. **The managers' own `this.client` reads were not safe "by construction"** — another claim stated as fact in an earlier revision. Integration CI caught it. The enumeration missed them because readers were found by grepping `ChromaManager.client`, so every internal `this.client` read was invisible to a search meant to be exhaustive.
6. **7 pre-existing trailing-whitespace lines** stripped in `smart-search.mjs`; the whitespace lint is file-scoped. `git diff -w` confirms whitespace-only.
7. **One out-of-scope one-line repair:** `lifecycleGuardPath` was called at four sites in `heavyMaintenanceLeasePrimitives.mjs` and never imported — pre-existing on `dev`, fixed because it hard-blocked #16619's required witness. Splittable on request.
...
npm run test-unit -- test/playwright/unit/ai/services/knowledge-base/ \
                     ...
                     test/playwright/unit/ai/services/bodyTierBarrelRuntime.spec.mjs \
                     test/playwright/unit/ai/mcp/server/shared/services/DestructiveOperationGuard.spec.mjs
  532 passed   (3 consecutive runs)
```

**The runtime witness is the new evidence, and it goes RED against the previous head.**

| probe | result |
|---|---|
| `chromadb` denied → import barrel, wait through singleton init | **SURVIVED** |
| same, at `ef6a182a44` (the reviewed head) | **RED** — resolved `chromadb` during eager boot |
| `chromadb` denied → `defragChromaDB.mjs` | hits the denial (**positive control**) |
| full Brain-only set denied → barrel | fails on `better-sqlite3` (**KNOWN STATE**, asserted as such) |

A static reachability walk cannot observe any of this, and full-install CI cannot reproduce it — which is exactly why the retired sunset spec passed while the runtime property was false.

**Scope boundary.** I have not run the broader `ai/` suite from this worktree: `backup.spec.mjs` false-greens here for the reason #16617 documents. The `memory-core` tree is ambiently flaky independent of this diff — 5 failures on clean `origin/dev`, 4 on this branch, different sets, my one extra passing 3/3 in isolation.
...
**Three claims in this PR were stated as fact and were false**, each caught by evidence rather than by me: that the managers' own reads were safe by construction (integration CI); that moving the import into `initAsync()` deferred it (@neo-gpt's runtime falsifier); and that no barrel consumer changed (his contract audit).

The common shape is one thing: **a property was measured at one level and asserted at another.** A static import walk establishes static reachability and nothing about runtime. A grep for `Manager.client` establishes external readers and nothing about internal ones. Both measurements were sound; both conclusions overshot them. The runtime witness added here exists because that is the level the claim actually lives at.
