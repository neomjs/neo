...
updatedAt: '2026-08-15T13:33:15Z'
...
- 2026-08-15T13:33:15Z @tobiu referenced in commit `f672ccb` - "fix(test): a p95 over five samples is the maximum — refuse the statistic the sample cannot support (#17172) (#17174)

* fix(test): a p95 over five samples is the maximum — refuse the statistic the sample cannot support (#17172)

`assertSustainedHealth` reddened an unrelated PR with `p95 latency should be <= 500ms / Received: 501`
on a diff that touches only `ai/services/github-workflow/**`. The change and the failing plane share
no surface.

The formula is textbook nearest-rank and the original author implemented it faithfully. The defect
is where it is applied: `ceil(0.95n) - 1 === n - 1` for every n <= 19, so below twenty samples the
"p95" IS the maximum. `healthcheck.spec.mjs` runs a 5s/1s window — five samples — so a budget whose
entire purpose is tolerating a slow tail was forbidding one, and a single 501ms probe failed the
build for the whole team.

Interpolating would not have helped: a linear p95 of five samples still lands on the largest of
them. Five samples do not contain a 95th percentile under any definition, so the helper now says so
instead of returning the max wearing a p95 label:

- `MIN_PERCENTILE_SAMPLES = 20`, derived from the 19->20 transition rather than chosen for
  roundness, and asserted from both sides so it cannot drift.
- `p95Ms` below that count throws, naming the sample count and the two remedies. A guard that
  refuses without saying what to do instead gets routed around.
- `maxMs` bounds the slowest probe, valid at any n — the statistic a short window CAN support.
- `percentileIndex` is exported so the degeneracy is assertable as arithmetic; an implementation
  that quietly returns `n - 1` cannot satisfy the pinned table.

The 5s/1s call site now asserts `maxMs` with a generous liveness ceiling and `p95Ms: null`. Its own
name calls it a composability check — it exists to prove two probes compose under `Promise.all`,
not to hold a latency SLO, and it should never have gated the merge queue on a 500ms tail.

The two HeartbeatPropagation call sites run at the n=30 default, where the p95 is genuine
(index 28, one sample tolerated). Reviewed and deliberately unchanged. `NEO_INTEGRATION_SUSTAINED_*`
is set nowhere in the repo, verified by grep, so no environment shortens those windows into the
refusal path.

18 unit arms, red-proved three ways: generalizing the degeneracy fails 4, removing the refusal
fails 2, drifting the constant to 25 fails 2.

Co-Authored-By: Neo Opus Vega <uhlig@klarso.com>

* fix(test): the summary refuses the p95 too — reporting it below the sample floor republished the defect as data (#17172)
...
Co-authored-by: Neo Opus Vega <uhlig@klarso.com>"
- 2026-08-15T13:33:15Z @tobiu closed this issue
