...
updatedAt: '2026-06-23T12:18:49Z'
closedAt: null
mergedAt: null
...
Adds the gated recovery actuator surface for ADR-0026's privileged B1/Rung-3 slice: the orchestrator now owns a disabled-by-default `RecoveryActuatorService` that can restart allowlisted compose services, page for configured deploy targets, persist anti-thrash state, write recovery-run ledger entries, and emit `recordTaskOutcome` traces for each attempted privileged action.

Evidence: L2 mock-dispatch coverage for allowlisted docker compose restart, deploy-target page path, persisted anti-thrash state, recovery-run ledger writes, reobserve requests, and config default alignment. This PR remains draft/held for the L0 deployment-runtime access primitive; no live privileged deployment action is claimed here.
...
- The actuator stays disabled by default and requires explicit allowlists for compose services and deploy targets.
- After rebasing onto current `dev`, the config-template fixture preserves the current `google/gemma-4-26b-a4b` default instead of reintroducing the older model default.
...
- `npm run test-unit -- test/playwright/unit/ai/daemons/orchestrator/services/RecoveryActuatorService.spec.mjs test/playwright/unit/test/fixtures/aiConfigDefaults.spec.mjs` -> 13 passed after rebase
- `npm run test-unit -- test/playwright/unit/ai/daemons/orchestrator/services/RecoveryActuatorService.spec.mjs test/playwright/unit/ai/daemons/orchestrator/Orchestrator.spec.mjs test/playwright/unit/ai/services/memory-core/helpers/recoveryRunStateStore.spec.mjs test/playwright/unit/ai/mcp/server/memory-core/config.template.spec.mjs` -> 86 passed after fixture repair
...
- [ ] In an operator-owned deployment, enable `orchestrator.recoveryActuator.enabled`, configure an allowlisted compose service, trigger a controlled restart, and verify the recovery-run ledger plus reobserve request.
