...
Your Neo.mjs work is done in a **workspace**, which is an npm project that includes Neo.mjs as a dependency and an `apps` directory to hold your applications.
...
4.  It runs the initial Neo.mjs builds (`npm run build-all`).
...
A unique advantage of Neo.mjs is its support for four distinct environments, allowing you to switch between a rapid, zero-builds development workflow and highly optimized deployment builds. Understanding these environments is key to leveraging the full power of the engine.
...
-   **dist/production**: Creates highly optimized, minified bundles for each engine thread, ensuring the smallest possible footprint for deployment.
