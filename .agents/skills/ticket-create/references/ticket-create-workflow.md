...
1. **Explicit substantive-rationale declaration** with cite-able context (e.g., session-mode urgency, time-critical empirical evidence, lane-coordination need that legitimately blocks waiting for graduation, operator-directed sequence with documented reasoning). The author identifies who made the substantive call — peer, operator, or self — for attribution-metadata, but the **rationale-content is what is audit-able by any reviewer**, not the identity. (Per `AGENTS.md §15.6` Flat Peer-Team + Discussion #11091 cycle 2-3 operator-as-peer-with-context-asymmetry refinement: operator-identity grants merge-gate authority, not substrate-discipline authority — substantive rationale is the load-bearing gate. Discussion #11079 OQ3's original "operator-override declaration" wording is preserved as one valid path, but the gate is now identity-neutral: peer/self can claim the exception with audit-able substantive rationale just as legitimately as operator can.)
...
**Empirical anchors:** #11084 (filed 2026-05-10 under @tobiu's "e.g. a new ticket" directive, before #11079 graduated; included inline divergence matrix in Avoided Traps preempting Option E's gap; explicitly acknowledged downstream-amendment) is the **right shape** of the exception path. #11078 (earlier abandoned approach, closed `not_planned`) and #11082 / PR #11083 (Gemini's premature implementation before #11079 graduated, no inline rationale, no preempting matrix; closed `not_planned`) are **wrong-shape** examples — both caught + retracted before substrate damage. #11078 + #11082 together demonstrate that the ungraduated-Discussion + insufficient-rationale combination consistently fails the gate; this AC's exception structure prevents repetition.
