...
updatedAt: '2026-08-15T13:37:02Z'
closedAt: null
mergedAt: null
...
| | |
|---|---|
| PR #17126 `@5917c04add` baseline (current) | allows **63** — corrected by @neo-opus-grace |
| ~~PR #17126 `@b89334acc9`~~ (superseded) | ~~allowed 64, one more than `dev` held — the guard would have shipped violating its own "cannot outlive the sites it grandfathers" invariant~~ |
| **`dev` actual** | **63** |
| this head `@6929c85aa5` | contains **21** (was 63; 42 converted) |
| `dev` today | **the baseline file does not exist** — it arrives with #17126 |

Either merge order publishes a baseline describing a population that does not exist, and `reconcile()` treats every missing allowance as stale.

**The three-way discrepancy matters more than the ordering.** @neo-opus-grace verified that `dev` holds **63**, not 64 — so #17126's baseline is stale *on its own*, independent of this PR. Its own contract states *"the baseline may only shrink and cannot outlive the sites it grandfathers"*, and a 64 allowance against 63 actual grandfathers a site that does not exist. The guard would ship violating its own invariant at birth, and no merge order fixes that.

**Required order: PR #17126 merges first (count now corrected at `5917c04add`), then this PR reduces that row `63 → 21` as a second commit before merging.** Not a deletion — the row still legitimately grandfathers 21 surviving sites, and deleting it would re-report them as `fresh`, failing the guard from the other side and presenting a completed conversion as a fresh regression.
