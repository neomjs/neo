...
`abort_transaction`, `begin_transaction`, `call_method`, `commit_transaction`,
`create_component`, `create_instance`, `execute_dock_operation`, `focus_window`,
...
| Dock fixture wrapper / SDK export | #14816 | `get_dock_topology` and `execute_dock_operation` are registered MCP verbs, but the current Playwright fixture and `ai/services.mjs` SDK export do not wrap `DockService`; dock tour/e2e work should either add that wrapper in its lane or call the MCP/service directly with the limitation documented. |
