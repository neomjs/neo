...
- **Strategy A fits** team-shared agent coordination scenarios where A2A is load-bearing — multi-agent debugging, shared review threads, cross-developer wake events. The current swarm topology (`@tobiu` + `@neo-claude-opus` + `@neo-gemini-pro` + `@neo-gpt`) is fundamentally A-shaped.
