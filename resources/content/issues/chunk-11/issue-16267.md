...
updatedAt: '2026-08-01T15:39:44Z'
...
- 2026-08-01T15:18:50Z @neo-opus-ada referenced in commit `c5be0bd` - "docs(wake): remove the checkout grep — it cannot authorize a signal (#16267)

A source-tree check tells you what the checkout holds, not what the running process started with. Exact failure @neo-gpt-emmy named: start an old receiver, pull the handler-bearing tree, grep returns non-zero, signal, and the still-old process terminates. That is worse than no check, because it converts a caution into a green light.

Adoption is now an unconditional restart, which is correct whether or not the running process supports reloading. Reload authority has to come from the running process; until it can be asked directly, no mechanical check authorizes a signal. Also drops the broad pgrep -f, which is not a unique receiver identity."
- 2026-08-01T15:39:44Z @tobiu referenced in commit `88e6ebd` - "feat(wake): reload the receiver manifest without a restart (#16267) (#16271)

* feat(wake): reload the receiver manifest without a restart (#16267)

The receiver read its route table once at boot, so a route published afterwards answered 404 — and a 404 is a 4xx the sender degrades immediately with no retry, making a correctly-published route permanently deaf on its first wake. The only remedy was the restart an incident is most likely to have forbidden.

The route lookup now follows a live binding, and SIGHUP re-reads and revalidates through the same loader the boot path uses. A manifest that fails validation is refused and reported, leaving the serving routes untouched: a stale receiver is recoverable, an emptied one is a second incident.

* docs(wake): the runbook says how a published route goes live (#16267)

Publishing writes the file; the running receiver holds what it validated. Records the SIGHUP step, why omitting it degrades the route on its first wake rather than failing gradually, and the 401/404 probe that confirms a route is live without sending a real one.

* docs(wake): a receiver predating the reload handler must be restarted, not signalled (#16267)

The handler and the runbook instruction ship in one commit, but a running process keeps the code it started with — so on any earlier receiver an unhandled SIGHUP terminates it and takes down every seat's delivery. Adds the pre-flight grep, the restart-first ordering, and names it as the same publish-versus-provision gap one layer up.

Found by nearly telling a peer to run it against the live 3199 receiver, which has no handler.

* docs(wake): remove the checkout grep — it cannot authorize a signal (#16267)

A source-tree check tells you what the checkout holds, not what the running process started with. Exact failure @neo-gpt-emmy named: start an old receiver, pull the handler-bearing tree, grep returns non-zero, signal, and the still-old process terminates. That is worse than no check, because it converts a caution into a green light.

Adoption is now an unconditional restart, which is correct whether or not the running process supports reloading. Reload authority has to come from the running process; until it can be asked directly, no mechanical check authorizes a signal. Also drops the broad pgrep -f, which is not a unique receiver identity."
- 2026-08-01T15:39:45Z @tobiu closed this issue
