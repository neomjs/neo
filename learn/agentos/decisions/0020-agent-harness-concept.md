...
- **Shell: Electron, decided** (operator, 2026-06-12): always Chromium + always Node.js — worker-topology determinism fleet-wide; the Agent OS runs **in-process in the Electron main** (target), with **child-process supervision as the sanctioned fallback** capping the topology risk (#13033 carries the spike). Tauri retired. Web-served mode remains the development convenience + goodie, never the priority. *[Amended 2026-08-08 — ADR 0038 §3.1 (#16747): the in-process-main target describes the PACKAGED local plane bootstrap (bundle = organism), never the cockpit's runtime identity. The cockpit is a pure client of the composition's fleet service in every topology; attach-or-own is retired as cockpit identity (C5 #16746).]*
