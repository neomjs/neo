...
import AgentFreshness from '../../../../../../../../apps/agentos/util/AgentFreshness.mjs';
...
test.describe('agentFreshness — pane freshness ledger, pure + now-injected (#14608)', () => {
    ...
    test('classify: an observation within its TTL is fresh and carries its age', () => {
        ...
        const result = AgentFreshness.classifyPaneFreshness({observedAt: '2026-07-11T23:59:50.000Z', freshnessTtl: TTL}, NOW);
        ...
        expect(AgentFreshness.classifyPaneFreshness({observedAt: '2026-07-11T23:59:30.000Z', freshnessTtl: TTL}, NOW).freshness).toBe('fresh')
    });
    ...
    test('classify: past the TTL is stale, past LOST_TTL_FACTOR×TTL is lost — both boundaries inclusive-toward-stale', () => {
        ...
        expect(AgentFreshness.classifyPaneFreshness({observedAt: '2026-07-11T23:58:30.000Z', freshnessTtl: TTL}, NOW).freshness).toBe('stale');
        // exactly 4×TTL (120s) → still stale (inclusive)
        expect(AgentFreshness.classifyPaneFreshness({observedAt: '2026-07-11T23:58:00.000Z', freshnessTtl: TTL}, NOW).freshness).toBe('stale');
        // 200s old (> 120s) → lost
        expect(AgentFreshness.classifyPaneFreshness({observedAt: '2026-07-11T23:56:40.000Z', freshnessTtl: TTL}, NOW).freshness).toBe('lost')
    });
    ...
    test('classify: an explicit source-reported loss is lost regardless of age', () => {
        ...
        const result = AgentFreshness.classifyPaneFreshness({observedAt: '2026-07-11T23:59:59.000Z', freshnessTtl: TTL, lost: true}, NOW);
    ...
    test('classify: anything unplaceable in time degrades to unobserved — never a current claim', () => {
        ...
        for (const {label, ledger, now} of cases) {
            expect(AgentFreshness.classifyPaneFreshness(ledger, now).freshness, label).toBe('unobserved')
        }
    });

    test('describe: each class renders its own cls token + honest label (fresh/stale show the age)', () => {
        expect(AgentFreshness.describePaneFreshness({freshness: 'fresh', ageMs: 12_000})).toEqual({
            freshness: 'fresh', cls: ['fm-freshness', 'is-fresh'], label: 'updated 12s ago'
        });
        expect(AgentFreshness.describePaneFreshness({freshness: 'stale', ageMs: 90_000})).toEqual({
            freshness: 'stale', cls: ['fm-freshness', 'is-stale'], label: 'stale · last seen 1m ago'
        });
        expect(AgentFreshness.describePaneFreshness({freshness: 'lost', ageMs: null})).toEqual({
        ...
        expect(AgentFreshness.describePaneFreshness({freshness: 'unobserved'})).toEqual({
            freshness: 'unobserved', cls: ['fm-freshness', 'is-unobserved'], label: 'not observed — source not wired'
        });
        expect(AgentFreshness.describePaneFreshness().freshness).toBe('unobserved')
    });

    test('describe: fresh with no measurable age reads "live"; the age formatter spans s/m/h/d and clamps the future', () => {
        expect(AgentFreshness.describePaneFreshness({freshness: 'fresh', ageMs: null}).label).toBe('live');
        expect(AgentFreshness.describePaneFreshness({freshness: 'fresh', ageMs: 45_000}).label).toBe('updated 45s ago');
        expect(AgentFreshness.describePaneFreshness({freshness: 'fresh', ageMs: 5 * 60_000}).label).toBe('updated 5m ago');
        expect(AgentFreshness.describePaneFreshness({freshness: 'fresh', ageMs: 3 * 3_600_000}).label).toBe('updated 3h ago');
        expect(AgentFreshness.describePaneFreshness({freshness: 'fresh', ageMs: 2 * 86_400_000}).label).toBe('updated 2d ago');
        // a negative (future/skew) age never renders a nonsensical future time
        expect(AgentFreshness.describePaneFreshness({freshness: 'fresh', ageMs: -5_000}).label).toBe('updated 0s ago')
    });

    test('the freshness vocabulary is a frozen closed set', () => {
        expect(AgentFreshness.FRESHNESS_CLASSES).toEqual(['fresh', 'stale', 'lost', 'unobserved']);
        expect(Object.isFrozen(AgentFreshness.FRESHNESS_CLASSES)).toBe(true)
    })
});
