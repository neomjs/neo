...
Three suites, all green at `32bdb8d1ed`:

- `embeddingDispatchPlan.spec.mjs` — **17 arms** over the pure plan (task budget, spans, dispatch plan, carryable prefix).
- `TextEmbeddingService.retry.spec.mjs` — **52 arms**, four of them new production-path concurrency scenarios.
- `TextEmbeddingService.spec.mjs` — **59 arms**, including the exact-peak-4 overlap probe.
...
- `1957f3ea1f` — the declared parallelism becomes a concurrency, and the carry stops being arithmetic.
- `8be3a12ac0` — the capacity unit is a task, so width and concurrency share one budget.
- `32bdb8d1ed` — four concurrency scenarios pure prefix arithmetic cannot reach.
