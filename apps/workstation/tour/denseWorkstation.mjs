...
export const initialDocument = Object.freeze({
    ...
    root  : 'root',
    items : {
        scale    : {title: '100k Operations Matrix',       kind: 'grid'},
        feed     : {title: 'Live Event Stream',            kind: 'grid'},
        alerts   : {title: 'Priority Alert Observatory',   kind: 'panel'},
        activity : {title: 'Resident Activity Timeline',   kind: 'panel'},
        topology : {title: 'Workspace Topology Inspector', kind: 'panel'},
        runtime  : {title: 'Runtime Health Envelope',      kind: 'panel'},
        traces   : {title: 'Distributed Trace Explorer',   kind: 'panel'},
        logs     : {title: 'Structured Log Console',       kind: 'terminal'},
        console  : {title: 'Command and Control Console',  kind: 'terminal'},
        builds   : {title: 'Build Pipeline Monitor',       kind: 'panel'},
        deploys  : {title: 'Deployment Flight Deck',       kind: 'panel'},
        security : {title: 'Security Signal Center',       kind: 'panel'},
        memory   : {title: 'Memory Pressure Telemetry',    kind: 'panel'},
        graph    : {title: 'Dependency Graph Explorer',    kind: 'panel', autoHidden: true},
        queues   : {title: 'Task Queue Pressure',          kind: 'panel'},
        metrics  : {title: 'System Metrics',               kind: 'panel'},
        audit    : {title: 'Evidence Audit',               kind: 'panel'},
        commits  : {title: 'Commit Stream',                kind: 'panel'},
        files    : {title: 'Workspace Files',              kind: 'panel'},
        inspector: {title: 'Selection Inspector',          kind: 'panel', autoHidden: true}
    },
    nodes: {
        root               : {
            type : 'edge-zone',
