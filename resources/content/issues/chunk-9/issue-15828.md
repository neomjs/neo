...
- 2026-07-24T20:01:47Z @tobiu referenced in commit `276c7a5` - "fix(ci): the lint annotation must carry the answer, not cite a missing comment (#15828) (#15829)

`agent-pr-body-lint` runs on [opened, edited, synchronize, ready_for_review] but
posts its diagnostic comment only on `opened` — while the failure always said
"See follow-up comment on PR #N". So on every edit it failed pointing at a
comment that was never written.

That is precisely when the diagnostic matters: an anchor breaks BECAUSE someone
edited the body, which is an `edited` event by construction. The one path where
the comment exists is the one where the author has the template freshest in mind.

Three instances today, two agents, two model families, all on the same anchor —
a lowercase rewrite, a `## Residual / ` prefix, and a reviewer's `## Review
Closure` rename. Every word survived each time; the substring did not. All three
authors had to open this workflow to learn which anchor was missing.

`missingVisible`/`missingInvisible` were already computed at the failure site and
simply never reached the annotation. Now they do, with the literal-substring rule
stated inline, and the message tells the truth about whether a comment exists for
this event rather than asserting one does.

Deliberately NOT done: commenting on every `synchronize` (a comment per push is
noise that gets muted, which is how a real signal stops being read), and relaxing
the matcher to tolerate "better prose" (the anchors are a graph-ingestion
contract — strict matching is correct, an undiagnosable failure is not).

Verified: the embedded script parses (`node --check` on the extracted block), and
the failure text was replayed against a body missing two anchors for all three
event types — each names `## Post-Merge Validation` and `Authored by `, and only
the `opened` variant mentions a follow-up comment."
- 2026-07-24T20:01:47Z @tobiu closed this issue
