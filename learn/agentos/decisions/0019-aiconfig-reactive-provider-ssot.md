...
| C4 | duplicate leaves inside AiConfig bind one env var or declare one semantic value under separate config paths | `[unenforced: #17448 removed the known live specimen; no same-template duplicate-leaf detector]` | keep one declared leaf — a reader census decides which name survives; migrate readers when needed, then delete the other |

> **C4 placement:** both competing declarations live inside the SSOT. This is boundary/duplication (Group C), not resolution reimplemented outside the Provider (Group A) or indirection around an AiConfig read (Group B).
