...
updatedAt: '2026-06-07T17:12:39Z'
...
1. **First prescription** (hardcoded `['@neo-opus-4-7', '@neo-gemini-3-1-pro', '@neo-gpt']`): would break every other Memory Core client deployment with different agent identity namespaces.
...
So my coordination error (sending ~10 A2As to fictional `AGENT:gpt` / `AGENT:gemini` instead of canonical `@neo-gpt` / `@neo-gemini-3-1-pro`) IS a real agent-mental-model failure — but it's NOT a substrate gap that can be mechanically closed without breaking multi-tenant clients.
