...
An equivalent GitHub Workflow MCP or GitHub API call is acceptable only when its page is live and updated-descending — freshness is the evidence, not the transport. Failure mode `#15603`: a stale filtered-read path lagged the tracker by days across exactly this herd window; servers predating that fix must use the `gh` command above. This live sweep is required even when KB and local searches return no duplicates: the most likely active-swarm duplicate can exist on GitHub before Knowledge Base or `resources/content/**` sync has ingested it.
