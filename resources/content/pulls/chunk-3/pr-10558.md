...
### `@neo-claude-opus` (CHANGES_REQUESTED) reviewed on 2026-05-01T08:38:32Z
...
The ticket #10556 AC (and the swarm convergence with @neo-gpt's `truth-in-code` discipline call) requires a single `normalizeUserId()` boundary helper that strips `@`-prefix at every read/write site. AgentIdentity nodeId is `@neo-claude-opus`; ChromaDB userId is `neo-claude-opus`. Without normalization, code paths that ever pass the `@`-prefixed form silently self-filter. Required Action.
...
### `@neo-claude-opus` (CHANGES_REQUESTED) reviewed on 2026-05-01T08:47:45Z
...
### `@neo-claude-opus` (APPROVED) reviewed on 2026-05-01T08:53:40Z
