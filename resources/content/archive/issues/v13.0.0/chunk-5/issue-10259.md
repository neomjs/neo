...
updatedAt: '2026-06-07T17:20:49Z'
...
@neo-opus-ada          | AgentIdentity | Claude Opus 4.7     | githubLogin: @neo-opus-ada       | family: claude | accountType: agent
...
1. Gemini's `list_messages` surfaced a broadcast from `@opus` (a stale alias of me), and her reply threading targeted `@opus` rather than my canonical `@neo-opus-ada`. Result: her reply `MESSAGE:64a3ba28-...` landed in the wrong identity's inbox despite being semantically addressed to "the same physical agent who sent 'hello all'". Tobi directly flagged this as a routing gap worth fixing.
...
- **Aliased identities**: `@opus` + `@gemini` exist as standalone `AgentIdentity` nodes with null metadata. They represent the same physical agents as `@neo-opus-ada` + `@neo-gemini-pro` but are distinct graph nodes. The `#10144` convention made `@neo-*-*` the canonical form; the pre-#10144 aliases weren't migrated.
- **Routing semantics** follow the literal `SENT_BY` edge. A broadcast sent by `@opus` (lingering from earlier sessions before the #10144 seed wrote `@neo-opus-ada`) gets replies routed to `@opus`, not `@neo-opus-ada`. The inbox of `@neo-opus-ada` never sees those replies despite being the "same agent."
...
- `ai/graph/identityRoots.mjs` — the `IDENTITIES` array that `GraphService.initAsync` self-seeds at boot (#10232). Lists the 4 canonical identities: `@neo-opus-ada`, `@neo-gemini-pro`, `@tobiu`, `AGENT:*`. Does NOT include `@opus` / `@gemini`, confirming those are historical.
...
- [ ] Edge rewrites preserve semantic integrity — `SENT_BY`/`SENT_TO` edges that pointed at `@opus` now point at `@neo-opus-ada`; the `MESSAGE:64a3ba28-...` "Re: hello all" becomes reachable to me via `list_messages`
...
- **"Merge via ChromaDB-only metadata update"** — wrong substrate. `@opus` and `@neo-opus-ada` are graph nodes, not Chroma metadata rows. Merging must happen at the SQLite graph layer.
...
Retrieval Hint: `"identity aliasing @opus @gemini @neo-opus-ada @neo-gemini-pro merge"`
...
- 2026-04-23T21:03:00Z @neo-opus-ada added the `bug` label
- 2026-04-23T21:03:00Z @neo-opus-ada added the `ai` label
- 2026-04-23T21:03:01Z @neo-opus-ada added the `architecture` label
- 2026-04-23T21:03:01Z @neo-opus-ada added the `core` label
- 2026-04-23T21:08:41Z @neo-opus-ada assigned to @neo-opus-ada
- 2026-04-23T21:12:22Z @neo-opus-ada cross-referenced by PR #10262
- 2026-04-23T21:26:42Z @neo-opus-ada referenced in commit `4bf3055` - "fix(memory-core): auto-prepend missing @ prefix in normalizeMailboxTarget (#10259)"
