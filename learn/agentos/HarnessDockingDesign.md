# Agent Harness Docking Design Specification

`@summary` Design-tier contract above `dockZone.v1`: the multi-window layout model, named perspectives, cross-window drag semantics, the SharedWorker seam, the auto-hide UI contract, the core-lift disposition, and the container contract for embedded product surfaces.

## Scope

`learn/agentos/HarnessDockZoneModel.md` (dockZone.v1) is the model contract of record: the serializable dock-zone tree, its semantic operations, the preview payload, and the persistence wrappers. That contract is landed and shipped (`src/dashboard/DockZoneModel.mjs`, `src/dashboard/DockLayoutAdapter.mjs`, `src/dashboard/DockSplitter.mjs`, `apps/agentos/view/DockPreview.mjs`, verification surface `examples/dashboard/dock/`).

This document is the design tier **above** that contract. It settles the questions the remaining capability leaves share and cannot answer coherently one leaf at a time:

1. the full multi-window layout model — which docking state is worker-owned shared truth and which is per-window render projection (§1);
2. named perspectives across a multi-window topology (§2);
3. cross-window drag as semantic operations (§3);
4. grouped drag and tab overflow (§4);
5. the disposition of the core-lift clause (§5);
6. the container contract embedded product surfaces consume (§6);
7. the auto-hide UI contract (§7).

Everything here is **additive on the landed dockZone.v1 model — not a redesign**. Where this spec names a new schema or operation, it extends the existing family; it never alters a landed shape. Implementation leaves follow this spec, one Contract-Ledgered leaf per capability; the leaf inherits its contract from the named section and must cite it.

**The capability bar** is the [Qt Advanced Docking System](https://github.com/githubuser0xFFFF/Qt-Advanced-Docking-System) interaction set — dock everywhere, auto-hide sidebars, named perspectives, grouped drag, tab overflow — composed with Neo's multi-window reality: Chromium popups sharing one SharedWorker application heap (ADR 0020), where a docked pane detaches into a real OS window and returns as the same live object. Qt-ADS is the bar, not the design: its single-process, single-window-tree assumptions do not survive the worker/multi-window architecture (see §Precedent).

**Design provenance:** Discussion #13370 (graduated 2026-06-15) resolved the cross-window seam contract, the placement-hint layer, and the contract-over-lift constraint absorbed here; issue #14423 carries the criteria mapping. The window-manager leaves #13025 (popup terminal drop) and #13028 (OS-window drag reintegration) are landed and consumed as boundaries — their arbitration substrate is formalized, not reopened, by §3.

## Decision-Record Disposition

`aligned-with ADR 0020`. **No new ADR is required by this spec.** The cross-window seam (§3) formalizes arbitration behavior that already ships in `src/manager/DragCoordinator.mjs`; formalizing a landed duck-type into a named contract is contract-doc authority, matching Discussion #13370's own graduated disposition ("Decision Record: OPTIONAL — seam contract lives in the contract docs, not a new ADR"). **Escalation trigger, explicitly named:** if a future leaf needs to *change* `DragCoordinator`'s arbitration semantics — its target-resolution order, its registry shape, or its dock-blindness (§3 invariant) — that change exceeds contract-doc authority and requires an ADR before implementation.

## §1 Layout Model Formalization — Multi-Window State Space
...
|---|---|---|
| Durable | item identity, detached-vs-docked intent, `owningWorkspaceId` + perspective id (fulfilling the graduated "owning `dockLayoutId` / perspective" hint at topology scope), semantic `fallbackTarget` (a dock-zone node reference, e.g. "the tabs node that owned me") | Persisted with perspectives (§2). |
...
|---|---|---|---|
| **Worker-owned shared truth** | committed `dockZone.v1` documents; the workspace-set registry; `dockLayout.v1` wrappers; `dockLayoutCollection.v1`; `dockPerspective.v1` (§2); durable placement hints; item catalogs incl. `pinned` / `autoHidden` | workspace container(s) in the App Worker | Serializable per contract rules |
...
| **Per-window runtime interaction state** | `dockPreview` payloads; reveal/open state of auto-hidden panes (§7); hover state; splitter pixel math mid-drag | the window's drag/interaction surfaces | Never persisted; never crosses the seam except as operation descriptors |
| **Main-thread-only state** | DOM nodes; `DOMRect`s; screen coordinates; native window geometry; `getWindowAt` lookups | main-thread addons (`Neo.manager.Window`, `WindowPosition`) | Never persisted; consumed by arbitration (§3), results delivered as semantic events |
...
- `placementHints` contains **durable hint fields only** (§1 table). A perspective containing `windowId`, screen coordinates, monitor geometry, or rects is invalid and must be rejected at validation.
...
Windows are render targets (§1); restoring a perspective restores **worker-owned truth first**, then lets windows catch up:
...
- **Identity-preserving:** the item record (id, `componentRef`, `title`, `kind`, policy hints, `blueprint`, metadata) travels verbatim. The live component instance is **moved, never re-instantiated** — it exists once in the shared heap throughout (this is the §Precedent moat behavior; it must never regress to serialize-and-recreate).
...
- `transferNode` — the §3 `transferItem` semantics applied to a subtree: atomic two-document commit, all member item records travel verbatim, all live component instances move without re-instantiation, hints update per item.
...
Overflow is a **projection affordance, not a model change**: when a `tabs` node's headers exceed the available extent, the adapter's tab projection surfaces the hidden remainder through an overflow control (dropdown listing hidden tabs; selecting one sets `activeItemId` via the ordinary semantic path). Nothing new persists — `items` order and `activeItemId` already capture the state. The leaf implements this against `Neo.tab.Container`'s existing header mechanics; it must not fork a dock-specific tab container (inherited from the contract's §Split/Tab Adapter Boundary).

## §5 Core-Lift Clause Disposition

The contract's §Ownership Boundary keeps the dock subsystem in `src/dashboard/` — reusable across apps, not yet a core layout primitive — with a further lift gated on "a second independent consumer beyond dashboard adaptation."

**Disposition: DEFER, with the trigger sharpened.** The approaching consumers — Institution Cockpit panes (#13444) and FM-UX cards (#13015) — consume the docking **shell** through the §6 container contract; they are consumers *of the dashboard docking subsystem*, not evidence that a dashboard-independent core primitive is needed. Lifting now would duplicate API surface for zero demanding consumer, while the operator-assessed brittleness lives in the interaction layer — which §§1–4 and §7 address by contract, not by relocation.

**The named trigger (fire condition):** a consumer that needs dock semantics **without dashboard adaptation** — concretely, the Portal learning workspace (the contract's own named candidate) or any consumer that must compose dock trees from non-dashboard containers. When such a consumer files, the lift executes as its own Contract-Ledgered leaf: model + adapter move to a core namespace, `src/dashboard/` re-exports for compatibility, and the §Ownership Boundary section of the model contract is amended in the same change. Until that consumer exists, any lift PR is premature and should be declined citing this section.

## §6 Container Contract for Embedded Product Surfaces

The minimal interface between the docking shell and the product surfaces that live inside it. Named consumers: **Institution Cockpit panes (#13444)** and **FM-UX cards (#13015)**. The contract is deliberately interface-minimal; it can absorb steward feedback (the #13015 options thread) without breaking, because every guarantee below is already landed behavior or a §1–§3 obligation.
...
| Nothing else — no layout knowledge, no dock imports, no preview access | Semantic placement continuity across moves, splits, transfers (§3), perspective capture/restore (§2), and auto-hide transitions (§7); item identity (`dockItemId`) stable across all of them |
...
- **Layout is pane-blind.** The shell knows items only as catalog records. Cockpit panes and FM-UX cards add zero cases to the docking code; if a product surface needs a new docking behavior, that behavior enters through this spec, not through a pane-specific branch.

## §7 Auto-Hide UI Contract

Grace's #13280 implements this section; it is written to be implementation-sufficient without further design decisions. Sequencing per the leaf owner's decision (2026-07-02): the leaf follows this spec.
...
| *revealed* | **never** | Runtime-only overlay state of an auto-hidden item in one window (§1 table, row 3). A revealed-but-unpinned item serializes as `autoHidden: true`. |
| *detached* | hint layer (§1) | An item embodied in its own OS window. **Mutually exclusive with `autoHidden`:** auto-hide is an in-workspace affordance; `detachItem` on a railed item clears `autoHidden` in the same commit, and `setItemAutoHidden(true)` rejects detached items. |
...
|---|---|
| Auto-hidden item in a multi-window topology | Rails are per-window projections of the owning workspace document (§1). The item rails in the window rendering that workspace — exactly one place. |
| Reveal state across windows | Runtime-only and per-window; never mirrored, never persisted. |
| `transferItem` of a railed item (§3) | Transfers the catalog record verbatim — including `autoHidden: true`; the item arrives railed on the target's corresponding edge if placed in an edge zone, else the placement descriptor governs and the §7 clearing rule applies. |
| Perspective capture (§2) | Captures `pinned` / `autoHidden` as committed item state (landed serialization rules); reveal state is invisible to perspectives. |
...
## Precedent — the 2026-07-02 parity sweep
...
| Capability | [Qt-ADS](https://github.com/githubuser0xFFFF/Qt-Advanced-Docking-System) (bar) | [Dockview](https://dockview.dev/docs/core/groups/popoutGroups/) | [GoldenLayout](https://golden-layout.com/docs/GoldenLayout.html) | [FlexLayout](https://github.com/caplin/FlexLayout) | [rc-dock](https://github.com/ticlo/rc-dock) | [Lumino](https://github.com/jupyterlab/lumino) | Neo target (this spec) |
|---|---|---|---|---|---|---|---|
| Dock/split/tab + drag preview | ✓ | ✓ | ✓ | ✓ | ✓ | ✓ | landed (dockZone.v1) |
| Auto-hide sidebars | ✓ (click/hover reveal, drag to borders) | — | — | — | — | — | §7 (rails landed; reveal = #13280) |
| Named perspectives | ✓ (save/restore by name) | layout serialize | layout serialize | layout serialize | `saveLayout`/`loadLayout` | — | §2 (single-workspace landed; topology-scope specified) |
| Grouped drag | ✓ (title bar moves tab group) | — | — | — | — | — | §4 (`moveNode`/`transferNode`) |
| Tab overflow | ✓ (dropdown) | — | — | ✓ | — | — | §4 (projection affordance) |
...
| Survives opener close/reload with popouts open | n/a (native) | no — popouts are opener-tied | no — popout re-creation is opener-coordinated | no — portals live in the opener's realm | not documented | n/a | yes — heap persists while ≥1 window is connected (§1) |
| Mid-gesture drag **between** OS windows | ✓ (native) | — | — | — | — | — | landed (`DragCoordinator` remote-drag arbitration, §3) |
...
2. **Auto-hide as afterthought.** The most visible Qt-ADS affordance is absent across all five surveyed web libraries. It is specified here (§7) as committed model state + per-window runtime reveal — the same seam discipline as everything else, not a bolt-on.

## Decomposition — the leaves this spec gates
...
|---|---|---|
| Auto-hide UI: reveal overlay + pin control + rail drag source | §7 | **#13280 (in flight, Grace)** — held design-first by owner decision; this spec is its contract |
| `CrossWindowDragTarget` formalization + dock workspace target + `transferItem` | §3 | unfiled; file at claim time citing §3 |
| Topology perspectives: `dockPerspective.v1` + switcher + restore reconciliation | §2 | unfiled; single-workspace substrate landed |
| Grouped drag (`moveNode`/`transferNode`) + tab overflow affordance | §4 | unfiled |
| Core lift to a non-dashboard namespace | §5 | **gated** — fires only on the named trigger |

## JSON-First Guardrail (restated, applied)

Inherited unchanged from the model contract's §Serializable vs Runtime State and applied to every shape this spec introduces:
...
- Durable placement hints persist intent and semantic targets only; every geometric or window-identity field is runtime-only (§1 hint table).
- `dockPreview` (including the §4 `groupNodeId` field) remains runtime-only in its entirety.
- Reveal/open state of auto-hidden panes is never serialized (§7).
...
This spec satisfies the design-tier ticket (#14423) when:

- the SharedWorker seam is stated as a normative state-class table (§1);
- perspective semantics define capture scope, cross-topology restore, and fail-closed behavior (§2);
- cross-window drag is defined as semantic operations riding the existing preview → operation path, with the coordinator dock-blind and no parallel drag system (§3);
- grouped drag and overflow have a leaf-implementable contract (§4);
- the core-lift clause is dispositioned with a named trigger (§5);
- the container contract names its two consumers and their minimal interface (§6);
- the auto-hide UI contract is implementation-sufficient for #13280 (§7);
...
Implementation leaves begin after this document merges; a leaf that contradicts a section here amends the spec first, in its own reviewed change.
