...
updatedAt: '2026-06-25T09:09:56Z'
closedAt: null
mergedAt: null
...
This PR no longer emits a `Golden Path Forecast Status` section, route-provenance paragraph, or candidate-count diagnostics. Those mechanics are already represented by the existing Computed Golden Path section and `DreamPipeline.md`; duplicating them inside the handoff was rejected as bloat.

Evidence: L2 focused static/unit verification completed locally. Residual: GitHub CI is running on the corrected head; L3 post-merge orchestrator validation must inspect the next generated `resources/content/sandman_handoff.md`.
...
- `bfdf928550` - `fix(ai): degrade Golden Path interpretation explicitly (#13983)`
