...
> - **Auto-Wakeup Substrate (Epic [#10601](https://github.com/neomjs/neo/issues/10601))** — sunset is no longer terminal in the post-#10602 era **for identities with a configured recovery route**. A sunsetted agent with a shipped `identityMap` entry in `ai/scripts/resumeHarness.mjs` is auto-resumed by the heartbeat-driven `swarm-heartbeat.sh` → `checkSunsetted.mjs` → `resumeHarness.mjs` chain within ~10-15 minutes. Current `dev` ships routes for `@neo-gemini-3-1-pro` only; cross-harness coverage for `@neo-opus-4-7` (via #10607) and `@neo-gpt` (via deferred Codex Desktop verification) is pending. For unconfigured identities, sunset still requires manual @tobiu intervention. Sunset narrative is now A2A-unsubscribe + recovery-via-substrate (where shipped), not session-ending. This shifts the cost-of-sunset calculation: the cost of *premature* sunset is reduced for covered identities (substrate auto-recovers), but the cost of substrate-MISFIRE during recovery (e.g., infinite cooldown spam) is amplified. Discipline still matters; the recovery layer is a safety net, not a license.
