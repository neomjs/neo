...
class VdomLifecycle extends Base {
                    ...
                    while (parent) {
                        if (updates[parent.id]) {
                            ...
                            if (parentDepth === -1 || parentDepth > distance) {
                                const parentMergedIds = componentMergedChildren.get(parent.id);

                                if (!parentMergedIds || parentMergedIds.has(id)) {
                                    delete updates[id];
     ...
     * @param {Set<String>|null} [mergedChildIds] IDs of childr