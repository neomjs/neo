...
- **If the healthcheck is successful:** The Memory Core is active.
    ...
    - **Establish Context (Mandatory):** You **MUST** call two tools at boot:
        ...
        3. `view_file` on `resources/content/sandman_handoff.md`: If this file exists, you MUST parse it immediately. It contains actionable Sandman topological alerts (e.g., OPEN tickets discovered to be superseded by the Native Edge Graph). **Conflict Resolution:** If you resolve an item from this list, you MUST update the Native Edge Graph (via tickets, dependencies, or Neo toolings) to permanently reconcile the conflict so the alert does not reappear during the next nightmare cycle.
