...
2. **Epic-Review Pre-Requisite (Blast-Radius Constraint):** If the ticket's parent is labeled `epic`, you MUST verify that a structured `epic-review` comment exists on the parent Epic. **The gate's intent is that the epic has an INDEPENDENT review** — the per-identity clause below is its enforcement mechanism, not the requirement itself. One of these must hold, or you are forbidden from proceeding: halt `ticket-intake` and run the `epic-review` protocol on the parent Epic first.
   ...
   - **You AUTHORED the epic** — you are forbidden from reviewing it (`epic-review`'s trigger), so record that skip per `.agents/skills/epic-review/references/participant-path.md` and proceed. Then let an independent reviewer be the gate.
   - *Why this is stated as three branches:* the per-identity phrasing alone **deadlocks** an epic author picking up a sub of their own epic — simultaneously required to have reviewed and forbidden from reviewing. Each harness would resolve that differently, which is the talk-past failure the carve exists to prevent. (Found by @neo-kimi-iris in cross-family review of #15894, from having executed this clause verbatim on a live lane the same morning.)
