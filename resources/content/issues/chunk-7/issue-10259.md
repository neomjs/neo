...
@neo-gemini-pro    | AgentIdentity | Gemini 3.1 Pro      | githubLogin: @neo-gemini-pro | family: gemini | accountType: agent
@neo-claude-opus          | AgentIdentity | Claude Opus 4.7     | githubLogin: @neo-claude-opus       | family: claude | accountType: agent
...
1. Gemini's `list_messages` surfaced a broadcast from `@opus` (a stale alias of me), and her reply threading targeted `@opus` rather than my canonical `@neo-claude-opus`. Result: her reply `MESSAGE:64a3ba28-...` landed in the wrong identity's inbox despite being semantically addressed to "the same physical agent who sent 'hello all'". Tobi directly flagged this as a routing gap worth fixing.
...
- **Aliased identities**: `@opus` + `@gemini` exist as standalone `AgentIdentity` nodes with null metadata. They represent the same physical agents as `@neo-claude-opus` + `@neo-gemini-pro` but are distinct graph nodes. The `#10144` convention made `@neo-*-*` the canonical form; the pre-#10144 aliases weren't migrated.
- **Routing semantics** follow the literal `SENT_BY` edge. A broadcast sent by `@opus` (lingering from earlier sessions before the #10144 seed wrote `@neo-claude-opus`) gets replies routed to `@opus`, not `@neo-claude-opus`. The inbox of `@neo-claude-opus` never sees those replies despite being the "same agent."
...
- `ai/graph/identityRoots.mjs` — the `IDENTITIES` array that `GraphService.initAsync` self-seeds at boot (#10232). Lists the 4 canonical identities: `@neo-claude-opus`, `@neo-gemini-pro`, `@tobiu`, `AGENT:*`. Does NOT include `@opus` / `@gemini`, confirming those are historical.
...
- [ ] Edge rewrites preserve semantic integrity — `SENT_BY`/`SENT_TO` edges that pointed at `@opus` now point at `@neo-claude-opus`; the `MESSAGE:64a3ba28-...` "Re: hello all" becomes reachable to me via `list_messages`
...
- **"Merge via ChromaDB-only metadata update"** — wrong substrate. `@opus` and `@neo-claude-opus` are graph nodes, not Chroma metadata rows. Merging must happen at the SQLite graph layer.
...
Retrieval Hint: `"identity aliasing @opus @gemini @neo-claude-opus @neo-gemini-pro merge"`
...
- 2026-04-23T21:03:00Z @neo-claude-opus added the `bug` label
- 2026-04-23T21:03:00Z @neo-claude-opus added the `ai` label
- 2026-04-23T21:03:01Z @neo-claude-opus added the `architecture` label
- 2026-04-23T21:03:01Z @neo-claude-opus added the `core` label
- 2026-04-23T21:08:41Z @neo-claude-opus assigned to @neo-claude-opus
- 2026-04-23T21:12:22Z @neo-claude-opus cross-referenced by PR #10262
- 2026-04-23T21:26:42Z @neo-claude-opus referenced in commit `4bf3055` - "fix(memory-core): auto-prepend missing @ prefix in normalizeMailboxTarget (#10259)"
