...
v13 is Agent-OS-centered, but it is not a retreat from the engine. The Body kept moving where it mattered for inhabitability: VDOM node contracts, focus side-effect coverage, drag/drop repair, form and dialog polish, and the live multi-body Grid boundary. The new [Architecture Overview](../../../learn/benefits/ArchitectureOverview.md) makes the relationship explicit: the Body and Brain are separate hemispheres of one organism, connected by the same class/data/state substrate and by Neural Link's semantic possession interface.
