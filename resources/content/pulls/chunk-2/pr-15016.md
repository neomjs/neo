...
updatedAt: '2026-07-11T11:31:30Z'
closedAt: null
mergedAt: null
...
> **Ready — all 8 ACs met; AC8 live-validated.** `aggregationEnabled` intentionally stays `false`: this ticket is scoped *before runtime enablement* (AC1 is a safety gate, not a flip), so the enablement flip is the deliberate downstream act — now unblocked. The lane is inert by construction until then.
...
Enablement (`aggregationEnabled → true`) is the deliberate downstream flip, now unblocked; the committed default stays `false`.
