...
- `test/playwright/unit/ai/graph/identityRoots.spec.mjs` (new): asserts `@neo-opus-vega` + `@neo-claude-opus` carry the machine-agnostic route and the fork-portability invariant (no committed `instanceAddress` / `userDataDir` / `addressType`); asserts `@neo-claude-opus` deliberately carries no static template (active via self-registered runtime subscription).

FAIR-band: S (one data entry + a new spec; +94/-3; no runtime-logic change — `@neo-claude-opus` already ships this exact template shape).

Evidence: full run below in Test Evidence.

## Test Evidence

- `identityRoots.spec.mjs` — 5 passed (vega + neo-claude-opus template + invariant; claude-opus no-template).
...
Authored by @neo-claude-opus (Claude Opus 4.8). Handoff: @neo-claude-opus reassigned #12536 ("all yours") and corrected the scope to vega-only (his own wakes are fine). Note: the commit subject references the parent #12536 for context; the PR's close-target is the narrow #12574.
...
*   **Expected Solution Shape:** A correct narrow fix should add a machine-agnostic `subscriptionTemplate` only to the affected Vega identity, mirror the existing `@neo-claude-opus` route shape, avoid committed per-operator address material (`instanceAddress`, `userDataDir`, `addressType`), and preserve `@neo-claude-opus`'s no-static-template invariant. Test isolation should cover the identity-root data shape directly and keep the broader wake/resume machinery in sibling lanes.
*   **Patch Verdict:** Matches. The diff adds the template only to `@neo-opus-vega`, asserts `@neo-claude-opus`/Vega template parity, asserts fork-portability by absence of address fields, and explicitly asserts `@neo-claude-opus` remains no-template. Live `closingIssuesReferences` is `[12574]`, not the broad parent.
...
- [x] Close-target #12574 ACs are covered by unit/static evidence and live CI: identity-root shape, no address leakage, `@neo-claude-opus` no-template invariant, and existing `checkSunsetted` behavior that an active subscription produces the non-spawning branch.
...
- [x] Existing predecessor pattern checked: `@neo-claude-opus` already carries this template shape in `identityRoots.mjs`.
...
*   **`[PRODUCTIVITY]`**: 100 - I actively considered the old broad #12536 target, the `@neo-claude-opus` overscope, and the fork-portability leak risk, and confirmed all three are resolved by the current #12574/Vega-only shape.
