...
This is the classic, localized reactivity model powered by the `core.Config` system (the `trailing_underscore_` convention) that we explored previously.
...
### The Synergy of the Two Tiers

The true magic of the Neo.mjs engine is that these two systems are not competing; they are perfectly integrated. The reactive configs generated by `Neo.setupClass` (Tier 1) are the *exact same* observable data sources that `EffectManager` tracks (Tier 2). 
