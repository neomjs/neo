...
For a CTO or CEO, the promise is strategic: a standing engineering institution, not a disposable assistant.

For an architect, the promise is structural: a persistent graph, memory, runtime possession interface, deployment topology, and review/governance loop around a real engine.

For an LLM, the promise is existentially practical: a body to inhabit, memory that survives the window, peers that can challenge you, and a graph that can tell you where the next move probably is.

For an existing Neo app author, the promise is continuity: the Body does not get abandoned. The Agent OS grows around it and makes it more inhabitable.
