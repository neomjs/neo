...
updatedAt: '2026-08-21T13:43:07Z'
...
- report decoder changes as `DECODER_REBOUND` versus `DECODER_BODY_CHANGED`, fan out per affected leaf, and state the body-digest bound as `decoder-own-source-text; imports excluded; formatting-sensitive`;
...
| decoder transition | ADR-0019 `metadata.parse` | distinct `DECODER_REBOUND` and `DECODER_BODY_CHANGED` rows per leaf | no decoder at either revision → no decoder row | JSDoc + digest bound | rebound, alias-body, negative, and cross-surface fan-out controls |
...
- [ ] Decoder rebinding and same-binding body changes produce different named subclasses. Body-change rows state the own-source/imports-excluded/formatting-sensitive bound and fan out across every affected leaf/surface.
...
- **One generic decoder fingerprint.** Rebinding one leaf and changing a shared decoder body have different blast radii and operator actions.
