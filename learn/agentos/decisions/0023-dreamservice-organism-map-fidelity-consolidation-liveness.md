...
- **CONCEPT nodes are a *selective* bridge guides↔source** — NOT every source file needs a guide, and not every concept needs an implementation. The **curated ontology is 59 concepts** (`.neo-ai-data/concepts/nodes.jsonl` = 59 lines, 1:1 with `resources/content/concepts/*.md` = 59 files; all `verifiedAt:null`, **planned to grow**) — this is the SSOT. The graph's **19,513** total CONCEPT nodes are a **different, non-curated population** (gemma4-self-created@0.8 — `extractMessageConcepts` ← `MailboxService` + `ConceptDiscoveryService` candidate-mining; `state=NULL`, routing-inert; the full model is catalogued in ADR 0024 / #13814), NOT the curated count (operator ground-truth correction). The gap-inference is **weight-gated** (`GapInferenceEngine`'s `guideGapWeightThreshold`, default 0.8); selectivity is by design — and crucially, the audit's guide-disconnect / orphaned-concept / reverification entries largely measure the **non-curated ~19.5k**, not the curated-59. **Map-fidelity for the knowledge layer: the audit must measure against the curated target, not the extraction noise** — else a fresh agent chases false gaps. A `content-should-be-SSOT` redundancy also stands: the 59 concepts live as BOTH `.md` content AND `nodes.jsonl` graph-data.
...
| **Knowledge-graph (selectivity)** | curated-59 ontology vs graph-~19.5k non-curated (the audit's reverification noise measures the latter); 59 concepts duplicated across `.md` + `nodes.jsonl`; ADR nodes inserted-but-inert (no vectors) | audit measures vs the curated target; content-SSOT de-duplicated; ADR nodes first-class / embedded |

**V-B-A note (do not enshrine unverified):** the curated-59 count is verified (`nodes.jsonl` = 59 lines = 59 `.md` files, 1:1) + the gap-inference weight-gate (`GapInferenceEngine.guideGapWeightThreshold`); the ADR-node embedding is **roadmapped, not built** (the 23 ADR nodes are un-embedded). **Resolved V-B-A (@neo-opus-vega):** `nodes.jsonl` is **not** derived from the `.md` content — it is **independently maintained** (KB-pipeline vs Graph), so the 1:1 count is current-coincidence, not a sync guarantee. The `content-should-be-SSOT` is therefore a **target, not current state**: the drift is real (a future de-dup lane), not a closed invariant.
