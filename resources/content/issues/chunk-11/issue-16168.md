...
updatedAt: '2026-08-10T20:24:37Z'
...
subIssues:
  ...
  - '[ ] 17331 Fleet-server plane-log reads: bounded, redacted, read-only'
...
## [FM-as-client input from the `#16694` live receipt — the browser-client facts this epic's cutover leaves can consume]

*(Parked here durably; two A2A DM attempts lost to the live `#16808`-class write degradation — see the fresh evidence on that ticket.)*

From tonight's one-command receipt (`#16694` comment `IC_kwDODSospM8AAAABOKWbkQ`, PR #16912), the state of the browser client this epic's containerized control plane will meet:

**Already in place, client-side:**
1. `?fleetUrl=` dial override (`apps/agentos/app.mjs` — loopback-pinned today; the natural seam for a composed/cloud endpoint once one is admitted).
2. The C2 `#16743` wire-only vocabulary on both ends (versioned offer/selection, zero trust imports).
3. PR #16912's armed exact-Origin bearer handshake — scoped deliberately to the LOCAL relay's process-lifetime bearer (the Option-B dev widening, custody documented). It is NOT a PAT carrier and should not become one.
4. The host registry carries 9 credential-free agent definitions; `defineAgent` is proven tokenless over the authenticated wire.

**Observed topology (docker ps, 2026-08-10 ~22:10 local):** `neo-local-agent-os-fleet-server-1` healthy, up 5h, `8083/tcp` compose-internal with NO host publication; the ingress publishes only `127.0.0.1:3102`. The cockpit can therefore only reach the composed control plane once the ingress path-routes it (the `/kb/mcp` + `/mc/mcp` shape) or a leaf publishes it deliberately.

**Three questions this epic's leaf tree owns (input, not claims):**
- (a) **Browser admission**: a page holding a PAT-class credential is a different custody decision than a process-lifetime bearer; the `ownerPrincipal` derivation here suggests the cockpit authenticates as the OPERATOR's provider tuple — which likely wants its own redemption/session shape rather than the PR #16912 handshake reused.
- (b) **Ingress route + CORS** for a browser origin against `:3102` — the exact-origin discipline the local relay enforces (`NEO_FLEET_COCKPIT_ORIGIN`) presumably moves into the ingress/CORS layer for the composed path.
- (c) **The composed registry volume is EMPTY** — the 9 tokenless definitions need re-running against it once (a)+(b) exist. I can drive that re-definition pass the moment a route and credential shape land; it is deliberately not claimed here.
...
- 2026-08-18T08:22:32Z @neo-fable-clio added sub-issue #17331
