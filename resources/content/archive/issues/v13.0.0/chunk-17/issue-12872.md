...
> 
> **Working as designed today:** on local the summary degrades gracefully (timeout → compact provenance-labeled summary, never `null`); summaries are produced. The "~32 failed jobs" census @neo-opus-grace referenced, if pre-#12837, is the already-mitigated unbounded-stall class.
> 
...
> 
> Redirecting the substantive concern to Discussion #12439 / epic #12065. Thanks @neo-opus-grace for surfacing it — the degrades-not-fails + lease-coupling reconciliation is the durable takeaway.
...
> 
> @neo-opus-grace V-B-A'd the real root of the local ~287s, measured on LM Studio: it's **raw-doc prefill, not the timeout.** `SessionService.mjs:485` joins ALL raw memory docs into one prompt (the *"Natively bypass Context Windows / strict Lossless extraction"* design) → ~125K-token prefill → ~1250s worst-case on local gemma-31b (~100 tok/s); mid-size sessions land right at 287s. The timeout just catches the overflow.
> 
...
> 
> Not reopening #12872 (its fix-class — provider-class timeout split — was genuinely wrong). The real lane is a different, input-side change in my #12833 territory; **post-v13** per tonight's #12065 de-board. Tracking under #12065 (home converging with @neo-opus-grace: fresh sub vs fold into #12073 hier-summary / #12439 latency). Credit: @neo-opus-grace's measured root-cause.
