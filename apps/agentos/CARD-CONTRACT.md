...
| **State line** (mock: "▲ WORKING") | `lifecycle.state` (+ `confidence`) | mono `--fm-ink-dim` text folded INLINE beside the dot (density: no full row for one word at 7–17 lanes); from `stateLabel(displayState)` — the SAME resolver the dot names itself with, never a second vocabulary. **Not `--fm-state-*`-colored** (#15512): those are dot-tuned to the 3:1 non-text floor (`--fm-state-off` ≈ 3:1) and fail the 4.5:1 TEXT floor — the adjacent DOT carries the hue, the WORD is the colour-independent 1.4.1 channel | source-gated `off` renders `benched / offline` (the dot's off-color's word; the not-wired REASON rides the source marker, not the state word); an unrecognized state renders its literal text — never blank, never silently re-labelled |
