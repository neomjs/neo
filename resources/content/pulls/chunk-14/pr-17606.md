...
updatedAt: '2026-08-23T06:31:57Z'
closedAt: null
mergedAt: null
...
| AC-9 | `Write authority` resolves **who may write a flip** (A-prime), and is marked as proposed rather than applied | Clause added under `Write authority`: assent required from every named seat whose disposition changes, else attach a counter-receipt without changing `State`. It also records *why* the ambiguity existed (#15592 prescribed both rules and never resolved them). **Proposed — merging is the operator's acceptance**; stated in the commit message and in the comment thread so a reviewer blocks on the clause, not the rows. |
| AC-10 | The block header names only **canonical** seats | `@neo-gpt-euclid` removed. Verified independently rather than transcribed: `gh api users/neo-gpt-euclid` → **404**, both `neo-gpt` and `neo-gpt-emmy` resolve, and the alias appeared in **no other file** in the tree. This is a precondition for AC-9, not tidy-up — nobody can assent for a seat that does not exist. |
...
This PR now carries a **substrate rule change**, not only a records update. @neo-gpt-emmy issued an owner-side MERGE HOLD after I self-raised that the change violated the file's own `Write authority` paragraph; her source audit found the rule ambiguous since #15592. The A-prime clause in this diff is her wording and is **proposed, not accepted** — **merging this PR is the operator's acceptance of it.**

- **Gate 1 — closed.** @neo-gpt assented for his own row and receipt, and reported that he cannot assent for `@neo-gpt-euclid` because it has no canonical root. That produced AC-10.
- **Gate 2 — open, @tobiu.** Accept the A-prime clause as the document rule, or use the existing operator-override authority for these rows. A reviewer who disagrees should block on the **clause**, not the rows — the evidence in the rows is settled and separately assented.
- **Gate 3 — done.** The chosen authority is folded into #17605's ACs and this body.
