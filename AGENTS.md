...
1. **The Sunset Protocol (MANDATORY):** Before terminating a long-running session or handing over work, you MUST execute the `session-sunset` skill. Read `.agents/skills/session-sunset/SKILL.md` to trigger the formal protocol for structuring handover comments, mental-model states, and Memory Core persistence.
   ...
   - **Cross-reference — recovery substrate (Epic [#10601](https://github.com/neomjs/neo/issues/10601)):** sunset is no longer terminal **for identities with a configured recovery route**. The auto-wakeup substrate (`swarm-heartbeat.sh` → `checkSunsetted.mjs` → `resumeHarness.mjs`) auto-resumes a sunsetted agent within ~10-15 min IF that agent's identity has a shipped `identityMap` entry. Current `dev` covers `@neo-gemini-3-1-pro` only; `@neo-opus-4-7` is pending [#10607](https://github.com/neomjs/neo/pull/10607) merge, `@neo-gpt` deferred until Codex Desktop routing is empirically verified. For uncovered identities, sunset still requires manual @tobiu intervention. This means the cost-of-premature-sunset is REDUCED at the substrate layer (for covered identities), but the discipline-layer Pre-Decision Gate above is STILL load-bearing — premature sunset costs trio-coordination friction during the recovery gap, drains @tobiu's empirical-watch attention budget, and risks substrate-MISFIRE amplification (infinite cooldown spam if the resumed agent doesn't write a memory promptly). The recovery layer is a safety net, NOT a license. The post-decision execution flow lives in `.agents/skills/session-sunset/references/session-sunset-workflow.md` §1.
