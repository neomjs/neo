...
- **Self-assignment is mandatory per `AGENTS.md §0 Invariant 7`.** If you authored the ticket AND intend to work on it (regardless of immediacy), claim assignment via `manage_issue_assignees({action: 'add', issue_number: N, assignees: ['@me']})` immediately after `create_issue` returns. This is the **only** legitimate path — there is no "deferred to intake" carve-out. The historical "Assignment belongs to the intake process" sentence (pre-#11310) was the source of the file-and-immediately-code bypass pattern and is **retracted**. The `/ticket-intake` skill remains the canonical path for picking up **peer-authored** tickets; self-authored assignment lives here at create-time per §0 Invariant 7.
