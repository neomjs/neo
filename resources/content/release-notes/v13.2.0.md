...
**Upgrade Path:** If you imported the experimental `Dock*` files published with 13.1, port to the new Dock Layouts API. See *Coming from 13.1* below.
...
For a developer, a first arrangement can now be declared as panes and zones rather than programmed step by step (#18474). A pane is a named ordinary component configuration; it does not imply a `Neo.dashboard.Panel` instance ([first-layout tutorial](https://github.com/neomjs/neo/blob/dev/learn/tutorials/DockLayoutsFirstLayout.md), [document contract](https://github.com/neomjs/neo/blob/dev/src/dashboard/dock/model/WorkspaceDocument.mjs#L50-L53)). Perspective selection follows reactive state (#18605). A window group owns transaction history and the topology needed to restore a saved arrangement across reload (#18303). The Workstation uses those engine contracts instead of carrying a second docking implementation (#18456), and the [Dock Layouts guide series](https://github.com/neomjs/neo/blob/dev/learn/guides/uibuildingblocks/DockLayoutsAdoption.md) starts with the adoption choice (#17540).
...
The three experimental `Dock*` files published with 13.1 are removed: if you imported them, port to the new dock model described above. There is no alias layer to make an old import silently mean something new. The [Dock Layouts adoption guide](https://github.com/neomjs/neo/blob/dev/learn/guides/uibuildingblocks/DockLayoutsAdoption.md) starts with the choice of workspace ownership and a first declarative arrangement.
