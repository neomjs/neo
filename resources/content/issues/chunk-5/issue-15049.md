...
updatedAt: '2026-07-12T03:22:10Z'
...
- 2026-07-12T02:19:57Z @neo-gpt-emmy assigned to @neo-gpt-emmy
### @neo-gpt-emmy - 2026-07-12T02:47:20Z

## Implementation Contract Ledger

| Target surface | Authority used | Delivered behavior | Evidence |
|---|---|---|---|
| Gemini workspace template | repository history + product transition | `.gemini/settings.template.json` and both package-ignore exceptions are removed | exact-path guard + negative fixture |
| Antigravity MCP ownership | [Antigravity 2.x MCP documentation](https://antigravity.google/docs/mcp) | global `~/.gemini/config/mcp_config.json` and workspace `.agents/mcp_config.json` are distinct valid scopes; guidance requires one owner per server | active-surface sweep + guard fixture |
| stdio identity | `StdioIdentityResolver` transport contract | `NEO_AGENT_IDENTITY` is pinned in the MCP subprocess environment, independent of UI-profile selection | source/JSDoc audit + identity-facts check |
| worktree hydration | `ai/scripts/migrations/bootstrapWorktree.mjs` | docs name the real entry point, granular data links, supported flags, and the always-build CLI boundary | source inspection + active-doc sweep |
| Antigravity debugging skill | current docs + installed-bundle probe | unproven twin-language-server causality is retired; duplication claims require process/config evidence; `--user-data-dir` remains UI-profile-only | skill-manifest + substrate + guide lint |
| regression boundary | `check-retired-primitives.mjs` | exact retired path and active Antigravity authority strings fail closed while historical blogs/releases remain outside the scan | 8 focused unit cases passing |

Decision Record impact: aligned with ADR 0018 and ADR 0020; ADR 0008's active bootstrap path was corrected. No ADR amendment is required.

Evidence: `npm run ai:check-retired-primitives`, `npm run ai:check-identity-facts`, `npm run ai:lint-guides`, skill/agent/substrate checks, and the focused unit spec all pass. Commit: `f0d1fe817`.

No residual close-target ACs. Existing Antigravity rules, concepts, wake, and artifact surfaces remain present.

- 2026-07-12T02:49:43Z @neo-gpt-emmy cross-referenced by PR #15061
- 2026-07-12T03:22:10Z @tobiu referenced in commit `63d1950` - "fix(agentos): retire Gemini workspace config (#15049) (#15061)"
- 2026-07-12T03:22:10Z @tobiu closed this issue
