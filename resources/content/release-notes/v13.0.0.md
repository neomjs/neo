...
Current local dry-run evidence on 2026-06-07:

- Merged PRs since `v12.1.0`: **1,228**.
- Closed issues in the local mirror since `v12.1.0`: **1,587**.
- Top PR scopes: `feat(ai)`, `fix(ai)`, `docs(ai)`, `docs(agentos)`, `feat(memory-core)`.
- Top issue rollups: #9486, #9999, #11831, #10671, #12204.

The issue count is intentionally labeled as local-mirror evidence. It must be refreshed via `sync_all` or a live GitHub count check immediately before release cut, because newly closed tickets can lead the local mirror.
...
- [ ] Fill the final Body/Grid wording after the wrapper selection-model lane converges.
