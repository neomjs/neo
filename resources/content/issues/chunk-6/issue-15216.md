...
updatedAt: '2026-07-16T14:48:49Z'
...
blockedBy: []
blocking:
  - '[x] 15233 Stop-hook deference lexicon: exempt correct role-attribution in human-only domains'
closedAt: '2026-07-16T14:48:49Z'
---
...
| `queued_command` + `commandMode:'prompt'` + non-empty STRING `source_uuid` + `origin.kind === 'human'` attachment | Dialogue candidate (shape-VALIDATED operator/wake delivery; any missing/malformed/non-human leg = walk-stopping autonomous boundary) | `isOperatorDialogueText` |
...
| Prompt-bearing attachment with ABSENT / `null` / blank-string `prompt` | Skipped — genuinely prompt-less kind, not a prompting record | Envelope provenance (structural) |
| Prompt-bearing attachment with a PRESENT non-string `prompt` (object/array/number) | **Malformed envelope — walk-stopping autonomous boundary** (never a skip) | Envelope provenance (structural) |
...
Corpus grounding: the full local two-project transcript census resolves **360/360 current-format prompt deliveries** with every live provenance leg, **118/118 task notifications** with neither field, and **188 pre-July prompt records** that lack `origin`/`timestamp` and therefore remain format history outside the live contract. Payload text markers (`<task-notification` in the shared synthetic patterns) remain defense-in-depth only — never the sender/kind identity.
...
- 2026-07-16T11:15:54Z @neo-opus-vega referenced in commit `ca7434b` - "fix(hooks): validate the operator-delivery envelope shape, not field truthiness (#15216)"
- 2026-07-16T11:45:27Z @neo-opus-vega referenced in commit `4e35fd8` - "fix(hooks): stop malformed present prompts, isolate mode falsifiers, sync census (#15216)"
- 2026-07-16T14:48:49Z @tobiu referenced in commit `657d6aa` - "fix(hooks): surface attachment-delivered mid-turn operator messages to the stop-hook walk (#15216) (#15222)

* fix(hooks): surface attachment-delivered mid-turn operator messages to the stop-hook walk (#15216)

* fix(hooks): classify attachment envelope provenance before payload text (#15216)

* fix(hooks): validate the operator-delivery envelope shape, not field truthiness (#15216)

* fix(hooks): stop malformed present prompts, isolate mode falsifiers, sync census (#15216)"
- 2026-07-16T14:48:49Z @tobiu closed this issue
