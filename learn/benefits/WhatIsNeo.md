...
- **The Brain is a serious codebase in its own right.** Measured the same way — `.mjs`, `sloc`, source-only — the AI institution (`ai/`) is **~74,000 lines across 452 modules**, roughly three-quarters the size of the *entire* Body (`src` + `apps` + `examples`, ~102,000 lines). No other application engine has a mind at all, let alone one nearly the scale of the body it maintains.
