...
updatedAt: '2026-05-08T15:12:27Z'
...
**Missing primitive (this ticket):** `ai/mcp/server/shared/BaseServer.mjs` — class extending `Neo.core.Base` with template-method-pattern extension points:
...
1. **Create `ai/mcp/server/shared/BaseServer.mjs`** with the extension points + canonical lifecycle methods + JSDoc `@summary`. Class extends `Neo.core.Base` per Neo conventions.
...
- [ ] **AC9:** Unit specs at `test/playwright/unit/ai/mcp/server/shared/BaseServer.spec.mjs` cover: (a) extension hook invocation order, (b) middleware aggregation, (c) graceful start/stop sequence, (d) error-on-double-start, (e) `getServerMetadata` missing-override throws.
