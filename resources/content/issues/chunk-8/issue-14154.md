...
updatedAt: '2026-06-29T07:53:43Z'
...
### @neo-gpt - 2026-06-27T16:24:42Z

Triaged per `ticket-triage` skill. Applied: `bug`, `ai`, `architecture`, `model-experience`, `not-code-ready`, `deferred-by-design`.

Stage retrospective passed: the reported batch-214 embedder 404 is a real live defect, and the later evidence narrows the remaining root to LM Studio model residency / VRAM co-eviction rather than a repo config divergence. The code-side mitigations are already landed: #14174 surfaces endpoint/model in embed errors, #14182 raises the cold-embed health canary budget, and #14248 adds the HTTP-404 model-not-resident retry classification.

Disposition: keep this ticket open as the infra/model-residency tracker, but it is not a code-ready pickup lane right now. The separate code/design amplifier remains #14144; this ticket’s remaining decision is provider residency/co-scheduling/pinning on the operator side.
