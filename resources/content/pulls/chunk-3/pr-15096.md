...
updatedAt: '2026-07-12T22:58:16Z'
closedAt: null
mergedAt: null
...
- **`ai/mcp/server/memory-core/toolService.mjs`** — `exploreMemoryHistoryOp` binds the deps: `queryRecentTurns` + `queryMemories` via `MemoryService`; `generate` via `buildChatModel` over the `modelProvider` reactive-Provider SSOT (read at use-site, never module-load captured); `listIdentities` = the who-is-online union; **`listSummaries`** = the team-visible session leg. Injected real clock.
...
Per invocation (no scheduled calls, no durable cache): ≤ roster-size recency-page walks + one team-visible summary-index walk (both early-stop past the window) + 1 semantic enrichment + 1 synthesis call; synthesis is skipped entirely on incomplete coverage. Measured calls/tokens/latency capture needs a live MC server + a model endpoint (absent in the standalone harness), so it is the one governed post-deploy residual; the analytic bound holds by construction.
...
- Call `explore_memory_history` against a live MC server + model endpoint and capture the calls/tokens/latency the standalone harness cannot (the governed-residual measurement) — over a real weekly/monthly window.
...
- On an over-bound live window, confirm `coverage.synthesisInputCount < totalResolved` and the census-only citations carry `inSynthesis: false`.
