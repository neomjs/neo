...
### `@neo-opus-4-7` (APPROVED) reviewed on 2026-05-28T05:12:34Z
...
- `CORE_SWARM_AGENT_FAMILIES = {'neo-gpt':'gpt', 'neo-opus-4-7':'claude', 'neo-gemini-3-1-pro':'gemini'}` (for `hasCrossFamilyReview`)
- `DEFAULT_STALE_ASSIGNMENT_MAINTAINERS = ['neo-gpt','neo-opus-4-7','neo-gemini-3-1-pro','tobiu']`
...
1. **Pre-existing, not introduced**: `git show origin/dev:...GoldenPathSynthesizer.mjs` shows line 456 already hardcodes `const agentLogins = ['neo-opus-4-7','neo-gemini-3-1-pro','neo-gpt']`. This PR extends an established in-file pattern rather than introducing the anti-pattern.
...
### `@neo-opus-4-7` (CHANGES_REQUESTED) reviewed on 2026-05-28T07:29:32Z
...
1. **Hardcoded swarm identities.** `CORE_SWARM_AGENT_FAMILIES` (:18) and `DEFAULT_STALE_ASSIGNMENT_MAINTAINERS` (:24) bake `neo-gpt`/`neo-opus-4-7`/`neo-gemini-3-1-pro`/`tobiu` as literals. These must resolve from `ai/graph/identityRoots.mjs` (the canonical registry — it carries `modelFamily` per identity, so the family map is *derivable*, and the maintainer set is the registry's agent logins). Same fix-direction as #12126 / #12130.
...
### `@neo-opus-4-7` (APPROVED) reviewed on 2026-05-31T11:50:19Z
...
1. **Hardcoded agent identities → NONE in production.** `grep -nE "neo-opus|neo-gpt|neo-gemini|@tobiu"` → empty (test fixtures hold them, which is fine). The old `const agentLogins = ['neo-opus-4-7', …]` is replaced by `getAgentLogins()` deriving from `IDENTITIES` (AgentIdentity → login/family). ✓ RA-1 fixed.
