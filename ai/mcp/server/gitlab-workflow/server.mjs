...
class Server extends Base {
    ...
    async initAsync() {
        ...
        logger.info('[neo-gitlab-workflow MCP] Server started on stdio transport');
        logger.info('[neo-gitlab-workflow MCP] Available tools loaded from OpenAPI spec');
    }
    ...
    logStartupStatus(health) {
        if (health.status === 'unhealthy') {
            logger.warn('⚠️  [Startup] GitLab dependencies are not available. Server will start but tools will fail until resolved.');
            ...
            logger.warn('⚠️  [Startup] GitLab configuration is partially configured. Some operations may fail.');
            health.details.forEach(detail => logger.warn(`    ${detail}`));
        } else {
            logger.info('✅ [Startup] GitLab health check passed');
        }
    }
