...
- **If the healthcheck is successful:** The Memory Core is active.
    ...
    - **Automatic Wake Substrate Bootstrap:** Wake-subscription bootstrap is automatic during Memory Core MCP initialization. Normal startup must **not** manually call `manage_wake_subscription({ action: 'bootstrap' })`.
