...
updatedAt: '2026-08-07T05:48:47Z'
closedAt: null
mergedAt: null
...
| Surface | Change |
|---|---|
| `classifyBundleRecoverability` | new — per-substrate payload bytes + meta presence |
...
| retention logging | names → per-substrate byte counts on every keep and drop |
| `backup-retention.spec.mjs` | 14 → **22** tests; `seedBackup` now builds real bundles |
...
```
npm run test-unit -- test/playwright/unit/ai/scripts/maintenance/ --workers=1
  615 passed (7.9s)
```

`backup-retention.spec.mjs`: **22 tests**, up from 14. The pre-existing 14 kept their intent — `seedBackup` now produces a *real* bundle (meta + non-empty payload) by default, because a payload-less directory models the **empty** bundle, not the normal one, and that conflation is precisely what this change removes. `{restorable: false}` opts into the empty shape.
