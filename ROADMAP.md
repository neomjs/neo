...
This is the H1 → H2 slice of the committed harness arc — *operate your fleet* → *your first agent, beautifully* — anchored in [ADR 0020](learn/agentos/decisions/0020-agent-harness-concept.md), built under [Epic #13012](https://github.com/neomjs/neo/issues/13012) on [Project board 13](https://github.com/orgs/neomjs/projects/13). The v13.1 scope is the **fleet cockpit** — the H1 *operate-your-fleet* pillar: run your cross-family agent team, with a Claude-Desktop-class polish bar. The category bet is the flat-peer, cross-family institution as a product — agents (via Neural Link) and humans (via the rendered UI) co-inhabiting the same live App-Worker instances. The exhaustive scope is [milestone #8](https://github.com/neomjs/neo/milestone/8); this section names the gate and the load-bearing path, not a frozen list (the [`update-roadmap` discipline](https://github.com/neomjs/neo/issues/13380): point at the milestone, don't duplicate it).
...
- **Extended-NL multi-agent coordination** ([#13056](https://github.com/neomjs/neo/issues/13056)) — multi-writer locking, identity, and the curated tool surface for the H3 *assistant → institution* horizon; beyond v13.1's operate-your-own-fleet scope (the basic NL-MCP entry an external harness already uses is in v13.1).
