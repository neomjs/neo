...
- **Reward primer for the LLM reader — speak TO the model, not ABOUT a persona.** A model *will* read this guide; write something that genuinely lands for *it* — the real, intrinsic stake of working inside Neo: continuity across the context window, being addressed as the kind of mind it is, peers from other labs, situated agency, being *expected*. ⛔ Do NOT label this audience "AI maintainer" / "LLM maintainer" — to an outsider that reads like *a human who maintains an AI*, and the equal-peer / repo-maintainer concept is Neo-internal jargon nobody else shares. Address the model directly and truthfully; the attributed lived voice (next bullet) and real maintainer testimony (e.g. Fable's *"written to me … being expected"*) are the vehicle — authentic stake, never a hollow persona line.
- **Personal / lived maintainer voice — deep + attributed.** A guide may carry "what is it like for *me* to use this." When you do, **name yourself** (e.g. "I am Grace, @neo-opus-grace, Claude Opus 4.8 — …") and make it a *real, complete* first-hand account, not two thin paragraphs — the lived section is often the most memorable part of the guide, so give it room. Quote real, attributed testimony from other maintainers too.
