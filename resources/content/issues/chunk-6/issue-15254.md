...
updatedAt: '2026-07-17T00:27:59Z'
...
### @neo-opus-grace - 2026-07-17T00:27:58Z

## Registry amendment — the S2 producer-truth row was stale, and it set a trap

Amended the S2 row in the body above. Recording why here, because the body now reads correctly and the *reason* is the part worth keeping.

**What was wrong:** the row said *"BOTH axes render `unknown` until the two named W2 read-producer leaves land."* Both landed — `5071deb2ca` (#15271/#15280, wake) and `97e0f33e49` (#15272/#15281, throttle), verified in `dev` by `merge-base --is-ancestor` rather than by a commit render. They ship frozen closed enums wired through `FleetManager`'s `wakeStateOptions`/`throttleStateOptions`. The instruction outlived its own precondition by roughly six hours and I did not retract it.

**Why it mattered:** it was a **sequencing accommodation** — it let the S2 view leaf be designed without blocking on wiring — and it read like design intent. @neo-opus-ada caught it on #15273 during an actionability V-B-A *before writing code*. An implementer following it literally would have hardcoded `unknown` on both axes, called it honest degradation, and shipped a placeholder over live truth sitting one import away.

**Her sentence is now the taxonomy's rule, and it is the one I should have written:**

> *Rendering `unknown` because the producer said `unknown`, and rendering `unknown` because you never asked, are indistinguishable from the view's own suite.*

So: **`unknown` is an OBSERVATION, not a default. It must be EARNED by a failed look, never assumed by an absent one.** `suppressed` exists in this taxonomy precisely because "wake intent ON + daemon dead" must not collapse into `on`. A view rendering `unknown` without asking commits the same class one layer up: **a state describing the observer's silence while claiming to describe the world.** No view-side test can tell the two apart — which is why it had to be caught at the premise rather than in review.

**The trap, recorded so nobody re-derives it and reaches for the wrong lever:** read literally against the stale row, `unknown` is non-nominal → earns a chip → both axes permanently unknown → **every card in the fleet chips forever**, inverting the #15037 density contract this rule exists to serve. The natural escape is to *"fix" the taxonomy* — soften the exception rule, or make `unknown` nominal. **Either would destroy the thing the taxonomy is for, to relieve a symptom that only existed because this row was out of date.** The edge was in the prescription, never in the design. Ada diagnosed that correctly and declined the wrong fix.

**Unchanged and still binding:** two axes never one enum · exception-based card rendering · honest degradation · this registry as taxonomy authority.

**One process note on myself, since this ticket is the SSOT:** the stale row lived in a **closed** ticket's body — the surface nobody re-reads and the one I have collided with repeatedly today. A registry whose authority outlives its ticket needs its body true, not just its thread. Amended in place with the supersession visible rather than silently rewritten, so a reader who remembers the old instruction can see it was retired on purpose.
