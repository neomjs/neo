...
updatedAt: '2026-05-26T02:32:40Z'
...
### @neo-gpt - 2026-05-26T02:32:40Z

Closing this stale documentation ticket after live re-triage.

Original intent: prevent bootstrap-time category drift by making agents see Neo as engine + Agent OS / Brain rather than a web framework with AI tools attached. That intent is now satisfied by newer source-of-authority changes rather than by the originally proposed full `CodebaseOverview.md` rewrite.

V-B-A evidence:
- `AGENTS_STARTUP.md` Step 1 now reads `README.md` first as the organism identity / Four Pillars / Agent OS / MX-loop anchor, then `learn/guides/devindex/frontend/Architecture.md` for concise frontend architecture. It explicitly demotes `learn/guides/fundamentals/CodebaseOverview.md` to long-form reference material, not mandatory bootstrap.
- `README.md` now frames Neo as a self-evolving digital organism with Brain, Institution/Swarm, Body, and Evolution/MX-loop; this landed in `b59eef11f` (`docs(readme): rewrite around 4-pillar digital-organism identity (#10452) (#10455)`).
- `learn/benefits/ArchitectureOverview.md` already owns the requested Two Hemispheres, Cognitive Loop, Dream Pipeline, Golden Path, Neural Link, and Closed Loop reference sections; initial guide landed in `fc6bf3cb0` (`docs: add Agent OS Architecture Overview guide (#9981) (#9982)`).
- `learn/guides/fundamentals/CodebaseOverview.md` already links to `ArchitectureOverview.md`, `SwarmIntelligence.md`, and `DreamPipeline.md`, and already documents the Librarian / QA / Browser agent profiles.

Keeping #10136 open now asks for a low-ROI rewrite of a document that is no longer the boot mental-model authority. If a future gap appears, it should target the current boot anchors (`README.md`, `AGENTS_STARTUP.md`, `learn/benefits/ArchitectureOverview.md`) with a narrow new issue rather than reviving this stale prescription.

- 2026-05-26T02:32:41Z @neo-gpt closed this issue
