...
- **Budgeted Review Closure:** Post-cutover PRs get **one ordinary `CHANGES_REQUESTED` per canonical reviewer family**, counted across heads, identities, and retractions — so a family cannot buy a second round by rotating seats, and another family keeps its own independent round. A submitter the identity graph cannot classify is refused rather than waived. Round 2 is then a **disposition** over the Round-1 actions (`ADDRESSED` / `DEFENDED` / `STILL_OPEN`), carrying no premise snapshot, audit rerun, or metrics restatement: a round that can always find a new concern is not terminal, so fresh findings there are accepted risk. Convergence otherwise continues through approval/polish, Approve+Follow-Up, one validator-complete terminal Drop+Supersede, or a guarded repair-minted re-entry whose receipt names the prior head, repaired head, prior fact, and repair coordinate. The bound is the **demand**, not the review state: a demand round is a `CHANGES_REQUESTED`, a `COMMENT` never opens a new action packet (on submission or on a later edit), and an approval's follow-up must cite an *independent* owning issue — never a coordinate, never the PR's own close target. Binding on state instead was the measured defect twice over: first the budget read only one channel, then the channel guard read only families that had already spent a round, so declining to use the enum avoided the round entirely. Discussion size remains cost telemetry, never a scope classifier.
- **LGTM/Required Actions:** Ensures every review resolves in a clear state.
- **Micro-Review (blast-scaled Cycle-1):** A mechanical PR — no architectural concept to teach (test-only / config-leaf / behavior-preserving / docs / receipt refresh), any size — may take the light `# PR Micro-Review` form (`Class` + `Verdict` + `Glance` + origin session; guide §6.4) instead of the full gauntlet. Never for ADR / new-abstraction / consumed-contract / security / migration / fleet-critical zones; authors may signal eligibility in the PR body, reviewers decide, and escalating to the full form is always free.
