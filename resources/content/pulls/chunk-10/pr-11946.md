...
Scans `ai/daemons/` for `@neo-claude-opus` / `@neo-gpt` / `@neo-gemini-pro` in string-literal form. Daemon code must resolve identities via `ai/graph/identityRoots.mjs` registry, never via hardcoded literals.
...
**String literals preserved during scan** — AC1-3 specifically target string-literal occurrences (e.g., `'@neo-claude-opus'` would be a violation, but a comment saying "agents like @neo-claude-opus" would not). The scan strips comments but preserves string literals exactly per AC1-3 scope.
...
- [ ] Operator confirms the invariants catch deliberate regression — e.g., temporarily inserting `const me = '@neo-claude-opus';` into a daemon file should turn AC1 red.
