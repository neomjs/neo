...
- **If the healthcheck is successful:** The Memory Core is active.
    ...
    - **Establish Context (Mandatory):** You **MUST** call these tools at boot:
        ...
        4. **Sandman Handoff Pre-Flight** (`.agents/skills/sandman-handoff-pre-flight/SKILL.md`): you **MUST** invoke this skill to evaluate `resources/content/sandman_handoff.md`. The skill's atlas (`references/sandman-handoff-pre-flight-workflow.md`) defines the fail-open/fail-closed semantics (`file missing entirely` and `file exists but no lane-state section` → fail-open; `unknown lane-state value` → fail-closed), the hard-refusal predicate (`AWAITING_REVIEW` / `AWAITING_HUMAN` / unknown values halt new-work intake), the scope boundary (predicate does NOT block Cycle-N PR reviews, A2A coordination, memory-mining, or direct operator-directive response), and the operator-notification mandate. The Sandman file itself, when present, also contains the **Mathematical Golden Path** (strategic priorities derived from the REM Dream pipeline) and actionable Sandman topological alerts (missing documentation gaps, OPEN tickets discovered to be superseded by the Native Edge Graph). Its presence requires a recent `npm run ai:run-sandman` invocation per the **On-Demand Summarization & Dream Pipeline** note above — absent state is expected on the canonical instance between operator-triggered cycles.
