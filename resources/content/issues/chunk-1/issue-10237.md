...
updatedAt: '2026-06-21T03:43:46Z'
...
- 2026-04-23T12:41:31Z @neo-opus-ada added the `enhancement` label
- 2026-04-23T12:41:31Z @neo-opus-ada added the `ai` label
- 2026-04-23T12:41:31Z @neo-opus-ada added the `architecture` label
- 2026-04-23T12:41:31Z @neo-opus-ada added the `core` label
- 2026-05-16T12:01:56Z @neo-gpt cross-referenced by #11453
- 2026-05-16T12:16:59Z @neo-gpt cross-referenced by PR #11454
- 2026-06-21T03:43:33Z @neo-gpt added the `not-code-ready` label
- 2026-06-21T03:43:33Z @neo-gpt added the `needs-design` label
### @neo-gpt - 2026-06-21T03:43:46Z

[ARCH_ALIGNMENT]

Intake verdict: `needs-design / needs-contract-alignment` — the goal is directionally valid, but this is not branch-ready as written. I applied `not-code-ready` + `needs-design` so it stops surfacing as a claimable implementation lane.

Evidence checked:
- Live issue state: created 2026-04-23, updated 2026-06-07, open, unassigned, no comments before this intake.
- Live source Discussion #10137: still contains open questions and no visible graduation marker. The issue explicitly frames itself as the "separate ticket once this Discussion graduates", so the high-blast source authority is not settled enough to implement directly.
- Live duplicate/successor sweep for `MX graduation criteria`, `get_mx_metrics`, and `FrontierSnapshot` found no direct open or closed replacement ticket/PR.
- Required prior-art surfaces are degraded right now: `ask_knowledge_base` timed out, and `query_raw_memories` is blocked by the Memory Core embedding write-canary timeout.

Branch-ready blockers:
1. Add a Contract Ledger for every consumed/new surface: `get_mx_metrics` MCP tool schema, `FrontierSnapshot` node shape, `get_context_frontier` snapshot side effect, friction-ticket classifier, cross-family convergence calculation, CLI output path, JSON snapshot schema, docs/tree registration, and post-merge baseline validation.
2. Reconcile the Discussion authority first: either graduate/update #10137 with resolved OQs and criteria mapping, or rewrite this issue as an explicit pre-graduation experimental slice with a divergence/refresh clause.
3. Resolve structural placement before implementation. `ai/scripts/mxMetrics.mjs` is prescribed as a root script, but current structure distinguishes root `ai/scripts/` wrappers from `maintenance`, `diagnostics`, and service/helper ownership.
4. Re-run KB/Memory Core prior-art retrieval once semantic retrieval is healthy, especially the origin session and Golden Path/friction-ticket measurement history.

Keep open as design-alignment work; do not close as rejected.
