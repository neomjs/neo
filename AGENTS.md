...
- **Wake/Heartbeat-without-active-lifecycle:** Heartbeats are next-lifecycle-event prompts, not passive notices. Failure mode = silent mark-read across a heartbeat window while producing ZERO substrate-evolution signals (PRs, design dialogue, peer reviews, A2A coordination, ticket triage/retractions, skill improvements, ideation graduations — per §contributions_over_commits). Substrate-correct alternatives: (a) substantive in-flight work this window, (b) declared `lane-state: paused — <named reason>` (post-ship coordination / peer-review-in-flight / operator context-switch), (c) named halt after querying unassigned current-release Project, with explicit zero-candidate reason. Exhausted-self-assigned-bench is NOT a named halt. **Critical threshold:** 3 consecutive delivered heartbeat/wake pulses without progress evidence is a critical failure; load `/post-review-pickup` + `NightShiftLeasedDriver.md` and route recovery instead of declaring another pause/halt. See `peer-role-mode.md §7`.
