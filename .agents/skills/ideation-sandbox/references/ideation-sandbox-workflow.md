...
**Closure is evidence-state, never a clock (#15996):** the window closes when the mandatory non-author cycle has landed AND the latest cycle added no new option row or falsifier (a cycle = a non-author comment adding a row or a falsifier; acks are not cycles). No invented `until <ts>` — a clock cannot be shortened by evidence; only an operator-set bound is valid, cited as theirs. **Exhaustion trigger:** two consecutive cycles with no decision-changing delta ⇒ open convergence on present evidence or split, recording which. Dryness predicate, not a volume count; the audit below reconciles it with the divergence-theater guard (exit-forcing ≠ entry-gating count).
