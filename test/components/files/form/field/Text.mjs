StartTest(t => {
    ...
    t.it('Check isTouched on focusEnter', async t => {
        await setup({isTouchedEvent: 'focusEnter', value: 'Hello World!'});
        t.is(inputField.value, 'Hello World!');
        t.hasNotCls(fieldComponent, 'neo-is-touched');
    ...
    t.it('Check isTouched set initially', async t => {
        await setup({isTouched: true, value: 'Hello World!'});
        t.is(inputField.value, 'Hello World!');
        t.hasCls(fieldComponent, 'neo-is-touched');
    });
    ...
    t.it('Label Positions', async t => {
        ...
        t.diag('top');
        await Neo.worker.App.setConfigs({id: testId, labelPosition: 'top'});
        await t.waitFor(50);
        t.hasCls(fieldComponent, 'label-top');

        t.diag('bottom');
        await Neo.worker.App.setConfigs({id: testId, labelPosition: 'bottom'});
        await t.waitFor(50);
        t.hasCls(fieldComponent, 'label-bottom');

        t.diag('right');
        await Neo.worker.App.setConfigs({id: testId, labelPosition: 'right'});
        await t.waitFor(50);
        t.hasCls(fieldComponent, 'label-right');

        t.diag('inline');
        await Neo.worker.App.setConfigs({id: testId, labelPosition: 'inline'});
        await t.waitFor(50);
