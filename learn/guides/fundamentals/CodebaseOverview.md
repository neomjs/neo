...
- `pr-review`: Evaluation matrix templates spanning `[ARCH_ALIGNMENT]` to `[EFFORT_PROFILE]` + graph ingestion tags for the Dream Pipeline + deterministic budgeted closure (one ordinary RC **per canonical reviewer family**, counted across heads and retractions and failing closed on an unclassifiable submitter; then a disposition-only Round 2 over the Round-1 actions, approval / polish / guarded A+FU / one validator-complete terminal D+S / one guarded repair-minted re-entry) + a prior-art Memory Core sweep gate before scoring (CI-green ≠ AC-met) + a conditionally-loaded reviewer-instrument audit for capability gates that check existence rather than execution, and for absence claims made without a positive control (enforces mandatory ROI template usage).
