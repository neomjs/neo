...
| **MCP-server count** | `package.json` scripts matching `ai:mcp-server-*` — NOT `ls ai/mcp/server/` (that returns 7: it includes `shared/` infra + `gitlab-workflow/` PoC; functional = 5: knowledge-base, memory-core, github-workflow, neural-link, file-system) | derive into prose/JSON or lint coherence-check against the script manifest | README; AI_QUICK_START; Introduction.md; index.html ld+json + FAQ; AiToolchain.mjs; llms.txt header; `.gemini/settings.template.json`; `.codex/config.template.toml` |
