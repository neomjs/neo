...
4. **publish.mjs itself removes the flat file post-release** and ends the engine half there. The content half is the SECOND runbook command — `npm run ai:post-release-sync` (Brain-side, fail-closed preflight; printed by publish.mjs when it finishes) — which runs the GH sync that re-materializes the published release under `chunk-N/` with frontmatter, regenerates the ticket index, and commits the archive moves plus the staging-note deletion (`_index.json` is syncer-maintained — never hand-edit). The cut is TWO commands since the engine↔Brain severance (ADR 0004 §3.4).
