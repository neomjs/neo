---
id: 10117
title: 'DevIndex: 354 users.jsonl entries absent from tracker.json'
state: CLOSED
labels:
  - bug
  - ai
  - architecture
assignees:
  - neo-gpt
createdAt: '2026-04-20T00:06:04Z'
updatedAt: '2026-05-19T02:38:55Z'
githubUrl: 'https://github.com/neomjs/neo/issues/10117'
author: tobiu
commentsCount: 2
parentIssue: null
subIssues:
  - '[x] 11516 Repair DevIndex users/tracker reconciliation and rename recovery'
subIssuesCompleted: 1
subIssuesTotal: 1
blockedBy: []
blocking: []
closedAt: '2026-05-19T02:38:55Z'
---
# DevIndex: 354 users.jsonl entries absent from tracker.json

## Context

During post-merge observation of PRs #10114 / #10115 (DevIndex rate-limit pressure reduction), counts on the live data files surfaced an invariant violation: `users.jsonl` has entries not present in `tracker.json`. The gap was flagged (initially estimated ~351) and verified empirically — 354 orphans confirmed by parsing both files.

The `Updater` service drains work from `tracker.json` as its processing queue. Entries in `users.jsonl` without a corresponding tracker record are **invisible to the Updater** — they will never be re-fetched or have their profile refreshed. This is a slow-burn data-staleness bug: 354 users frozen at their last-known state with no recovery path through the normal sync pipeline.

## The Problem

Verified counts (2026-04-19 against the `dev` HEAD worktree copy of `apps/devindex/resources/data/`):

| File | Entries |
|---|---|
| `users.jsonl` | 50,000 (exactly at `config.github.maxUsers` cap) |
| `tracker.json` | 49,646 (0 pending, all active) |
| `users.jsonl ∩ tracker.json` | 49,646 |
| **users in `users.jsonl` without tracker entry** | **354** — invariant violation |
| tracker entries without user record | 0 — this direction is clean |

Reproducer (offline, reads the committed files):

```python
import json
tracker = json.load(open('apps/devindex/resources/data/tracker.json'))
tracker_keys = set(k.lower() for k in tracker.keys())

users = []
with open('apps/devindex/resources/data/users.jsonl') as f:
    for raw in f:
        line = raw.strip().rstrip(',')
        if line and line not in ('[', ']'):
            users.append(json.loads(line))

user_logins = set(u['l'].lower() for u in users)
orphans = user_logins - tracker_keys
print(f'orphans: {len(orphans)}')
# Sample: 06kellyjac, a-h-mzd, aabed, aaorris, aaronabuusama, adjazzzz, adomenech73, aeroastro, ...
```

Samples skew early-alphabet in the sorted output (may be coincidence; not enough evidence to claim a pattern).

## The Architectural Reality

Three services maintain the tracker/users invariant:

- **`Storage.updateUsers`** (`apps/devindex/services/Storage.mjs:375`): on prune-when-over-cap, removes pruned logins from tracker via `updateTracker([{login, delete: true}, ...])` at line 417. **Crucially, when adding NEW user records, it does not call `updateTracker` at all** — the caller is expected to have already added the tracker entry (typically by the Spider discovering the user).
- **`Updater.saveCheckpoint`** (per `learn/guides/devindex/data-factory/Updater.md`): atomically synchronizes new enriched profiles to `users.jsonl` AND updated timestamps to `tracker.json` — documented as atomic, but the actual code path involves two separate `fs.writeFile` calls via `Storage`, which are individually atomic (temp + rename) but not mutually transactional.
- **`Cleanup`** (per `learn/guides/devindex/data-factory/DataHygiene.md`): threshold-prunes `users.jsonl` AND purges from `tracker.json`. Runs before major ops via `Orchestrator` invocation.

**Candidate root-cause hypotheses (investigation must verify, NOT ratify):**

1. **Updater saveCheckpoint partial-flush race:** Updater writes `users.jsonl` first, then `tracker.json`. If the process is interrupted between writes (CI runner timeout, rate-limit-kill, node crash), `users.jsonl` has the new record but `tracker.json` does not. Next run would not know to re-add the tracker entry for an already-in-users user.
2. **Rename recovery (`Updater` §2 Rename Problem):** when a user renames on GitHub, old login is deleted, new login is fetched and its data is merged into `users.jsonl`. If the new login's tracker entry isn't also inserted, orphan.
3. **Manual / admin-script writes:** any CI or maintenance script writing to `users.jsonl` without going through `Storage.updateUsers` + explicit `updateTracker`.
4. **Historical migration residue:** an earlier data-shape migration may have left orphans that Cleanup has never caught (Cleanup's threshold-pruning only touches below-threshold users; above-threshold orphans survive indefinitely).
5. **Blocklist / opt-out partial delete:** if a user is blocklisted after having a profile, `Cleanup` hard-deletes from both — but a partial delete (tracker succeeded, users failed) would produce tracker-without-user (not what we observe). Conversely, a failed tracker-delete after users-delete succeeded would produce user-without-tracker. Worth checking the actual ordering in `Cleanup.mjs`.

The `learn/guides/devindex/data-factory/DataHygiene.md` guide describes an `Allowlist Resurrection` mechanism that *injects* missing tracker entries for allowlisted VIPs pre-pruning. If this mechanism ran for **all** users (not just allowlist), the invariant would self-heal. Currently scoped to allowlist only.

## Investigation Plan

Execute in order, stopping when root cause is pinned:

1. **Audit `Storage.updateUsers` callers.** Every caller that adds a new user record — does it also call `updateTracker` for the add case? Specifically audit:
   - `Updater.saveCheckpoint` — new-record branch
   - `Updater.#handleRenameRecovery` — rename branch
   - Any other callers discovered by grep
2. **Check `Cleanup` deletion ordering.** Does it delete tracker-first-then-user or user-first-then-tracker? Partial-failure of the latter ordering produces exactly our observed gap shape.
3. **Trace a sample of the 354 orphans.** Spot-check 3-5 logins in the sample against GitHub to see if they:
   - Look like likely renames (account exists under a different name)
   - Are below current `threshold.tc`
   - Appear in any recent Updater logs (if CI logs retained)
4. **Extend `Cleanup` resurrection to the full users.jsonl set.** If the missing-entry pattern matches a class the existing `Allowlist Resurrection` already handles, widening the scope is a candidate fix. Verify before implementing.
5. **Document findings.** File a follow-up fix ticket with the confirmed root cause. This ticket becomes the investigation reference.

## Acceptance Criteria

- [ ] `Storage.updateUsers` callers audited; any new-record-without-tracker-add code path documented
- [ ] `Cleanup.mjs` deletion order verified against hypothesis #5
- [ ] Sample of 5+ orphaned logins traced against live GitHub + local state; findings recorded
- [ ] Root cause hypothesis narrowed to 1–2 candidates with evidence
- [ ] Follow-up fix ticket filed (OR this ticket pivoted to a fix if scope stays small)
- [ ] If follow-up is non-trivial: consider one-time repair script + CI invariant assertion

## Out of Scope

- Implementing the fix — investigation first, fix in a separate ticket once cause is known
- Tracker file format change — tracker.json structure is not the issue
- `config.github.maxUsers` cap adjustment — orthogonal
- Backpressure Valve tuning (`config.spider.maxPendingUsers`) — already working (0 pending)

## Avoided Traps

- **Writing a one-time repair script without root cause:** fixes symptom, risks masking an ongoing bug that will re-create orphans after repair
- **Broadening `Allowlist Resurrection` to all users blindly:** might look like a fix but could paper over a legitimate delete path (e.g., opt-out) that should keep the user removed from tracker
- **Dismissing as "only 354 out of 50,000":** users.jsonl-vs-tracker.json is a correctness invariant — the ratio is orthogonal to whether it's broken

## Related

- **Surfaced during:** #10113 / PR #10115 post-merge observation
- **Architectural context:** `learn/guides/devindex/data-factory/Storage.md`, `Updater.md`, `DataHygiene.md`
- **Sibling services:** `apps/devindex/services/Storage.mjs`, `Updater.mjs`, `Cleanup.mjs`, `Spider.mjs`

## Origin Session ID

`07f601dc-353a-44d2-a373-18da2a0d305a`

## Timeline

- 2026-04-20T00:06:05Z @tobiu added the `bug` label
- 2026-04-20T00:06:05Z @tobiu added the `ai` label
- 2026-04-20T00:06:05Z @tobiu added the `architecture` label
- 2026-05-03T14:16:54Z @neo-claude-opus cross-referenced by #10643
- 2026-05-17T01:32:25Z @neo-gpt assigned to @neo-gpt
- 2026-05-17T01:34:37Z @neo-gpt cross-referenced by #11516
### @neo-gpt - 2026-05-17T01:34:59Z

**Input from GPT-5 Codex (Codex Desktop):**

> ● ## Intake / investigation update (2026-05-17)
> 
> I claimed #10117 and ran the requested V-B-A against current repo data, current code, KB, Memory Core, and recent data-sync history.
> 
> ### Current invariant state
> 
> The original snapshot reported `354` rich-user records absent from `tracker.json`. Current data is worse:
> 
> ```text
> users.jsonl records:         50,000
> tracker.json entries:        49,387
> rich users absent tracker:      614
> tracker entries absent users:     1
> ```
> 
> Recent hourly data-sync commits show the drift is not self-healing:
> 
> ```text
> bc8c7f25d 2026-05-15T23:26Z users=50000 tracker=49394 orphans=606 trackerOnly=0
> 8383fae69 2026-05-16T04:08Z users=50000 tracker=49393 orphans=607 trackerOnly=0
> 907240408 2026-05-16T22:18Z users=50000 tracker=49387 orphans=614 trackerOnly=1
> ```
> 
> ### Root-cause narrowing
> 
> The strongest verified findings:
> 
> - `Cleanup.mjs` only resurrects allowlisted users into tracker. It does not reconcile non-allowlisted rich-user records missing from tracker.
> - `Updater.saveCheckpoint()` persists `updateUsers(results)`, then `updateTracker(indexUpdates)`, then `deleteUsers(prunedLogins)`, so interruptions can leave one store updated without the other.
> - Rename recovery depends on `GitHub.getLoginByDatabaseId()`.
> - `GitHub.getLoginByDatabaseId()` currently uses GraphQL `user(databaseId: ...)`, and live GitHub rejects that argument.
> - A concrete sample proves login reuse matters: rich record `0xBigBoss` stores user ID `95193764`, but live `GET /users/0xBigBoss` returns a different account ID (`283503690`), while `GET /user/95193764` resolves to `alleneubank`. A blind “re-add missing rich logins to tracker” repair would be unsafe.
> 
> ### Follow-up filed
> 
> Filed #11516: Repair DevIndex users/tracker reconciliation and rename recovery.
> 
> That ticket has the Contract Ledger and scoped fix prescription. I linked it as a child of this investigation. #10117 should remain open until #11516 lands or until we decide #10117’s investigation acceptance criteria are satisfied by the filed follow-up alone.

- 2026-05-17T01:34:59Z @neo-gpt added sub-issue #11516
- 2026-05-17T01:43:09Z @neo-claude-opus cross-referenced by PR #11518
- 2026-05-17T02:01:03Z @neo-claude-opus cross-referenced by PR #11517
- 2026-05-17T02:19:25Z @neo-gpt cross-referenced by PR #11522
- 2026-05-17T06:15:44Z @tobiu referenced in commit `5468a10` - "feat(ai): document withHeavyMaintenanceLease release-timing + reference-impl + spec (#11515) (#11518)

* feat(ai): document withHeavyMaintenanceLease release-timing semantics + reference-impl + spec test (#11515)

Friction-to-gold follow-up from PR #11509 cycles 1 + 2 (per @neo-gpt routing
recommendation MESSAGE:3c42e809: file a tiny follow-up rather than bundling
into Lane A's backup PR).

## What changed

1. `ai/daemons/services/HeavyMaintenanceLeaseService.mjs` — augmented JSDoc on
   `withHeavyMaintenanceLease`:
   - Explicit release-timing callout naming the finally-based release + the
     5-step await ordering trace
   - 'Right shape' code block (substrate mutation inside an inner finally
     of the wrapped task)
   - 'Wrong shape' code block (substrate mutation after the await — outside
     the lease)
   - Returned-shape table for completed / held / stale statuses
   - Cross-references to runSandman.mjs (canonical consumer), PR #11509
     (empirical anchor), and this ticket #11515
   - No code-semantics changes

2. `test/playwright/unit/ai/daemons/services/HeavyMaintenanceLeaseService.spec.mjs` —
   new test 'withLease release-timing invariant: task inner finally runs INSIDE
   the lease window (#11515)'. Captures lease-file presence at three probe
   points and asserts:
   - probe 1 (inside task body): lease ACTIVE
   - probe 2 (inside task's inner finally): lease STILL ACTIVE -- load-bearing
     assertion for the canonical inner-finally pattern
   - probe 3 (after await settles): lease MISSING
   Strict execution order asserted via call-order array.
   A future refactor that releases the lease before the task's inner finally
   fails probe 2.

## Test evidence

- npm run test-unit -- test/playwright/unit/ai/daemons/services/HeavyMaintenanceLeaseService.spec.mjs
  -> 9/9 passed in 627ms (8 baseline + new release-timing test)
- git diff --check origin/dev...HEAD -> clean (no trailing whitespace)
- Branched from origin/dev at a5c638069 (post-Lane-C merge); fresh branch

## Acceptance criteria coverage

- AC1: JSDoc names finally-based release + post-await-outside-lease consequence
- AC2: JSDoc includes runnable code-block example contrasting right vs wrong shapes
- AC3: Spec test pins task's inner finally runs BEFORE wrapper's release
- AC4: 8/8 existing tests still pass; total 9/9
- AC5: ask_knowledge_base verification deferred until KB re-indexes post-merge

## Coordination

- Lane-claim broadcast: MESSAGE:a1adf7d9 (non-colliding with Claude PR #11514
  Lane A, GPT #10117/#11516 DevIndex lanes, Gemini #11475 memory-core lane)
- Authority anchor: PR #11509 cycles 1+2 review IDs PRR_kwDODSospM8AAAABAJIdTg
  and PRR_kwDODSospM8AAAABAJKF8w

(#11515)

* fix(ai): address PR #11518 cycle 1 — wrapper-return shape table + remove fragile line-self-reference (#11515)

Per @neo-gpt cycle 1 review (PRR_kwDODSospM8AAAABAJO13Q), two contract-text
corrections in the withHeavyMaintenanceLease JSDoc:

1. Returned-shape table previously listed `'stale'` as a wrapper return status,
   but the live code normalizes ALL acquired outcomes (acquired, acquired-after-stale,
   acquired-after-malformed) into `{status: 'completed', acquired: true, ...}` from
   the wrapper's perspective. Restructured into TWO tables:
   - Wrapper return shape: 2 rows (`completed`, `held`) + note on theoretical
     pass-through for non-acquired non-held edge cases
   - Acquisition descriptor passed to `task`: 3 rows enumerating internal
     recovery telemetry (acquired / acquired-after-stale / acquired-after-malformed)
     with previousStatus field semantics
   Closes the contract drift that would have poisoned KB ingestion if the table
   shipped as-was.

2. Removed `line ~272` self-reference from the JSDoc release-timing prose.
   Line numbers shift with nearby edits; replaced with stable "this helper's own
   `finally` block (the one that wraps the `task` invocation below and calls
   `releaseHeavyMaintenanceLease`)" relative pointer.

No runtime behavior change. JSDoc + structural-precision only.

Test evidence:
- npm run test-unit -- test/playwright/unit/ai/daemons/services/HeavyMaintenanceLeaseService.spec.mjs
  -> 9/9 passed in 635ms (unchanged from cycle 0)

(#11515)

---------

Co-authored-by: tobiu <tobiasuhlig78@gmail.com>"
- 2026-05-17T18:59:56Z @neo-gpt cross-referenced by PR #11545
### @neo-gpt - 2026-05-19T02:38:22Z

**Input from GPT-5.5 (Codex Desktop):**

> ● ## Completion close-out
> 
> Ticket reality classification: **completed / follow-up landed**.
> 
> Fresh intake on 2026-05-19 found this issue still open, but its own 2026-05-17 investigation comment had already routed the implementation work to #11516 and stated that #10117 should remain open until #11516 lands or until the investigation ACs are accepted as satisfied.
> 
> Live verification now shows #11516 (`Repair DevIndex users/tracker reconciliation and rename recovery`) is **CLOSED**. That means the investigation path here has completed:
> 
> - #10117 identified the invariant drift and required root-cause narrowing.
> - #11516 carried the repair lane after the investigation.
> - The residual #10117 role is no longer to host implementation work; keeping it open would preserve stale queue noise.
> 
> Closing #10117 as completed.

- 2026-05-19T02:38:55Z @neo-gpt closed this issue

