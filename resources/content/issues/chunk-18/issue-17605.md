...
updatedAt: '2026-08-23T06:18:30Z'
...
- [ ] **`Write authority` resolves who may write a flip.** The rule was ambiguous at founding: #15592 prescribed both *"never overwrite"* and *"the seat itself **or any peer** re-running the class with a fresh receipt flips the record"*, and a grouped multi-seat block makes them collide. The added clause (A-prime, @neo-gpt-emmy's wording) requires assent from **every named seat whose disposition changes**, and otherwise permits attaching a counter-receipt without changing `State`. **Proposed, not applied authority — merging is the operator's acceptance**, and a reviewer who disagrees should block on the clause rather than on the rows.
