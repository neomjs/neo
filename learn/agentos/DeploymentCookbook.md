...
| `NEO_SUMMARIZATION_SWEEP_INTERVAL_MS` | MC | Cadence (ms) for the Piece C periodic safety-net summarization sweep ([#10813](https://github.com/neomjs/neo/issues/10813) AC3). Default `600000` (10 min). Active only when `NEO_AUTO_SUMMARIZE=true` AND `NEO_MC_PRIMARY=true`. Set to `0` to disable Piece C while keeping the Piece B sunset-poll active. Catches the hard-crash edge case where neither a graceful sunset event nor an SSE-disconnect signal fired. |
