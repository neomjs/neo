 ...
 * ## Scope
 *
 * The population is `lint-staged` only, which is also why this guard belongs beside the guards it
 * governs: the registry it reads names `buildScripts/util/*` carriers, so it is only answerable in
...
const
    ...
    REGISTRY_REL = 'buildScripts/util/check-guard-ci-parity-registry.json',
...
/**
 * @summary Reads the accepted-exception registry, naming its own absence.
 *
 * An unreadable registry used to surface as a raw `ENOENT` stack from the constant that resolved
 * it, which reads as a crashed guard rather than a misconfigured one. Absence cannot be softened
 * into "nothing is accepted" either: every client-only guard would then report as unmirrored and
 * the output would blame the repository for a missing file.
 * @returns {Object} the parsed registry
 */
function readRegistry() {
    if (!fs.existsSync(REGISTRY_PATH)) {
        console.error(`[check-guard-ci-parity] registry not found: ${path.relative(REPO_ROOT, REGISTRY_PATH)}`);
        console.error('[check-guard-ci-parity] it ships beside this guard; NEO_GUARD_CI_PARITY_REGISTRY overrides the path.');
        process.exit(1)
    }
