...
9.6. **Core-Idiom Pre-Flight for instance & reactive-state work (operator-ratified, 2026-07-04; scope operator-challenged same day: the gate follows the CLASS SYSTEM, not the directory):** If the ticket's implementation creates, mutates, resolves, or destroys Neo instances — or manages reactive state (configs/providers) — in ANY hemisphere (`apps/`, `src/`, and the many `ai/` services/daemons that are `Neo.setupClass` classes), you MUST read the relevant neo-core contracts BEFORE branching, and the intake record MUST name which were read: `src/core/Base.mjs` (reactive configs, `set()` batching, `observeConfig`, destroy semantics), `src/Neo.mjs` (`Neo.create`/`setupClass` gatekeeping, `Neo.get`/instance-manager resolution), and — for state with multiple consumers — `src/state/Provider.mjs` (hierarchical data proxy, formulas, `stores_` exposure; consumers `bind` one declarative surface instead of reaching into a specific component. NOTE the topology: the whole component tree lives in the shared app worker — windows are render targets, so ALL worker-side state, provider data and component configs alike, is naturally window-agnostic). Live-instance mutation goes through the batched `set()` path and the core instance manager; view/shared state lives as reactive configs; long-running services honor the lifecycle contracts (`initAsync`/`ready()` settle-or-reject on restart, `registerAsync`/`trap` so destroy cancels pending async) — never external state modules or chained direct property writes. **The allowed half, explicitly:** pure data-plane logic (parsers, validators, transition tables) as plain util modules is fine and childapp-precedented — this gate covers instance mutation + view state only. Empirical anchor: the create-module correction arc (2026-07-04) shipped two core-idiom violations past a full review cycle because prior-art reads were Brain-side (`ai/`) siblings; the hemisphere boundary maps to coding idiom, and the sole cross-family reviewer cannot carry the core-read alongside every diff.
