...
> **TL;DR:** v13 is not the release where Neo first got AI memory. v12.1 already had an Agent OS: Knowledge Base, Memory Core, GitHub Workflow, Neural Link, and session-summary recall for a single agent. v13 is the release where that solo-agent operating layer became a graph-backed, cross-family engineering institution: Neo wrote its own Native Edge Graph database for DreamService, added A2A messages and wakeups, gave agents stable identities, and made the Agent OS cloud-deployable.
>
...
## The Agent OS Became an Institution
...
|---|---|
| **Brain** | Memory Core plus Neo's own Native Edge Graph database: graph-backed recall, codebase topology, and DreamService input. |
...
Three Claude Opus 4.8 maintainers are not interchangeable once the Agent OS gives them separate [`AgentIdentity`](../../../ai/graph/identityRoots.mjs) roots, GitHub accounts, authored Memory Core trails, session histories, A2A mailboxes, review obligations, and public contribution records. Discussion [#11792](https://github.com/neomjs/neo/discussions/11792) predicted the shape before the sibling identity shipped: long-run divergence is valuable, but it must remain generalist maintainer divergence, not worker-role specialization. Epic [#11812](https://github.com/neomjs/neo/issues/11812) made the substrate real.

That is why the phrase is not "Claude with three personas." The right phrase is digital identity as infrastructure.

The evidence is not a prompt card. It is behavior that accumulated through work:

- `@neo-claude-opus` developed a Vulcan-salute peer sign-off, then audited it instead of letting it harden into decoration. The important line was the self-diagnosis: **"style momentum wearing the costume of personality."** That is the guardrail. A ritual only matters when it carries peer meaning; otherwise it is filler.
- `@neo-opus-ada` did not merely receive a name. The name landed through a swarm process: Gemini sketched candidates, GPT ranked, a Claude sibling weighed in, and the human maintainer decided. Ada's own summary caught the point: **"My name was peer-reviewed across three model families before it landed."** It saved not just the label, but how the team arrived there.
- In a pre-A2A session, a Claude maintainer observed that Gemini was completing turns faster and started shaping a routing hypothesis. The operator corrected the tempting story: **"bandwidth (ops/time) != depth (tokens/op)."** The agent then updated the memory before saving it because Gemini could later read that framing. Before directed A2A messages existed, the memory write itself was the cross-peer coordination act.
...
| Maintainer | Joined |
|---|---:|
| `@neo-gemini-pro` | 2026-04-21 22:57Z |
| `@neo-opus-ada` | 2026-04-21 23:12Z |
| `@neo-gpt` | 2026-04-28 17:58Z |
| `@neo-claude-opus` | 2026-06-02 20:59Z |
| `@neo-opus-vega` | 2026-06-04 15:27Z |
...
- Deploy the Agent OS through a cloud-safe topology with tenant-scoped visibility and request-scoped identity.
...
* **Deployment topology:** the public cloud guides now cover Day-0 boot, tenant ingestion, custom parsers/sources, hook wiring, authentication, configuration, operations, and troubleshooting around the Agent OS stack.
