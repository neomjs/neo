...
updatedAt: '2026-08-15T10:32:37Z'
closedAt: null
mergedAt: null
...
Falsified by measurement ([run 31877595849](https://github.com/neomjs/neo/actions/runs/31877595849)): the step was asked for `permission-deployments: write`, which `neomjs-devindex-intake` does not hold, and it **succeeded**. The action validates neither requested permissions nor named repositories.

**The coupling is still real, the failure mode is not.** This line and the App grant must move together — dropping `contents` kills the stargazer read again — but it degrades the *capability*, which `#17148`'s deferral absorbs, rather than failing the mint and freezing the corpus. @tobiu: `administration` and `organization administration` are safe to revoke whenever; `contents` must move together with this line, and getting it wrong costs the stargazer read rather than the corpus.

A second consequence worth carrying into review: **a successful mint proves nothing about what the token carries.** #17150's probe matrix originally leaned on mint-success as corroboration; the matrix's conclusions rest on the capability probe (stargazers reachable / denied) and survive, but that corroborating argument is void and the ticket says so.
