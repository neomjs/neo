...
### 4.1 Health & Operator Synchronization
...
*   **`manage_pr_review`**: The **formal PR review** primitive (`action`: `create` | `update`). Atomically sets GitHub's visible `reviewDecision` (`APPROVED` / `REQUEST_CHANGES` / `COMMENT`) with the body and pre-submit validation. Post-cutover PRs receive **one** ordinary `REQUEST_CHANGES` **per reviewer family** — the budget is charged to the submitting family, not globally, so one family spending its round leaves another family's independent round intact. A second round from the same family requires an unused repair-minted re-entry receipt naming four checkable facts. The managed path appends `[review-budget-managed]`, fails closed on incomplete history, preserves that audit across body updates, and permits only one complete terminal Drop+Supersede (`REQUEST_CHANGES`) or a durable named override. An ordinary Cycle 2 is a **disposition** over Cycle 1's required actions (`pr-review-round-2-template.md`), never a fresh action packet. Direct `gh pr review` / UI submission is a bypass-with-telemetry, not equivalent enforcement: run the meter and disclose `[review-budget-bypass] reason: ...`; workflow lint can only react after submission. Review bodies follow the [`pr-review` skill](../../.agents/skills/pr-review/references/pr-review-guide.md).
*   **`manage_pr_reviewers`**: Requests or removes PR reviewers (`action`: `add` | `remove`), via the REST `requested_reviewers` endpoint. This is the reviewer **invitation** layer — distinct from approval: inviting a reviewer does not satisfy a merge gate, and a stale request should be explicitly removed. `add` first requires a positive live mergeability verdict; confirmed conflicts and unresolved computation refuse before mutation, while `remove` stays available.
