...
> **Status: iteration 3 of N — a staging draft.** The story, scope figures and upgrade guidance are still being checked before the 13.2 cut. The release designation and Full Changelog are pending.
...
```mermaid
flowchart TD
    gesture["Main thread<br/>each window<br/>pointer gesture<br/>live preview"]
    reducer["App Worker<br/>applyDockZoneOperation<br/>one pure reducer"]
    doc[("dockModel<br/>the committed document")]
    project["App Worker<br/>LayoutAdapter.project<br/>+ Reconciler"]
    store[("Persistence")]
...
Two delta repairs came out of the Workstation film. When a node's children collapse to text, the delta that cleared the old children had run after the one that wrote the text, which emptied a merged window's first tab label; [the text now survives](https://github.com/neomjs/neo/pull/19189). A node the diff moves into a freshly inserted subtree now [lands at its DOM index](https://github.com/neomjs/neo/pull/19191): a text child renders as three DOM nodes, and the logical index had put a moved tab handle inside the text's comment wrapper.
...
Claiming an issue is explicit too. [`CONTRIBUTING.md`](https://github.com/neomjs/neo/pull/19195) says what an assignment is for: the first approve-worthy PR for an issue is the one merged, so being assigned protects your time. A claim on a `good first issue` or `help wanted` issue [gets an automatic reply](https://github.com/neomjs/neo/pull/19211) saying whether you are first in line, or who holds it and up to three open alternatives. The reply never assigns anyone; a maintainer confirms.
